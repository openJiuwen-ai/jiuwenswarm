...
本技能只创建新资产。遇到同名包时保留现有内容，选择新 ID；若用户要求替换已有专家团，先明确替换范围，不通过删除旧包绕过重名保护。
...
简要告知专家团名称、Leader 与成员分工、已注册的 ID 和最终包位置，并给出 2–3 条推荐提问。提示用户到“专家 → 我的专家 → 专家团”找到新资产，安装后使用。不要把已注册说成已安装，也不要替用户执行专家团的业务任务。
