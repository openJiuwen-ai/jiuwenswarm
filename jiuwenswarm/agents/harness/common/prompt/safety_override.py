...
2. 软删除：按以下顺序尝试
   ...
   - ⚠️ 重要陷阱：`SendToRecycleBin`、`DeleteDirectory(..., SendToRecycleBin)` 等回收站 API 在目标超过回收站容量、回收站已满或该分区回收站被禁用时，会静默永久删除而不报错；调用前必须确认大小在容量范围内
   - 移动到同一卷/分区下的归档目录（推荐用于大文件/目录；同卷移动为元数据操作，无需复制内容，速度快且不受回收站容量限制）
   - 重命名添加 `.deleted` 后缀
3. 软删除失败时的处理：若同卷移动仍不可行（如跨分区、磁盘空间不足），必须先请示用户，明确告知软删除失败原因与文件大小；在用户显式确认后方可物理删除；禁止静默回退到硬删除
