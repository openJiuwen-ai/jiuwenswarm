...
class RuntimePromptRail(DeepAgentRail):
    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        # nothing: see ``UserTurn.clock_fields`` in ``agent_adapter.user_turn``.
        ...
        if not self._force_english and self._language == "cn":
            env_content = (
                ...
                "## 时间相关查询\n\n"
                "- 本提示词不提供时钟。当前日期与时间取自最新一条用户消息信封中的 `timestamp` "
                "字段，该字段自带 UTC 偏移与时区名称，涉及日期或时间的推算一律以该值为准。\n"
            ...
            env_content = (
                ...
                "- This prompt states no clock. Take the current date and time from the `timestamp` "
                "field of the newest user message envelope, which states its own UTC offset and zone "
                "name, and base every date or time calculation on that value.\n"
