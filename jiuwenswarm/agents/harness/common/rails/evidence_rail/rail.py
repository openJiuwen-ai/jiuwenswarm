...
# Runtime failures the rail converts into a stable reason code.  ``ValueError``
# is listed alone on purpose: pydantic's ``ValidationError`` and
# ``PydanticSerializationError`` and ``json.JSONDecodeError`` all subclass it,
# so naming them as well would only create overlapping except clauses.
# ``RecursionError`` covers a self-referential payload reaching the
# canonicalizer, ``AttributeError`` a duck-typed collaborator that does not
# implement the expected surface.
_DATA_ERRORS: tuple[type[Exception], ...] = (
    AttributeError,
    RecursionError,
    TypeError,
    ValueError,
...
# A store additionally touches the filesystem.
_STORE_ERRORS: tuple[type[Exception], ...] = (OSError, *_DATA_ERRORS)
...
class EvidenceRail(DeepAgentRail):
    ...
    types its call can actually raise (see ``_DATA_ERRORS`` / ``_STORE_ERRORS``)
    and converts them into a stable reason code plus an explicit
    ``request_force_finish``.  The rail deliberately does not add a broad
    last-resort handler of its own -- ``CallbackFramework.trigger`` already is
    the designated boundary for the callback chain: it catches ``Exception``
    from a callback, logs it with a traceback and continues.  An exception
    outside the named tuples is therefore a defect, and it surfaces there with
    its traceback instead of being flattened into a reason code.
    ``asyncio.CancelledError`` derives from ``BaseException``, so no clause
    here can swallow cancellation.
    """
    ...
    async def before_model_call(self, ctx: AgentCallbackContext) -> None:
        ...
        if prompt_builder is None:
            # ``init`` never ran, or the agent exposes no prompt builder: the
            # evidence projection cannot reach the model, so fail closed.
            self._block(ctx, state, REASON_CONTEXT_INJECTION_FAILED)
            return
        try:
    ...
    def _resolve_evidence(
        ...
        *,
        ...
        if raw is None:
            return list(
                fallback if fallback is not None else self.config.evidence_items
            )
    ...
    def _block(
        ...
        try:
            self.store.save_manifest(state.manifest)
        except _STORE_ERRORS as store_exc:
    ...
    def _force_finish(
        ...
        # Only the exception's class name is exported: the message may carry
        # tool arguments or credentials and must not reach a persisted or
        # user-visible artifact.
        evidencerail: dict[str, str] = {
            "status": "blocked",
            "reason_code": reason_code,
        }
        if exception is not None:
            evidencerail["error_type"] = type(exception).__name__
