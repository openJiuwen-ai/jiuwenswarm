...
class RootAskUserClarification:
    ...
    def __post_init__(self) -> None:
        ...
        option_count_invalid = bool(self.options) and not 2 <= len(self.options) <= 4
        labels_invalid = len(labels) != len(set(labels)) or "Other" in labels
        if option_count_invalid or labels_invalid:
...
def _parse_permission_user_content(
    ...
    if envelope is None:
        return None, False
    keys = set(envelope)
    if not _REQUIRED_ENVELOPE_KEYS <= keys:
        return None, False
    if not keys <= _REQUIRED_ENVELOPE_KEYS | _OPTIONAL_ENVELOPE_KEYS:
        return None, False
