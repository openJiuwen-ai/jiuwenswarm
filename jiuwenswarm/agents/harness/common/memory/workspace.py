...
def load_workspace_memory_config(config_file: Path) -> dict[str, Any]:
    """Read the target instance's config and overlay, including env placeholders."""
    from jiuwenswarm.common.config import merge_config_layers, resolve_env_vars
    ...
    overlay = config_file.with_name("config.user.yaml")
    if overlay.is_file():
        config = merge_config_layers(
            config, yaml.safe_load(overlay.read_text(encoding="utf-8")) or {}, sparse=True,
        )
