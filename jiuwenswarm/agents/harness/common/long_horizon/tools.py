...
class LongHorizonActions:
    def __init__(
        ...
        *,
        ...
        self.cron = cron
        if service_id != "default" or agent_id != "default":
            self.service_id = service_id
            self.agent_id = agent_id
        else:
            self.service_id, self.agent_id = _runtime_tenant_ids()
...
def _stamp_task_owner(task: LongHorizonTask) -> None:
    if str(getattr(task, "owner_user_id", "") or "").strip():
        return
    try:
        from jiuwenswarm.extensions.identity_provider.store import IdentityStore

        ident = IdentityStore.get_identity()
        uid = str(getattr(ident, "user_id", "") or "").strip() if ident else ""
        if uid:
            task.owner_user_id = uid
    except Exception as exc:
        logger.debug("[long_horizon] owner_user_id stamp skipped: %s", exc)


def _runtime_tenant_ids() -> tuple[str, str]:
    try:
        from jiuwenswarm.common.local_env_config import get_bound_agent_env_ns

        ns = get_bound_agent_env_ns()
        if ns and len(ns) >= 2:
            sid = str(ns[0] or "").strip() or "default"
            aid = str(ns[1] or "").strip() or "default"
            return sid, aid
    except Exception as exc:
        logger.debug("[long_horizon] tenant ids fallback default: %s", exc)
