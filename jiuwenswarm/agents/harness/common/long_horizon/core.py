...
_REWRITE_SKIP_DUE_STATUSES = _PROTECTED_STAGE_STATUSES | {"due"}
...
def rewrite_task_stages(
    ...
    *,
    ...
    return fill_stage_due_dates(
        ...
        minute=minute,
        now=now,
        skip_statuses=_REWRITE_SKIP_DUE_STATUSES,
    )
...
def mutate_long_horizon_task(
    workspace: str | Path,
    task_id: str,
    mutator: Any,
) -> LongHorizonTask | None:
    """Load-modify-save one task under ``_LOCK``. ``mutator`` returns False to abort."""
    if not callable(mutator):
        raise TypeError("mutator_must_be_callable")
    normalized = str(task_id or "").strip()
    if not normalized:
        return None
    with _LOCK:
        ...
        for index, current in enumerate(tasks):
            if current.id != normalized:
                continue
            committed = mutator(current)
            if committed is False:
                return current
            current.updated_at = touch_iso()
            tasks[index] = current
            _save_long_horizon_tasks(workspace, tasks)
            return current
    return None
...
def list_inbox(workspace: str | Path) -> list[dict]:
    """Due, snooze-expired, and overdue pending stages for active tasks."""
    ...
    for task in load_long_horizon_tasks(workspace):
        ...
        for stage in task.stages:
            if stage.status in ("done", "skipped", "in_progress"):
                continue
            item = {"task": task.to_dict(), "stage": stage.to_dict()}
            if stage.status == "due":
                inbox.append(item)
                continue
            raw = ""
            if stage.status == "snoozed":
                raw = str(stage.snooze_until or stage.due_at or "")
            elif stage.status == "pending":
                raw = str(stage.due_at or "")
            if not raw:
                continue
            try:
                ...
                logger.debug(
                    "[long_horizon] inbox due parse skipped task=%s stage=%s: %s",
                    task.id,
                    stage.id,
                    exc,
                )
