...
def build_due_payload(
    ...
    *,
    ...
    user_id = str(getattr(task, "owner_user_id", "") or "").strip()
    if not user_id:
        try:
            ...
            meta = get_session_metadata(
                task.exec_session_id, enable_writeback=False
            )
            if isinstance(meta, dict):
                user_id = str(meta.get("user_id") or "").strip()
        except Exception as exc:
            logger.debug("[long_horizon] due payload user_id skipped: %s", exc)
            user_id = ""
    return {
...
async def mark_stage_due(
    ...
    *,
    ...
    error = ""
    already_due = False
    stage_ref: LongHorizonStage | None = None

    def _mutator(task: LongHorizonTask) -> bool:
        nonlocal error, already_due, stage_ref
    ...
    task = mutate_long_horizon_task(workspace, task_id, _mutator)
    if task is None:
...
async def _push_due_toast(payload: dict[str, Any], *, channel_id: str) -> None:
    ...
    try:
        ...
        if delivered is not None and int(delivered) < 1:
            logger.warning(
                "[long_horizon] due toast not delivered task=%s stage=%s subscribers=%s",
                payload.get("task_id"),
                payload.get("stage_id"),
                delivered,
            )
    except Exception as exc:
        logger.warning("[long_horizon] due toast push skipped: %s", exc)
