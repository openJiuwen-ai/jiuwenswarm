...
class JiuWenSwarmDeepAdapter:
    ...
    def _prepare_react_image_tool_prompt(
        ...
        *,
        ...
        #
        # A false ``enable_read_image_multimodal`` covers three separate
        # findings: native input turned off by declaration or configuration, a
        # probe that came back negative, and a capability that is not confirmed
        # either way. The lines below therefore name ``image_input_status``
        # instead of calling all three "disabled". A log line must not state a
        ...
        if not isinstance(query, str):
            logger.warning(
                "[JiuWenSwarmDeepAdapter] Native image input not in use "
                "(image_input_status=%s) and the image-understanding tool prompt "
                "could not be built (no text query): %d image attachment(s) not "
                "routed to the model",
                image_input_status,
                len(image_files),
            )
        ...
        if not first_path:
            ...
            logger.warning(
                "[JiuWenSwarmDeepAdapter] Native image input not in use "
                "(image_input_status=%s) and the image-understanding tool prompt "
                "could not be built (no file path): %d image attachment(s) not "
                "routed to the model",
                image_input_status,
                len(image_files),
            )
        ...
        if not media_items:
            ...
            logger.warning(
                "[JiuWenSwarmDeepAdapter] Native image input not in use "
                "(image_input_status=%s) and the image-understanding tool prompt "
                "could not be built (no usable file path): %d image "
                "attachment(s) not routed to the model",
                image_input_status,
                len(image_files),
            )
            return inputs
        ...
        #
        # The tool context is built either way, but it only reaches a tool when
        # one is configured: without a vision model the very same block carries
        # a hint telling the model to say it cannot see images. Naming both
        # outcomes "routed to the image-understanding tool prompt" would be the
        # assumed outcome this function exists to stop reporting.
        if vision_tool_available:
            logger.info(
                "[JiuWenSwarmDeepAdapter] Native image input not in use "
                "(image_input_status=%s): %d image attachment(s) routed to the "
                "image-understanding tool prompt, not to the model context "
                "window",
                image_input_status,
                len(media_items),
            )
            ...
            logger.warning(
                "[JiuWenSwarmDeepAdapter] Native image input not in use "
                "(image_input_status=%s) and no image-understanding tool is "
                "configured: %d image attachment(s) reach the model as paths "
                "only, and the prompt instructs it to state that it cannot read "
                "them",
                image_input_status,
                len(media_items),
            )
