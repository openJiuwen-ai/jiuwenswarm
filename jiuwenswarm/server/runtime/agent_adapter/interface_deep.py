...
class JiuWenSwarmDeepAdapter:
    ...
    async def _update_session_tools(
        ...
        self._update_clouddoc_tools(session_id, config_base=config_base)

    def _update_clouddoc_tools(
        self, session_id: str | None = None, *, config_base: dict | None = None
    ) -> None:
        ...
        # The caller has already read the config for this update; one read per
        # update, so the two decisions cannot see different snapshots.
        if config_base is None:
            config_base = get_config()
