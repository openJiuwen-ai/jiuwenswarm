    ...
    def _suspend_subagent_permission_rails(subagent: Any) -> Callable[[], None]:
        ...
        for rail in configured or []:
            ...
            try:
                ...
                def _disabled_permission_snapshot(
                    _cfg: dict = suspended_config,
                ) -> dict:
                    """临时权限审批快照：返回「原配置 + enabled=False」。

                    resolve_interrupt 刷新时以此覆盖引擎配置；返回残缺 dict
                    会把 tools/file_guard 二次清空（同 update_config 整体替换）。
                    """
                    return dict(_cfg)
