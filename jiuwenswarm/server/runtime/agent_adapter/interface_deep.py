...
from jiuwenswarm.agents.harness.common.rails.permissions.root_ask_user import (  # noqa: E402
    ASK_USER_TOOL_NAME,
...
from jiuwenswarm.server.runtime.agent_adapter.browser_runtime_security import (  # noqa: E402
...
@dataclass(frozen=True, slots=True)
class _AgentRailBuildSpec:
    """One profile-owned rail factory consumed by the parent composition."""
...
@dataclass(frozen=True, slots=True)
class _AgentProfileRailSpecs:
    """Non-permission rail specs placed around parent-owned required rails."""

    before_execution_context: tuple[_AgentRailBuildSpec, ...] = ()
    before_stream_event: tuple[_AgentRailBuildSpec, ...] = ()
    before_permission: tuple[_AgentRailBuildSpec, ...] = ()
    after_permission: tuple[_AgentRailBuildSpec, ...] = ()


@dataclass(slots=True)
class _RootPermissionDispatchHandoff:
    """Request-local owner from input preparation through Core admission."""
...
_AGENT_COMPOSITION_SCOPES = frozenset(
    {"single_agent", "team_root", "team_member", "auto_harness"}
)
...
_ROOT_PERMISSION_ANSWER_KEY = "_jiuwenswarm_root_permission_answer"
_ROOT_PERMISSION_HANDOFF_KEY = "_jiuwenswarm_root_permission_handoff"
...
class JiuWenSwarmDeepAdapter:
    ...
    def __init__(self) -> None:
        ...
        # M5 staging boundary: M3 derives this from the authoritative host
        # snapshot and publishes the same value to ordinary child agents.
        ...
        *,
        ...
        if adapter.should_defer_permission_reload(
            ...
            logger.info(
                ...
                "session_id=%s version=%s reason=%s",
                session_id,
                current_version,
                "permission_reload_safe_boundary_unavailable",
            )
            return
        try:
    ...
    async def _evict_failed_permission_session_adapter(
        ...
        """Make a partially updated child unreachable before cleanup."""
        ...
        try:
            ...
            logger.warning(
                "[JiuWenSwarmDeepAdapter] failed permission child cleanup: "
    ...
    async def _get_session_adapter_for_request(
        ...
        *,
        ...
        """Select a child and best-effort publish Permission at a safe boundary.
        ...
        is busy, the desired version remains pending and this input continues
        on the complete previously installed policy epoch.
        """

        if (
            self._is_interrupt_resume_dispatch(request.params)
            and self._get_cached_session_adapter(request.session_id) is None
        ):
    ...
    def _has_live_root_permission_owner(
        ...
        """Protect queue, handoff and cutover ownership from replacement."""
        ...
        if self._root_permission_queue.has_live(root_session_id=root_session_id):
            return True
        lock = getattr(self, "_root_permission_dispatch_lock", None)
        if isinstance(lock, asyncio.Lock) and lock.locked():
            return True
        handoff = getattr(self, "_root_permission_handoff", None)
        return bool(
            isinstance(handoff, _RootPermissionDispatchHandoff)
            and not handoff.closed
    ...
    def _permission_delta_for_reload_preflight(
        ...
        """Compare a reload request before mutating a built root adapter."""
        ...
        if has_environment_overrides and has_environment_permissions:
            ...
            return True
        return self._has_permission_config_delta(resolved)
    ...
    async def _discard_frozen_permission_continuation(
        ...
        *,
        ...
        if (
            ...
            or self._deep_agent_loop_session_id() != target_sid
            or not frozen_keys
        ):
            return False
        ...
        try:
            ...
            logger.debug(
                "[JiuWenSwarmDeepAdapter] exact permission continuation discard failed "
                "session=%s",
                target_sid,
                exc_info=True,
            )
        ...
        logger.info(
            "[JiuWenSwarmDeepAdapter] discarded exact permission continuation "
            "session=%s cards=%d",
            target_sid,
            len(frozen_keys),
        )
    ...
    def _prepare_browser_runtime_security(
        ...
        """Build one guarded browser settings/profile pair for this adapter."""
        ...
        if not str(os.getenv("BROWSER_DRIVER") or "").strip():
            ...
            logger.info(
                "[%s] browser runtime enabled without BROWSER_DRIVER; "
                "defaulting to managed mode",
                type(self).__name__,
            )
        try:
            probe = build_browser_agent_config(
                model,
                workspace=self._workspace_dir or "./",
                language=self._resolve_runtime_language(),
            )
            factory_kwargs = getattr(probe, "factory_kwargs", None)
            settings = (
            ...
            self._browser_runtime_settings = replace(settings, mcp_cfg=guarded_cfg)
    ...
    def _reload_browser_runtime_security_profile(
        self,
    ) -> BrowserRuntimeSecurityProfile | None:
        """Return a conservative profile until reload config is installed."""
        profile = self._browser_runtime_security_profile
        if profile is None or not profile.network_guard_enforced:
            return profile
        return BrowserRuntimeSecurityProfile(
            failure_reason="browser_runtime_reload_pending",
            egress_guard_failure_reason="egress_guard_unverified",
        )

    def _set_permission_browser_runtime_security_profile(
        self,
        rail: Any | None,
        profile: BrowserRuntimeSecurityProfile | None,
    ) -> None:
        """Publish host-owned browser evidence only to an Auto rail."""
        if rail is not None and isinstance(rail, self._permission_rail_types()[1]):
            rail.browser_runtime_security_profile = profile
    ...
    def _build_subagents_with_general_purpose(
        ...
        *,
        ...
        if (
            ...
            is self._root_permission_queue
        ):
            excluded.add(id(stream))
        candidates = [
            rail
            for rail in rails or []
            if not isinstance(rail, SubagentRail) and id(rail) not in excluded
        ]
        if reload:
        ...
        if not self._filesystem_rail_enabled_for_profile():
            candidates = [r for r in candidates if not isinstance(r, SysOperationRail)]
        if not any(isinstance(r, SysOperationRail) for r in candidates):
            candidates.insert(0, SysOperationRail())
        self._general_purpose_rail_snapshot = tuple(candidates)
        if not (allow_general and add_general):
            return subagents
        result = list(subagents or [])
        if any(spec.agent_card.name == "general-purpose" for spec in result):
            return result
        result.insert(
            0,
            SubAgentConfig(
                agent_card=AgentCard(
                    ...
                    description=GENERAL_PURPOSE_AGENT_DESC.get(
                        workspace.language, GENERAL_PURPOSE_AGENT_DESC["cn"]
                    ),
                ),
    ...
    def _resolve_model_by_name(self, requested_model_name: str = "") -> Model | None:
        ...
        if keys:
            ...
            if resolved is not None:
                return resolved
        # 通道侧用全局 origin_index 拼出的 {model_name}#{index}，与后端 per-name
        # cache_key 序号语义不一致，上面两步均不命中。用 _global_index_to_cache_key
        # 把通道传入的全局序号换算成真实 cache_key；换算失败时记日志并回退默认模型，
        # 绝不静默取同名组首个条目（否则会把 agentos 同名第 2+ 个条目误解析成
        # defaults 的同名首条目，api_base/api_key 全错且无任何告警）。
        if "#" in requested:
    ...
    def _build_stream_event_rail(self) -> JiuSwarmStreamEventRail | None:
        ...
        try:
            stream_event_rail = JiuSwarmStreamEventRail(
                root_permission_queue=self._root_permission_queue,
            )
    ...
    def _instantiate_rails(
        self,
        rail_infos: list[_RailBuildInfo | _AgentRailBuildSpec],
        config_base: dict[str, Any],
    ) -> list[Any]:
    ...
    def _build_profile_rail_specs(
        ...
        *,
        ...
        """Return Deep-specific, non-permission rails for parent composition."""
        before_execution_context: list[_AgentRailBuildSpec] = []
        if self._filesystem_rail_enabled_for_profile():
            before_execution_context.append(
                _AgentRailBuildSpec("_filesystem_rail", self._build_filesystem_rail)
            )
        else:
            self._filesystem_rail = None
        before_execution_context.extend(
            (
                _AgentRailBuildSpec(
                    "_skill_rail",
        ...
        if isinstance(mode, str) and mode.startswith("agent"):
            after_permission.extend(
                (
                    _AgentRailBuildSpec(
                        "_ask_user_rail",
                        self._build_structured_ask_user_rail,
                    ),
                    _AgentRailBuildSpec(
                        "_work_agent_mode_rail",
                        self._build_work_agent_mode_rail,
                    ),
                    _AgentRailBuildSpec(
                        "_work_plan_approval_rail",
                        self._build_work_plan_approval_rail,
                    ),
                )
            )
        return _AgentProfileRailSpecs(
            before_execution_context=tuple(before_execution_context),
            before_stream_event=(
            ...
            before_permission=(
                _AgentRailBuildSpec("_task_planning_rail", self._build_task_planning_rail),
                _AgentRailBuildSpec("_security_rail", self._build_security_rail),
                _AgentRailBuildSpec("_heartbeat_rail", self._build_heartbeat_rail),
                _AgentRailBuildSpec(
                    "_model_anomaly_detection_rail",
                    self._build_model_anomaly_detection_rail,
                    {"config_base": config_base},
                ),
                _AgentRailBuildSpec("_circuit_breaker_rail", self._build_circuit_breaker_rail),
                _AgentRailBuildSpec("_avatar_rail", self._build_avatar_rail),
                _AgentRailBuildSpec(
                    "_memory_forbidden_rail",
                    self._build_memory_forbidden_rail,
                ),
            ),
    ...
    def _auto_permission_capability_enabled(self) -> bool:
        """Return whether this profile may construct the Auto permission rail."""
        return (
            _resolve_agent_composition_scope(
    ...
    @staticmethod
    def _profile_rail_infos(
        specs: _AgentProfileRailSpecs,
    ) -> tuple[_AgentRailBuildSpec, ...]:
        return (
            *specs.before_execution_context,
            *specs.before_stream_event,
            *specs.before_permission,
            *specs.after_permission,
        )

    @staticmethod
    def _validate_profile_rail_infos(
        rail_infos: tuple[_AgentRailBuildSpec, ...],
    ) -> None:
        for info in rail_infos:
            if info.attr_name in _REQUIRED_AGENT_RAIL_ATTR_NAMES:
                raise RuntimeError(f"profile_rail_reserved_attr_name:{info.attr_name}")

    def _instantiate_agent_rail_specs(
        self,
        rail_infos: tuple[_AgentRailBuildSpec, ...],
    ) -> list[Any]:
        rails: list[Any] = []
        for info in rail_infos:
            ...
            rail = info.build_func(**(info.params or {}))
            if rail is None:
                if info.attr_name in _REQUIRED_AGENT_RAIL_ATTR_NAMES:
                    setattr(self, info.attr_name, None)
                logger.warning(
                    "[JiuWenSwarmDeepAdapter] Rail %s build returned None",
                    info.attr_name,
                )
                continue
            setattr(self, info.attr_name, rail)
            rails.append(rail)
        return rails
    ...
    def _append_optional_agent_rails(
        ...
        try:
            hooks_config = load_hooks_config(config_base)
            if hooks_config.events:
                rails.append(UserHookRail(hooks_config))
        except Exception as exc:
            logger.warning("[JiuWenSwarmDeepAdapter] Failed to load UserHookRail: %s", exc)
        try:
            from openjiuwen.agent_teams.observability.rail import ObservabilityRail

            rails.append(ObservabilityRail())
        except Exception as exc:
            logger.warning(
                "[JiuWenSwarmDeepAdapter] Failed to attach ObservabilityRail: %s",
                exc,
            )
        return rails

    def _build_excluded_profile_rails(
        self,
        config: dict[str, Any],
        config_base: dict[str, Any],
        *,
        ...
        """Build profiles that do not own the generic Permission lifecycle."""
        if composition_scope not in _AGENT_COMPOSITION_SCOPES - {"single_agent"}:
            raise RuntimeError(f"agent_composition_scope_invalid:{composition_scope}")
        ...
        self._permission_rail = None

        specs = self._build_profile_rail_specs(config, config_base, mode=mode)
        profile_infos = self._profile_rail_infos(specs)
        self._validate_profile_rail_infos(profile_infos)
        rail_infos = (
            *specs.before_execution_context,
            *specs.before_stream_event,
            _AgentRailBuildSpec("_stream_event_rail", lambda: JiuSwarmStreamEventRail()),
            *specs.before_permission,
            *specs.after_permission,
        )
        rails = self._instantiate_agent_rail_specs(rail_infos)
        return self._append_optional_agent_rails(rails, config_base)
    ...
    def _validate_required_agent_rails(
        ...
        *,
        ...
        stream_event_rail: JiuSwarmStreamEventRail,
        permission_rail: Any | None,
        permission_config: Any,
        enable_auto_permission: bool,
    ) -> None:
        """Validate the exact parent-constructed required rail objects."""
        required_rails = (
            (
                "_root_permission_queue_rail",
                queue_rail,
        ...
        permission_enabled = bool(
            isinstance(permission_config, dict)
            and is_permission_boundary_enabled(permission_config)
        )
        if len(permission_rails) != int(permission_enabled):
        ...
        if not permission_enabled and (
            permission_rail is not None or self._permission_rail is not None
        ):
            raise RuntimeError("disabled_permission_rail_identity_mismatch")
        if permission_enabled:
            auto_rail_type = permission_types[1]
            if isinstance(permission_rails[0], auto_rail_type) is not enable_auto_permission:
    ...
    def _build_agent_rails(
        ...
        *,
        ...
        """Compose parent-required rails with profile-owned Deep rails."""
        if composition_scope not in _AGENT_COMPOSITION_SCOPES:
            raise RuntimeError(f"agent_composition_scope_invalid:{composition_scope}")
        if composition_scope != "single_agent":
            return self._build_excluded_profile_rails(
                config,
                config_base,
                mode=mode,
        ...
        root_context_rail = RootContextRail(
            root_permission_queue=self._root_permission_queue,
            expected_agent_id=self._agent_name,
            sys_operation=self._sys_operation,
            sandboxed=bool(
                self._sys_operation_card is not None
                and self._sys_operation_card.mode == OperationMode.SANDBOX
            ),
        )
        root_permission_queue_rail = RootPermissionQueueRail(
            self._root_permission_queue,
            answer_claimed=self._close_claimed_root_permission_handoff,
        )
        root_permission_completion_rail = RootPermissionCompletionRail(
            self._root_permission_queue,
        )
        profile_specs = self._build_profile_rail_specs(config, config_base, mode=mode)
        self._validate_profile_rail_infos(self._profile_rail_infos(profile_specs))

        stream_event_rail: JiuSwarmStreamEventRail | None = None
        permission_rail: Any | None = None

        def build_required_stream_event_rail() -> JiuSwarmStreamEventRail | None:
            nonlocal stream_event_rail
            stream_event_rail = self._build_stream_event_rail()
            return stream_event_rail

        def build_required_permission_rail() -> Any | None:
            nonlocal permission_rail
            permission_rail = build_permission_rail(
                config=config_base,
                llm=self._model,
                ...
                enable_auto_permission=self._enable_auto_permission,
                workspace_root=(
                    self._permission_workspace_root
                    if self._enable_auto_permission
                    else self._workspace_dir
                ),
                platform_trusted_root=(
                    self._platform_trusted_root
                    if self._enable_auto_permission
                ...
                browser_runtime_security_profile=(
                    self._browser_runtime_security_profile
                ),
                trusted_search_urls=(
                    self._trusted_search_urls
                    if self._is_session_scoped_adapter
        ...
        rail_infos = [
            _AgentRailBuildSpec(
                "_root_permission_queue_rail",
                lambda: root_permission_queue_rail,
            ),
            *profile_specs.before_execution_context,
            _AgentRailBuildSpec(
                "_root_context_rail",
                lambda: root_context_rail,
            ),
            *profile_specs.before_stream_event,
            _AgentRailBuildSpec(
                "_stream_event_rail", build_required_stream_event_rail
            ),
            *profile_specs.before_permission,
            _AgentRailBuildSpec(
                "_permission_rail",
                build_required_permission_rail,
            ),
            _AgentRailBuildSpec(
                "_root_permission_completion_rail",
                lambda: root_permission_completion_rail,
            ),
            *profile_specs.after_permission,
        ]
        ...
        rails_list = self._instantiate_agent_rail_specs(tuple(rail_infos))
        if stream_event_rail is None:
            raise RuntimeError("required_agent_rail_missing:JiuSwarmStreamEventRail")
        if (
            isinstance(permission_config, dict)
            and is_permission_boundary_enabled(permission_config)
            and permission_rail is None
        ):
            raise RuntimeError("required_agent_rail_missing:permission")
        self._validate_required_agent_rails(
            rails_list,
            queue_rail=root_permission_queue_rail,
            completion_rail=root_permission_completion_rail,
            root_context_rail=root_context_rail,
            stream_event_rail=stream_event_rail,
            permission_rail=permission_rail,
            permission_config=permission_config,
            enable_auto_permission=self._enable_auto_permission,
        )
        return self._append_optional_agent_rails(rails_list, config_base)
    ...
    def _permission_rails_for_reload(self) -> list[Any]:
        """Return public permission rails plus any failed-unregister pointer."""
        if self._instance is None:
            return [self._permission_rail] if self._permission_rail is not None else []

        find_rails = getattr(self._instance, "find_rails_by_type", None)
        permission_rails = (
            list(find_rails(self._permission_rail_types()))
            if callable(find_rails)
            else []
        )
        seen: set[int] = set()
        result: list[Any] = []
        for rail in permission_rails:
            if rail is None or id(rail) in seen:
                continue
            seen.add(id(rail))
            result.append(rail)
        if self._permission_rail is not None and id(self._permission_rail) not in seen:
            result.append(self._permission_rail)
        return result

    def _prepare_permission_rail_candidate(
        self,
        config_base: dict[str, Any],
        *,
        enable_auto_permission: bool,
        existing_rails: list[Any],
    ) -> Any | None:
        """Build or select a candidate without mutating the live rail pointer."""
        permission_config = config_base.get("permissions", {})
        auto_rail_type = self._permission_rail_types()[1]
        is_registered = getattr(self._instance, "is_registered_rail", None)
        if not enable_auto_permission and self._has_live_root_permission_owner():
            for rail in existing_rails:
                if not isinstance(rail, auto_rail_type):
                    continue
                if callable(is_registered) and is_registered(rail) is not True:
                    continue
                return rail
        if not isinstance(permission_config, dict) or not is_permission_boundary_enabled(
            permission_config
        ):
            return None

        staged_browser_profile = self._reload_browser_runtime_security_profile()
        for rail in existing_rails:
            if isinstance(rail, auto_rail_type) != enable_auto_permission:
                continue
            if callable(is_registered) and is_registered(rail) is not True:
                continue
            return rail

        candidate = build_permission_rail(
            config=config_base,
            llm=self._model,
            model_name=config_base.get("models", {})
            .get("default", {})
            .get("model_client_config", {})
            .get("model_name", "gpt-4"),
            enable_auto_permission=enable_auto_permission,
            workspace_root=(
                self._permission_workspace_root
                if enable_auto_permission
                else self._workspace_dir
            ...
            platform_trusted_root=(
                self._platform_trusted_root
                if enable_auto_permission and self._is_session_scoped_adapter
                else None
            ),
            sys_operation=self._sys_operation,
        ...
        if isinstance(candidate, auto_rail_type) != enable_auto_permission:
            raise RuntimeError("permission_rail_candidate_type_mismatch")
        return candidate
    ...
    async def create_instance(
        ...
        initial_runtime_workspace = str(
            self._permission_workspace_root
            if self._enable_auto_permission
            else self._project_dir or get_default_project_session_workspace_dir()
        )
        ...
        *,
        ...
        Args:
            ...
            permission_delta: Whether the global Permission policy changed.
                Such changes always advance one global lazy version.
        """
        if self._is_session_scoped_adapter:
            return
        if permission_delta or not target_sid:
    ...
    async def reload_agent_config(
        ...
        if self._is_session_scoped_adapter and target_sid:
            ...
            if own_sid != self._session_adapter_key(target_sid):
                ...
                return
        permission_delta = self._permission_delta_for_reload_preflight(
            config_base,
            env_overrides,
        )
        if (
            ...
            and permission_delta
        ):
            raise RuntimeError("root_permission_reload_requires_restart")
        if self._instance is None:
            if self._is_session_scoped_adapter:
        ...
        composition_scope = _resolve_agent_composition_scope(
            self._session_instance_mode,
            self._session_instance_sub_mode,
        )
        permission_config = config_base.get("permissions", {})
        candidate_auto_permission = self._auto_permission_enabled_for_config(
            permission_config,
            composition_scope=composition_scope,
        )
        existing_permission_rails = self._permission_rails_for_reload()
        candidate_permission_rail = None
        if composition_scope == "single_agent":
            candidate_permission_rail = self._prepare_permission_rail_candidate(
                config_base,
                enable_auto_permission=candidate_auto_permission,
                existing_rails=existing_permission_rails,
            )
        candidate_is_existing = any(
            rail is candidate_permission_rail for rail in existing_permission_rails
        )
        ...
        permission_rail_types = self._permission_rail_types()
        deep_cfg.rails = [
            rail
            for rail in (deep_cfg.rails or [])
            if not isinstance(rail, permission_rail_types)
        ]
        deep_cfg.permissions = None
        ...
        # Complete fallible non-Permission preparation before changing the
        # installed policy epoch. The parent evicts this child on any failure.
        ...
        unregister_rail = getattr(self._instance, "unregister_rail", None)
        old_permission_rails = [
            rail
            for rail in existing_permission_rails
            if rail is not candidate_permission_rail
        ]
        if old_permission_rails and not callable(unregister_rail):
            raise RuntimeError("permission_rail_unregister_unavailable")
        for old_permission_rail in old_permission_rails:
            await unregister_rail(old_permission_rail)

        if candidate_permission_rail is not None and not candidate_is_existing:
            register_rail = getattr(self._instance, "register_rail", None)
            if not callable(register_rail):
                raise RuntimeError("permission_rail_register_unavailable")
            await register_rail(candidate_permission_rail)

        if candidate_permission_rail is not None and candidate_is_existing:
            update_config = getattr(candidate_permission_rail, "update_config", None)
            if not callable(update_config):
                raise RuntimeError("permission_rail_update_config_unavailable")
            update_config(permission_config)

        # No await inside this publication block: rail config, browser evidence,
        # and adapter pointers are one installed policy epoch.
        self._permission_rail = candidate_permission_rail
        self._enable_auto_permission = candidate_auto_permission
        self._set_permission_browser_runtime_security_profile(
            candidate_permission_rail,
            self._browser_runtime_security_profile,
        )
    ...
    def _ensure_cron_tools_registered(self, session_id: str | None) -> None:
        ...
        scheduler_session = bool(
            session_id and session_id.startswith(("heartbeat", "cron"))
        )
    ...
    async def _acquire_root_permission_handoff(
        self,
        root_session_id: str,
        ...
        if not isinstance(lock, asyncio.Lock):
            lock = asyncio.Lock()
            self._root_permission_dispatch_lock = lock
        await lock.acquire()
        return _RootPermissionDispatchHandoff(lock, root_session_id)

    def _publish_root_permission_cutover(
        self,
    ...
    async def _start_root_permission_cutover(
        self,
        root_session_id: str,
    ) -> _RootPermissionDispatchHandoff | None:
        handoff = await self._acquire_root_permission_handoff(root_session_id)
        if self._publish_root_permission_cutover(handoff):
            return handoff
        handoff.closed = True
        handoff.lock.release()
        return None

    async def _complete_root_permission_cutover(
        self,
        handoff: _RootPermissionDispatchHandoff,
        *,
        session_id: str,
        ...
        try:
            ...
            if isinstance(handoff.superseded_answer, RootPermissionAnswer):
                self._root_permission_queue.consume_accepted_answer_if_quarantined(
                    handoff.superseded_answer
                )
                self._close_claimed_root_permission_handoff(
                    handoff.superseded_answer.card.key
                )
            frozen_keys = self._root_permission_queue.cutover_continuation_scope(
                root_session_id=handoff.root_session_id
            )
    ...
    async def _discard_superseded_permission_before_fresh_input(
        ...
        try:
            ...
            cancel_error = exc
        if not await self._complete_root_permission_cutover(
            handoff,
            session_id=request.session_id,
    ...
    async def _prepare_root_input_dispatch(
        ...
        try:
            ...
            if prepared is None:
                ...
                handoff.closed = True
                return None
            answer = prepared.get(_ROOT_PERMISSION_ANSWER_KEY)
            ...
            prepared[_ROOT_PERMISSION_HANDOFF_KEY] = handoff
            return prepared
        except BaseException:
            if isinstance(handoff.answer, RootPermissionAnswer):
                self._root_permission_queue.release_answer(handoff.answer)
            if getattr(self, "_root_permission_handoff", None) is handoff:
                self._root_permission_handoff = None
            if lock.locked():
                lock.release()
            handoff.closed = True
            raise
    ...
    @staticmethod
    def _prepare_nonpermission_resume(
        loop_session: Any,
    ...
    async def _attach_and_send_inputs(
        ...
        *,
        ...
        if not isinstance(handoff, _RootPermissionDispatchHandoff):
        ...
        try:
            ...
            self._release_permission_resume_dispatch(inputs)
    ...
    async def _consume_unary_output(
        self,
        stream: Any,
        ...
        *,
        ...
        async for chunk in stream:
            ...
            if text:
                collected_content.append(text)
            if str(parsed.get("event_type") or "").strip() in (
                "chat.error",
                "error",
            ):
                error_text = str(parsed.get("error") or parsed.get("message") or "")
        return error_text
    ...
    async def _send_input_with_permission_resume_guard(
        ...
        """Release an exact in-flight reservation only when Core send fails."""

        answer = request.inputs.pop(_ROOT_PERMISSION_ANSWER_KEY, None)
        handoff = request.inputs.pop(_ROOT_PERMISSION_HANDOFF_KEY, None)
        if answer is not None and not isinstance(answer, RootPermissionAnswer):
            raise RootPermissionQueueError("permission_queue_answer_invalid")
        if not isinstance(handoff, _RootPermissionDispatchHandoff):
            raise RootPermissionQueueError("permission_dispatch_handoff_missing")
        ...
        try:
            ...
            if answer is not None:
                self._root_permission_queue.release_answer(answer)
            if handoff.lock.locked():
                handoff.lock.release()
            handoff.closed = True
            raise
    ...
    def _close_claimed_root_permission_handoff(
        self,
        ...
        answer = (
            handoff.answer
            if isinstance(handoff, _RootPermissionDispatchHandoff)
            else None
        )
    ...
    def _release_permission_resume_dispatch(self, inputs: Any) -> None:
        if not isinstance(inputs, MutableMapping):
            return
        answer = inputs.pop(_ROOT_PERMISSION_ANSWER_KEY, None)
        handoff = inputs.pop(_ROOT_PERMISSION_HANDOFF_KEY, None)
        if isinstance(answer, RootPermissionAnswer):
            self._root_permission_queue.release_answer(answer)
        if isinstance(handoff, _RootPermissionDispatchHandoff):
            if handoff.lock.locked():
    ...
    def _finalize_permission_handoff(self, inputs: Any) -> None:
        """Close this request's exact handoff after callback/cutover wins its CAS."""

        if not isinstance(inputs, MutableMapping):
            return
        handoff = inputs.pop(_ROOT_PERMISSION_HANDOFF_KEY, None)
        inputs.pop(_ROOT_PERMISSION_ANSWER_KEY, None)
        if not isinstance(handoff, _RootPermissionDispatchHandoff):
            return
        if not handoff.accepted:
            ...
            handoff.closed = True
            return
        if getattr(self, "_root_permission_handoff", None) is not handoff:
            return
    ...
    async def process_interrupt(self, request: AgentRequest) -> AgentResponse:
        ...
        cutover_handoff: _RootPermissionDispatchHandoff | None = None
        ...
        if intent == "pause":
            ...
            cutover_handoff = await self._start_root_permission_cutover(_normalized_sid)
            (
                cancelled_tool_results,
    ...
    async def _process_interaction_interrupt(
        ...
        cutover_handoff = await self._start_root_permission_cutover(root_session_id)
    ...
    async def process_message_impl(
        ...
        root_invocation_token = bind_root_permission_request(
            ...
            enabled=not is_team_params(request_params)
    ...
    async def _process_message_impl(
        ...
        try:
            ...
            while interaction_stream is not None:
                error_text = (
                    await self._consume_unary_output(
                        interaction_stream,
                        collected_content,
            ...
            if interaction_stream is not None:
                try:
                    ...
                    logger.debug(
                        "[Goal] interaction stream close failed", exc_info=True
                    )
            self._finalize_permission_handoff(inputs)
            self._unregister_session_agent_task(session_id)
                ...
                if chunk_type == "answer":
                    ...
                    if has_streamed_content:
                        parsed = self._parse_stream_chunk(
                            chunk, _has_streamed_content=True
                        )
                        parsed = self._adapt_goal_intermediate_final(parsed)
                        if parsed is not None:
                            if should_skip_duplicate_ask_user(parsed):
