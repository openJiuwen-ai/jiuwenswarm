        ...
        async with self.lock("project", project_id):
            ...
            try:
                ...
                for sid in ids:
                    ...
                    # busy 判定统一下沉到 _session_is_busy_for_action（不再按 is_cron 豁免），
                    # 这里仅做前置短路以减少对 running cron session 的无谓 stop 尝试。
                    if was_active and self.runtime.is_session_running(sid):
                        busy_ids.append(sid)
                        continue
                    try:
