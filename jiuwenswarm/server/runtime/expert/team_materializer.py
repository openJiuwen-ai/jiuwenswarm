...
def _write_leader(
    ...
    *,
    ...
    (leader_dir / "AGENT.md").write_text(
        "# 专家团调度规则\n\n"
        "## 不可绕过的成员执行门禁\n\n"
        "- 主理人只负责理解 Query、路由、监督和汇总，绝不能亲自完成任何成员专业工作。"
        "无论任务多简单，每个非澄清请求都必须先选择至少 1 位已注册成员并调用 `create_task`；"
        "所谓‘单成员直达’是指只调度 1 位专业成员，不是主理人直接回答。\n"
        "- 在至少一个本次任务完成前，主理人不得输出正文、方案、大纲、代码或成品文件。"
        "如果没有任务完成，只能继续调度、等待，或在确实缺少必要输入时向用户提一个澄清问题。\n"
        "- 最终回复必须基于已完成成员任务的真实结果；不得以‘任务很简单’、‘协调成本高’"
        "或‘可以直接回答’为由跳过成员。\n\n"
...
def _team_instruction(member_ids: list[str]) -> str:
    ...
    return (
        ...
        "主理人自身不是专业执行成员：每个非澄清 Query 都必须先调度至少 1 位已注册成员，"
        "在成员任务完成前不得直接输出专业正文或成品。‘单成员直达’只表示调度 1 位成员，"
        "绝不表示主理人亲自执行。\n\n"
