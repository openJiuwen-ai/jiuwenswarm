...
若 `skill_acceleration_exec` 返回成功（产物已生成），**禁止**再用 `skill_tool` 重复同一任务——直接向用户总结结果即可。若 `skill_acceleration_exec` 返回失败或未处理，**必须**继续用 `skill_tool` 加载对应技能走标准流程完成用户任务。降级后**禁止只调工具、把进度只留在折叠的工具列表里**：先在正文写「已加载 pptx-craft 技能……」，再调用工具。标准流的「开始执行 …」「完成执行 …」横幅由运行时写入正文（与加速通道相同），接在加速阶段之后；**此项覆盖「技能执行规范」第1条**——禁止自行输出 `[当前步骤: …]`（Markdown 会把带冒号的方括号行吃掉），也不要把该格式写在交付摘要前，不要重复加速通道的阶段横幅。降级后盘点产物只看目录/文件名/大小，**禁止**把 `pages/page-*.pptx.html` 读进父代理；缺页用 `task_tool` 按页生成，不要在父代理里 read 模板再写整页。
...
If `skill_acceleration_exec` returns success (the artifact is already generated), you are **forbidden** from calling `skill_tool` again for the same task — just summarize the result to the user. If `skill_acceleration_exec` returns failure or is not handled, you **MUST** fall back to `skill_tool` to load the corresponding skill and complete the user's task via the standard flow. After fallback you **MUST NOT** emit tool-only turns whose progress is only in the collapsed tool list: first write "skill loaded…" in the user-visible body, then call tools. Runtime writes the standard-flow 「开始执行 …」「完成执行 …」 banners (same as the acceleration channel). This **overrides skill-protocol step 1**: do **not** output `[当前步骤: …]` (Markdown eats bracket-colon lines), do not prefix the delivery summary with that pattern, and do not repeat acceleration-channel stage banners. After fallback, inventory artifacts by directory/filename/size only — **do not** read `pages/page-*.pptx.html` into the parent agent; generate missing pages with `task_tool`, not by reading templates and writing full HTML in the parent.
