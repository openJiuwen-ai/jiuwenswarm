...
def _is_windows_drive_start(text: str, i: int) -> bool:
    """判断位置 i 是否为 Windows 盘符路径起点（``D:\\`` / ``D:/``）。

    盘符前不得是字母数字（排除 URL 协议尾字母，如 https 的 s）；
    盘符后必须紧跟 ``:`` + ``\\``/``/``。
    """
    if i + 2 >= len(text) or not text[i].isalpha():
        return False
    if text[i + 1] != ":" or text[i + 2] not in "\\/":
        return False
    return i == 0 or not text[i - 1].isalnum()


def _is_unix_abs_start(text: str, i: int) -> bool:
    """判断位置 i 是否为 Unix 绝对路径起点。

    前置字符为字母数字或 ``/ . : ?`` 时视为 URL 协议/相对路径，跳过。
    """
    if text[i] != "/":
        return False
    if i == 0:
        return True
    prev = text[i - 1]
    return not (prev.isalnum() or prev in _PATH_PRECEDING_SKIP)
...
def extract_path_candidates(
    ...
    while i < n:
        start = -1
        if _is_windows_drive_start(text, i):
            # Windows 盘符路径：D:\ 或 D:/
            start = i
            i += 3
        elif _is_unix_abs_start(text, i):
