...
def build_user_prompt_if_a2ui_event(
    ...
    *,
    ...
    ``clock`` carries the ``timezone`` / ``timestamp`` pair the caller's own
    message envelope states, so a client event and an ordinary message from the
    same turn give the model one date resolved one way.
