...
async def install_agent_group_with_hub(
    ...
    if _resolve_agent_group_definition_dir(package_id) is None:
        def validate(package_root: Path, expected_name: str) -> None:
            _restore_published_agent_group(package_root, expected_name)
...
def _restore_published_agent_group(package_root: Path, expected_name: str) -> None:
    """Turn a Hub expert-team snapshot back into the local AgentGroup layout."""
    published = _read_package_manifest(package_root)
    if published is None or published.get("package_type") == "agent_group":
        return
    if published.get("package_type") != "agent_template" or published.get("name") != expected_name:
        raise ValueError("Hub agent_group package type or name mismatch")
    subagents = published.get("subagents")
    if not isinstance(subagents, list) or not subagents:
        raise ValueError("Hub agent_group package has no members")

    members = ["leader"]
    for entry in subagents:
        path = entry.get("dir") if isinstance(entry, dict) else None
        if not isinstance(path, str) or not path.startswith("agents/"):
            raise ValueError("Hub agent_group member path is invalid")
        name = _reject_package_name(path.removeprefix("agents/"), "agent_group member")
        if "/" in name or name == "leader" or name in members:
            raise ValueError("Hub agent_group member name is invalid")
        members.append(name)

    leader_dir = package_root / "agents" / "leader"
    if not leader_dir.is_dir() or not (leader_dir / "AGENT.md").is_file():
        raise ValueError("Hub agent_group leader is missing")

    def localize(value: str) -> str:
        prefix = "agents/leader"
        if value == prefix:
            return "."
        if value.startswith(prefix + "/"):
            return value[len(prefix) + 1:]
        raise ValueError("Hub agent_group leader reference is outside its directory")

    leader = dict(published)
    leader.pop("subagents", None)
    leader["name"] = "leader"
    for field, key in (("persona", "dir"), ("model", "file")):
        item = leader.get(field)
        if isinstance(item, dict) and isinstance(item.get(key), str):
            leader[field] = {**item, key: localize(item[key])}

    shared_skills: list[str] = []
    for field in ("skills", "tools", "rails", "mcps", "memories", "rubrics"):
        entries = leader.get(field)
        ...
        for entry in entries:
            if not isinstance(entry, dict):
                ...
                continue
            item = dict(entry)
            if field == "skills" and isinstance(item.get("dir"), str) and item["dir"].startswith("skills/"):
                name = _reject_package_name(item["dir"].removeprefix("skills/"), "agent_group skill")
                if "/" in name or name in shared_skills:
                    raise ValueError("Hub agent_group shared skill is invalid")
                shared_skills.append(name)
                continue
            for key in ("dir", "file"):
                if isinstance(item.get(key), str):
                    item[key] = localize(item[key])
            restored.append(item)
        leader[field] = restored

    sections = leader.get("prompt_sections")
    instruction = ""
    if isinstance(sections, list):
        retained = []
        for section in sections:
            if isinstance(section, dict) and section.get("name") == "agent_group_instruction":
                content = section.get("content")
                if isinstance(content, dict):
                    instruction = str(content.get("cn") or content.get("en") or "")
            else:
                retained.append(section)
        leader["prompt_sections"] = retained

    for name in members[1:]:
        directory = package_root / "agents" / name
        subagent_path = directory / ".subagent.json"
        member = _read_package_manifest(directory) if (directory / "manifest.json").exists() else None
        if member is not None or not subagent_path.is_file():
            raise ValueError("Hub agent_group member manifest is invalid")
        member = json.loads(subagent_path.read_text(encoding="utf-8"))
        if not isinstance(member, dict) or member.get("agent_name") != name:
            raise ValueError("Hub agent_group member identity mismatch")
        member.pop("agent_name", None)
        member["package_type"] = "agent_template"
        member["name"] = str(member.get("display_name") or name)
        (directory / "manifest.json").write_text(json.dumps(member, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
        subagent_path.unlink()

    (leader_dir / "manifest.json").write_text(json.dumps(leader, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
    group = {
        "package_type": "agent_group", "name": expected_name,
        "description": published.get("description", ""), "agents": members,
        "skills": shared_skills, "instruction": instruction,
    }
    (package_root / "manifest.json").write_text(json.dumps(group, ensure_ascii=False, indent=2) + "\n", encoding="utf-8")
