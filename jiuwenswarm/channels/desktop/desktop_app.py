...
if(bridgeNeverResponded||bridgeStoppedResponding){
    ...
    def _on_closing(self) -> bool:
        ...
        # etc.) -- the quit flag is how we tell those two apart. No tray means no
        # way to reopen a hidden window, so degrade to the old close-quits behavior.
        if self._quit_requested.is_set() or self._tray is None:
            return True
        self.hide_window()
        return False
