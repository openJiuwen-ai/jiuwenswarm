...
def _hub_base() -> str:
    ...
    if parsed.scheme == "http" and parsed.hostname == "127.0.0.1" and parsed.port and not parsed.path:
        return base
    ...
    if error:
        ...
        if not 8 <= len(session) <= 256:
            ...
            try:
                ...
                open_request = build_opener(ProxyHandler({})).open if hub_base == "http://119.8.233.112:8080" else urlopen
                ...
                outcome = {"status": "complete", "provider": provider, "access_token": data["access_token"], "user": data.get("user")}
...
def result(flow: str, claim: str) -> tuple[int, dict]:
    with _lock:
        ...
        del _attempts[flow]
        return 200, {key: attempt[key] for key in ("status", "provider", "access_token", "user", "error") if key in attempt}
