...
async function mounted(onSubmit, run) {
...
test('routes Symphony experience candidates to the dedicated package prompt', () => {
...
test('renders direct package actions without generic question controls', async () => {
  const submissions = [];
  await mounted(
    async (...args) => {
...
test('submits install directly and shows a beginner-friendly busy state', async () => {
  let finish;
