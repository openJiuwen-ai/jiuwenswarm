 ...
 * （organization_id / display_name / owner_team_id / owner_leader_id / leaders /
 * metadata / unclaimed_task_policy），tasks 是 OrgTask.brief() 的列表。
 *
...
interface OrgLeaderHandle {
  organization_id?: string | null;
  team_id?: string | null;
  leader_id?: string | null;
  leader_member_name?: string | null;
  capabilities?: string[];
}
...
interface OrgSpec {
  ...
  owner_leader_id?: string | null;
  leaders?: OrgLeaderHandle[];
  metadata?: Record<string, unknown>;
  unclaimed_task_policy?: Record<string, unknown> | null;
}
  ...
  const load = useCallback(
    async (sessionId: string, teamId: string | null) => {
      ...
      try {
        ...
        // 失败时保留上一次的数据：organization 未绑定时后端返回的是空快照，
        // 把面板清空反而看不出是"还没绑"还是"请求炸了"。
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-3" data-testid="org-info-body">
        ...
        {organization && (
          <section className="mb-4 space-y-1.5" data-testid="org-info-organization">
            ...
            <InfoRow label={t('team.org.ownerLeader')} value={organization.owner_leader_id} />
            <InfoRow
              label={t('team.org.unclaimedPolicy')}
              value={
                organization.unclaimed_task_policy
                  ? Object.entries(organization.unclaimed_task_policy)
            ...
            {(organization.leaders?.length ?? 0) > 0 && (
              <div className="pt-2" data-testid="org-info-leaders">
                ...
                <div className="space-y-1">
                  {organization.leaders!.map((leader, index) => (
                    <div
                      key={`${leader.team_id ?? index}-${leader.leader_id ?? index}`}
                      className="rounded border border-border bg-secondary/40 px-2 py-1.5"
                      data-testid="org-info-leader-row"
                    >
                      <div className="flex items-baseline justify-between gap-2 text-xs">
                        <span className="truncate font-medium text-text" title={leader.team_id ?? ''}>
                          {leader.team_id || '-'}
                        </span>
                        <span
                          className="truncate text-text-muted"
                          title={leader.leader_member_name || leader.leader_id || ''}
                        >
                          {leader.leader_member_name || leader.leader_id || '-'}
                        </span>
                      </div>
                      {(leader.capabilities?.length ?? 0) > 0 && (
              ...
              {tasks.map((task) => (
                ...
                >
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-x-2 gap-y-0.5 text-[10px] text-text-muted">
                    ...
                    {task.unclaimed !== undefined && task.unclaimed !== null && (
                      <span className="text-amber-600">{t('team.org.taskUnclaimed')}</span>
                    )}
