...
import { CircleCheck, CircleX, CircleDot, Circle, Square, Smile, Loader2, Pause, RefreshCw } from 'lucide-react';
import {
  type WorkflowRun,
  type WorkflowPhase,
  type WorkflowAgent,
...
import { AgentDetailModal, buildDetailSections, type AgentModalState, type DetailSectionKey } from './AgentDetailModal';

// ── 状态图标 ──────────────────────────────────────────────

function StatusIcon({ status, className = 'w-4 h-4' }: { status: WorkflowStatus; className?: string }) {
  switch (status) {
    case 'completed':
      return <CircleCheck className={`${className} text-ok`} />;
    case 'running':
      return <Loader2 className={`${className} text-accent animate-spin`} />;
    case 'paused':
      return <Pause className={`${className} text-accent`} />;
    case 'failed':
      return <CircleX className={`${className} text-danger`} />;
    case 'waiting_for_human':
      return <Smile className={`${className} text-warn animate-pulse`} />;
    case 'pending':
      return <CircleDot className={`${className} text-muted`} />;
    case 'planned':
      return <Circle className={`${className} text-muted`} />;
    case 'stopped':
      return <Square className={`${className} text-muted`} />;
    default:
      return <Circle className={`${className} text-muted`} />;
  }
}
...
const RunGraphNode = ({ data }: NodeProps) => {
  ...
  const completed =
    ...
    (run.phases ?? []).reduce(
      (s, p) => s + (p.agents ?? []).filter(a => a.status === 'completed' || a.status === 'failed' || a.status === 'stopped').length,
      0,
    );
  const total = run.agent_count ?? (run.phases ?? []).reduce((s, p) => s + (p.agents ?? []).length, 0);
  ...
  return (
    <div
      className={`px-3 py-2 rounded-lg border-2 bg-card shadow-md min-w-[140px] ${statusBorder(run.status)}`}
      data-testid="team-area-swarmflow-graph-node"
      data-variant="runNode"
    >
      ...
      <div className="flex items-center gap-1.5 mt-1">
        <div className="flex-1 h-1 rounded-full bg-secondary overflow-hidden">
          <div className="h-full bg-accent rounded-full" style={{ width: `${total > 0 ? (completed / total) * 100 : 0}%` }} />
        </div>
      ...
      {showBudget && (
        <div className="flex flex-wrap items-center gap-1 mt-0.5 text-[10px] tabular-nums">
          {run.budget && run.budget.total != null && (
            <span
              className={`px-1.5 rounded ${run.budget.exhausted ? 'bg-danger/10 text-danger' : 'bg-accent/10 text-accent'}`}
          ...
          {run.workflow_budget && (
            <span
              className={`px-1.5 rounded ${run.workflow_budget.exhausted ? 'bg-danger/10 text-danger' : 'bg-ok/10 text-ok'}`}
...
const PhaseGraphNode = ({ data }: NodeProps) => {
  ...
  return (
    <div
      className={`px-2.5 py-1.5 rounded-md border bg-card shadow-sm min-w-[110px] ${statusBorder(phase.status)}`}
      data-testid="team-area-swarmflow-graph-node"
      data-variant="phaseNode"
    >
...
const AgentGraphNode = ({ data }: NodeProps) => {
  ...
  return (
    <div
      className={`px-2.5 py-1 rounded-md border bg-card shadow-sm min-w-[90px] ${statusBorder(agent.status)} ${interactive ? 'cursor-pointer' : ''} ${agent.status === 'waiting_for_human' ? 'ring-1 ring-warn/40' : ''}`}
      title={interactive ? '点击查看详情' : undefined}
      data-testid="team-area-swarmflow-graph-node"
      data-variant="agentNode"
    >
      <Handle type="target" position={Position.Left} className="!bg-border" />
      {agent.node_type !== 'human' && agent.node_type !== 'human_session' && <Handle type="source" position={Position.Right} className="!bg-border" />}
...
const LoopGraphNode = ({ data }: NodeProps) => {
  ...
  return (
    <div
      className="px-2.5 py-1.5 rounded-md border-2 border-accent/40 bg-accent/5 shadow-sm min-w-[100px]"
      data-testid="team-area-swarmflow-graph-node"
      data-variant="loopNode"
    >
      <Handle type="target" position={Position.Left} className="!bg-border" />
      <Handle type="source" position={Position.Right} className="!bg-border" />
      <div className="flex items-start gap-1">
        <RefreshCw className={`w-3 h-3 text-accent shrink-0 mt-0.5 ${loopStatus === 'running' ? 'animate-spin' : ''}`} />
      ...
      <div className="flex items-center gap-0.5 mt-0.5">
        {loop.members.map((m, i) => (
          <div key={i} className={`w-1.5 h-1.5 rounded-full ${statusDotColor(m.status)} ${i === activeIdx ? 'ring-1 ring-accent' : ''}`} />
        ))}
      </div>
    </div>
  );
...
function workflowToGraph(runs: WorkflowRun[], sessionId: string): { nodes: Node[]; edges: Edge[] } {
  const nodes: Node[] = [];
  const edges: Edge[] = [];
  ...
  for (const run of runs) {
    ...
    const topLevel = sortPhasesByExecution(run.phases ?? []).filter(p => p.phase_type !== 'child');
...
function addPhaseChildren(nodes: Node[], edges: Edge[], phase: WorkflowPhase, phaseId: string, run: WorkflowRun, sessionId: string) {
  ...
  for (const agent of phase.agents ?? []) {
    ...
    if (session) {
      ...
      const nodeAgent = status === representative.status ? representative : { ...representative, status };
...
export function SwarmflowGraphView({ runs, sessionId }: { runs: WorkflowRun[]; sessionId: string }) {
  const { nodes: rawNodes, edges: rawEdges } = useMemo(() => workflowToGraph(runs, sessionId), [runs, sessionId]);
  ...
  return (
    <div className="w-full h-full min-h-[400px] relative" data-testid="team-area-swarmflow-graph-view">
      ...
      <AgentDetailModal
        ...
        onTabChange={(key: DetailSectionKey) => setModalState(prev => (prev ? { ...prev, activeKey: key } : prev))}
      />
    </div>
  );
}
