...
export const accentChipClass: Record<DetailAccent, string> = {
  blue: 'border-accent/40 bg-accent/10 text-accent hover:bg-accent/20',
  amber: 'border-warn/40 bg-warn/10 text-warn hover:bg-warn/20',
  emerald: 'border-ok/40 bg-ok/10 text-ok hover:bg-ok/20',
  red: 'border-danger/40 bg-danger/10 text-danger hover:bg-danger/20',
};

export const accentTabActiveClass: Record<DetailAccent, string> = {
  blue: 'border-accent text-accent bg-accent/10',
  amber: 'border-warn text-warn bg-warn/10',
  emerald: 'border-ok text-ok bg-ok/10',
...
function JsonNode({ name, value, depth }: { name?: string; value: unknown; depth: number }) {
  ...
  return (
    <div className="py-0.5">
      <button type="button" onClick={() => setCollapsed(!collapsed)} className="inline-flex items-center gap-0.5 hover:bg-secondary/40 rounded px-0.5">
...
export function AgentDetailModal({ state, agentName, onClose, onTabChange }: AgentDetailModalProps) {
  ...
  return (
    <div className="fixed inset-0 z-[2200] flex items-center justify-center p-4" role="dialog" aria-modal="true" data-testid="team-area-swarmflow-detail-modal">
      <button
        type="button"
      ...
      <div className="relative w-full max-w-4xl max-h-[85vh] overflow-hidden rounded-xl border border-border bg-card shadow-2xl animate-rise flex flex-col">
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-3 border-b border-border bg-panel shrink-0">
          <div className="min-w-0 flex-1">
            <h3 className="text-base font-semibold text-text truncate" data-testid="team-area-swarmflow-detail-modal-title">
              {agentName}
            </h3>
            <p className="text-xs text-text-muted mt-0.5" data-testid="team-area-swarmflow-detail-modal-section-label" data-variant={activeSection?.key}>
              {activeSection?.label}
            </p>
          </div>
          <button
        ...
        {state.sections.length > 1 && (
          <div
            className="flex items-center gap-1 px-3 py-2 border-b border-border/60 bg-secondary/20 shrink-0 overflow-x-auto"
            data-testid="team-area-swarmflow-detail-modal-tabs"
          >
              ...
              return (
                <button
                  ...
                  className={`shrink-0 inline-flex items-center gap-1 rounded-md border px-2.5 py-1 text-xs font-medium transition-colors ${
                    active ? accentTabActiveClass[sec.accent] : 'border-transparent text-text-muted hover:text-text hover:bg-secondary/60'
                  }`}
                >
                  <span>{sec.icon}</span>
                  <span>{sec.label}</span>
        ...
        <div className="p-5 overflow-auto flex-1" data-testid="team-area-swarmflow-detail-modal-content">
            ...
            <div className="flex justify-end gap-1 mb-2">
              <button
                ...
                className={`px-2 py-0.5 rounded text-[11px] border transition-colors ${
                  !rawMode ? 'border-accent/50 bg-accent/10 text-accent' : 'border-border text-text-muted hover:text-text'
                }`}
              >
                格式化
              </button>
          ...
          {rawMode ? (
            ...
            <MarkdownRenderer content={content} className="text-sm text-text chat-text max-w-none" />
          )}
        </div>
      </div>
    </div>
