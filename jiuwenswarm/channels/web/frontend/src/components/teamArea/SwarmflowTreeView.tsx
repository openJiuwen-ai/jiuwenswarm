...
import { ChevronDown, ChevronRight, CircleCheck, CircleX, CircleDot, Circle, Square, Smile, Loader2, Pause, Play, RefreshCw } from 'lucide-react';
...
import { AgentDetailModal, buildDetailSections, formatCharCount, accentChipClass, type AgentModalState } from './AgentDetailModal';
...
function StatusIcon({ status, className }: { status: WorkflowStatus; className?: string }) {
  ...
  switch (status) {
    case 'completed':
      return <CircleCheck className={`${cls} text-ok`} />;
    case 'failed':
      return <CircleX className={`${cls} text-danger`} />;
    case 'running':
      return <Loader2 className={`${cls} text-accent animate-spin`} />;
    case 'paused':
      return <Pause className={`${cls} text-accent`} />;
    case 'pending':
    case 'planned':
      return <CircleDot className={`${cls} text-muted`} />;
    case 'stopped':
      return <Square className={`${cls} text-muted`} />;
    case 'waiting_for_human':
      return <Smile className={`${cls} text-warn`} />;
    default:
      return <Circle className={`${cls} text-muted`} />;
  }
}
      ...
      {members.map((member, i) => (
        <button
          ...
          className={`w-2.5 h-2.5 rounded-full transition-all hover:scale-125 ${statusDotColor(member.status)} ${
            i === selectedIndex ? 'ring-2 ring-accent ring-offset-1 ring-offset-card' : ''
          }`}
...
function AgentLoopNode({
  ...
  const completedCount = members.filter(m => m.status === 'completed' || m.status === 'failed' || m.status === 'stopped').length;
  ...
  return (
    <div>
      ...
      >
        ...
        <RefreshCw className={`w-4 h-4 shrink-0 text-accent ${loopStatus === 'running' ? 'animate-spin' : ''}`} />
        ...
        <IterationStrip members={members} selectedIndex={selectedIdx} onSelect={setSelectedIdx} />
      </div>
      ...
      {expanded && (
        <div>
          {members.map((member, i) =>
            i === selectedIdx ? (
              <AgentNode key={member.id} agent={member} phaseAgents={members} depth={depth + 1} runId={runId} sessionId={sessionId} />
            ) : (
              <div
                key={member.id}
              ...
              >
                ...
                <span className="text-xs text-text-muted">第{i + 1}轮</span>
                {member.outcome && <span className="text-xs text-text-muted/70 truncate flex-1">{member.outcome}</span>}
              </div>
            ),
          )}
        </div>
    ...
    <div>
      ...
      {expanded && (
        <div>
          {members.map((phase, i) =>
            i === selectedIdx ? (
              <PhaseNode key={phase.id} phase={phase} workflow={workflow} depth={depth + 1} runId={runId} sessionId={sessionId} />
            ) : (
              <div
                key={phase.id}
                className="flex items-center gap-2 px-2 py-1 rounded-md hover:bg-secondary/30 transition-colors cursor-pointer"
              ...
              >
                ...
                <span className="text-xs text-text-muted">第{phase.iteration ?? i + 1}轮</span>
                <span className="text-xs text-text-muted/70 shrink-0">
                  {phase.completed_agent_count ??
                    (phase.agents ?? []).filter(a => a.status === 'completed' || a.status === 'failed' || a.status === 'stopped').length}
...
function AgentNode({
  ...
  const sessionMembers = useMemo(
    () => (isSession ? (groupWorkflowAgentsByName(phaseAgents).sessions.find(s => s.label === agent.name)?.members ?? []) : []),
  ...
  const handleAgentClick = useCallback(() => {
    ...
    void (() => {
      const question = agent.human_prompt?.trim();
  ...
  return (
    <div>
      ...
      >
        ...
        <span className="text-sm text-text break-words flex-1" data-testid="team-area-swarmflow-agent-name" data-variant={agent.id}>
          {agent.name}
        </span>
        {agent.model && <span className="text-xs text-text-muted shrink-0">{agent.model}</span>}
        {/* 展开 input/output 详情：完整体已就绪（有 prompt/outcome/error）或仅为摘要待拉取时均显示 */}
        {(agent.prompt || agent.outcome || agent.error || agent.detail_pending) && (
          <button
            type="button"
            className="shrink-0 p-0.5 rounded hover:bg-secondary"
          ...
          >
            {showDetail ? <ChevronDown className="w-3 h-3 text-text-muted" /> : <ChevronRight className="w-3 h-3 text-text-muted" />}
          </button>
        )}
        {agent.status === 'waiting_for_human' && (
          <span className="text-xs text-warn shrink-0 animate-pulse" data-testid="team-area-swarmflow-agent-waiting-hint">
        ...
        >
          ...
          {detailSections.map(sec => (
            ...
            >
              ...
              <span className="ml-0.5 px-1 rounded bg-black/10 dark:bg-white/10 text-[9px] tabular-nums opacity-80">{formatCharCount(sec.content)}</span>
            </button>
          ))}
        </div>
      )}
      ...
      {(agent.error || agent.error_preview) && !showDetail && (
        <div
          className="flex items-start gap-1.5 px-2 pb-1 text-xs text-danger/80"
          style={{ paddingLeft: `${depth * 20 + 28}px` }}
          data-testid="team-area-swarmflow-agent-error-preview"
        >
          <span className="shrink-0 text-danger/40">└</span>
            ...
            return (
              <div key={member.id} className="relative pl-4">
                ...
                <div
                  ...
                  data-testid="team-area-swarmflow-agent-turn"
                  data-variant={member.id}
                >
                  <StatusIcon status={member.status} className="w-3.5 h-3.5" />
                  <span className="text-xs text-text-muted">Turn {turn ?? '?'}</span>
                  {member.outcome && <span className="text-xs text-text-muted/60 truncate flex-1">{member.outcome}</span>}
                </div>
              </div>
            );
          })}
...
function PhaseNode({
  ...
  const { t } = useTranslation();
  const [expanded, setExpanded] = useState(true);
  const { sessions, oneShots } = useMemo(() => groupWorkflowAgentsByName(phase.agents ?? []), [phase.agents]);
  // 检测同名 agent 循环（场景 A）
  const { loops: agentLoops, unique: uniqueAgents } = useMemo(() => detectAgentLoops(oneShots), [oneShots]);
  const childPhases = useMemo(() => childPhasesOf(workflow, phase), [workflow, phase]);
  ...
  const completedCount =
    phase.completed_agent_count ?? (phase.agents ?? []).filter(a => a.status === 'completed' || a.status === 'failed' || a.status === 'stopped').length;
  const totalCount = phase.agent_count ?? phase.agents?.length ?? 0;
  const hasChildren = sessions.length > 0 || uniqueAgents.length > 0 || agentLoops.length > 0 || childPhases.length > 0;
    ...
    <div>
      ...
      >
        ...
        {phase.phase_type === 'child' && (
          <span className="text-xs px-1.5 py-0.5 rounded bg-accent/10 text-accent shrink-0" data-testid="team-area-swarmflow-phase-sub-workflow-badge">
      ...
      {expanded && hasChildren && (
        <div className="border-l border-border/30 ml-4" style={{ marginLeft: `${depth * 20 + 16}px` }}>
          {/* 唯一 agent（非循环） */}
          {uniqueAgents.map(agent => (
            <div key={agent.id} className="relative pl-4">
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-3 border-t border-border/30" />
              <AgentNode agent={agent} phaseAgents={phase.agents ?? []} depth={0} runId={runId} sessionId={sessionId} />
            </div>
          ))}
          {/* 同名 agent 循环（场景 A） */}
          {agentLoops.map(loop => (
            <div key={`loop-${loop.name}`} className="relative pl-4">
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-3 border-t border-border/30" />
              <AgentLoopNode name={loop.name} members={loop.members} depth={0} runId={runId} sessionId={sessionId} />
            </div>
          ))}
          ...
          {sessions.map(session => {
            ...
            const node = status === representative.status ? representative : { ...representative, status };
            return (
              <div key={representative.id} className="relative pl-4">
                <div className="absolute left-0 top-1/2 -translate-y-1/2 w-3 border-t border-border/30" />
                <AgentNode agent={node} phaseAgents={phase.agents ?? []} depth={0} runId={runId} sessionId={sessionId} />
              </div>
            );
          })}
          ...
          {childPhases.map(child => (
            <div key={child.id} className="relative pl-4">
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-3 border-t border-border/30" />
              <PhaseNode phase={child} workflow={workflow} depth={0} runId={runId} sessionId={sessionId} />
            </div>
          ))}
        </div>
      )}
...
function RunNode({ run, sessionId }: { run: WorkflowRun; sessionId: string }) {
  const { t } = useTranslation();
  const [expanded, setExpanded] = useState(true);
  ...
  const [pendingControl, setPendingControl] = useState<'pause' | 'resume' | 'stop' | null>(null);
  useEffect(() => {
    ...
    if (pendingControl === 'pause' && run.status !== 'running') {
      ...
      setPendingControl(null);
    } else if (pendingControl === 'stop' && (run.status === 'stopped' || run.status === 'completed' || run.status === 'failed')) {
      setPendingControl(null);
    }
  ...
  const applyControlFailure = useCallback(
    (err: unknown): boolean => {
      ...
      const payload = webErr?.payload as { status?: unknown } | undefined;
      ...
      setControlError(
        status ? t(CONTROL_STATUS_KEY[status] || 'swarmflow.controlNotFound') : transport && webErr?.message ? webErr.message : t('swarmflow.controlNotFound'),
      );
      return !transport;
    },
  ...
  const completedCount =
    ...
    (run.phases ?? []).reduce(
      (sum, p) => sum + (p.agents ?? []).filter(a => a.status === 'completed' || a.status === 'failed' || a.status === 'stopped').length,
      0,
    );
  const totalCount = run.agent_count ?? (run.phases ?? []).reduce((sum, p) => sum + (p.agents ?? []).length, 0);
  ...
  const budgetExhaustedScope =
    run.budget_exhausted_scope ?? (run.workflow_budget?.exhausted ? 'workflow' : run.budget?.total != null && run.budget?.exhausted ? 'session' : null);
  ...
  const controlBtnClass = `flex items-center justify-center w-7 h-7 rounded text-text-muted hover:bg-secondary transition-colors disabled:hover:bg-transparent disabled:hover:text-text-muted ${
    ...
    <div className="border border-border rounded-lg overflow-hidden bg-card/50" data-testid="team-area-swarmflow-run" data-variant={run.id}>
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          {run.summary && (
            <p className="text-xs text-text-muted truncate mt-0.5" data-testid="team-area-swarmflow-run-summary" data-variant={run.id}>
              {run.summary}
            </p>
          )}
        </div>
        {waitingCount > 0 && (
          <span
            className="text-xs px-2 py-0.5 rounded-full bg-warn/10 text-warn shrink-0 animate-pulse"
            data-testid="team-area-swarmflow-run-waiting-badge"
            data-variant={run.id}
          >
        ...
        {run.budget && (
          <span
            className={`text-xs shrink-0 tabular-nums px-1.5 py-0.5 rounded-full ${
              run.budget.exhausted ? 'bg-danger/10 text-danger' : 'bg-accent/10 text-accent'
            }`}
        ...
        {run.workflow_budget && (
          <span
            className={`text-xs shrink-0 tabular-nums px-1.5 py-0.5 rounded-full ${
              run.workflow_budget.exhausted ? 'bg-danger/10 text-danger' : 'bg-ok/10 text-ok'
            }`}
        ...
        {budgetExhaustedScope && (
          <span className="text-xs px-2 py-0.5 rounded-full bg-danger/10 text-danger shrink-0">
            {t(budgetExhaustedScope === 'session' ? 'swarmflow.budgetExhaustedSession' : 'swarmflow.budgetExhaustedWorkflow')}
          </span>
        )}
        {(run.status === 'running' || run.status === 'paused') && (
          <div className="flex items-center gap-1 shrink-0" onClick={e => e.stopPropagation()}>
            <button
              ...
              className={`${controlBtnClass} hover:text-warn`}
            ...
            <button
              ...
              className={`${controlBtnClass} hover:text-danger`}
              disabled={controlsDisabled || pendingControl !== null}
              data-testid="team-area-swarmflow-run-stop-btn"
              onClick={() => {
                setPendingControl('stop');
                void webRequest('swarmflow.stop', { session_id: sessionId, run_id: run.id }).catch(err => {
                  if (applyControlFailure(err)) {
                    setPendingControl(null);
                  }
                });
            ...
            >
              {pendingControl === 'stop' ? <Loader2 className="w-3.5 h-3.5 animate-spin text-danger" /> : <Square className="w-3.5 h-3.5" />}
            </button>
          </div>
        )}
      </div>

      {controlError && <Toast message={controlError} onClose={() => setControlError(null)} variant="error" />}
      {run.error && (
        <button
          type="button"
          onClick={() =>
            setRunDetail({
              sections: [{ key: 'error', label: '错误', icon: '✕', content: run.error!, accent: 'red' }],
              activeKey: 'error',
            })
          }
          className="flex w-full items-start gap-1.5 px-3 py-1.5 text-left text-xs text-danger/90 hover:bg-danger/5"
          data-testid="team-area-swarmflow-run-error"
        >
          <CircleX className="w-3.5 h-3.5 shrink-0 mt-0.5 text-danger/70" />
      ...
      {run.result && (
        <button
          ...
          onClick={() =>
            setRunDetail({
              sections: [{ key: 'result', label: '结果', icon: '✓', content: run.result!, accent: 'emerald' }],
              activeKey: 'result',
            })
          }
          className="flex w-full items-start gap-1.5 px-3 py-1.5 text-left text-xs text-text-muted hover:bg-ok/5"
          data-testid="team-area-swarmflow-run-result"
        >
          <CircleCheck className="w-3.5 h-3.5 shrink-0 mt-0.5 text-ok/70" />
        ...
        <div className="py-1 border-l border-border/30 ml-4">
          ...
          {uniquePhases.map(phase => (
            <div key={phase.id} className="relative pl-4">
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-3 border-t border-border/30" />
              <PhaseNode phase={phase} workflow={run} depth={0} runId={run.id} sessionId={sessionId} />
            </div>
          ))}
          ...
          {phaseLoops.map(loop => (
            <div key={`loop-${loop.baseName}`} className="relative pl-4">
              <div className="absolute left-0 top-1/2 -translate-y-1/2 w-3 border-t border-border/30" />
              <PhaseLoopNode baseName={loop.baseName} members={loop.members} workflow={run} depth={0} runId={run.id} sessionId={sessionId} />
            </div>
          ))}
        </div>
      )}
