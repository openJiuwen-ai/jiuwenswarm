...
export type WorkflowStatus = 'planned' | 'pending' | 'running' | 'paused' | 'completed' | 'failed' | 'stopped' | 'waiting_for_human';
...
const SPLITTABLE_AGENT_FIELDS = ['prompt', 'outcome', 'human_prompt', 'human_reply', 'activity', 'error'] as const;
...
export function reassembleAgentFieldParts(agent: WorkflowAgent): WorkflowAgent {
  ...
  for (const field of SPLITTABLE_AGENT_FIELDS) {
    ...
    const sorted = [...parts].sort((a, b) => (a as WorkflowAgentPart).part_idx - (b as WorkflowAgentPart).part_idx);
    const joined = sorted.map(p => (p as WorkflowAgentPart).content ?? '').join('');
...
export function sessionMembersInPhase(phaseAgents: WorkflowAgent[], sessionLabel: string, nodeType?: WorkflowNodeType): WorkflowAgent[] {
  const members = phaseAgents.filter(agent => agent.name === sessionLabel && isSessionNode(agent) && (nodeType === undefined || agent.node_type === nodeType));
...
export function phaseLocalTurnNumber(agent: WorkflowAgent, phaseAgents: WorkflowAgent[]): number | null {
...
export function shouldShowSessionTree(agent: WorkflowAgent, phaseAgents: WorkflowAgent[]): boolean {
...
export function childPhasesOf(workflow: WorkflowRun, parent: WorkflowPhase): WorkflowPhase[] {
  return (workflow.phases ?? []).filter(phase => phase.phase_type === 'child' && phase.parent_phase === parent.name);
}
...
export function mergeWorkflowRun(existing: WorkflowRun | undefined, incoming: WorkflowRun): WorkflowRun {
  ...
  const incomingLogs = Array.isArray(incoming.logs) ? incoming.logs.filter((log): log is string => typeof log === 'string') : undefined;
  ...
  if (incomingLogs) {
    ...
    merged.logs = existing && !incomingHasPhases ? [...(existing.logs ?? []), ...incomingLogs] : incomingLogs;
...
export function normalizeWorkflowRun(workflow: WorkflowRun): WorkflowRun {
  return {
    ...workflow,
    logs: Array.isArray(workflow.logs) ? workflow.logs.filter((log): log is string => typeof log === 'string') : undefined,
            ...
            ? phase.agents.map(agent =>
                reassembleAgentFieldParts({
                  ...
                  activity: Array.isArray(agent.activity)
                    ? agent.activity.filter((activity): activity is WorkflowAgentActivity =>
                        Boolean(activity && typeof activity === 'object' && !Array.isArray(activity)),
                      )
                    : undefined,
                }),
              )
...
export function applyWorkflowUpdate(workflows: WorkflowRun[], incoming: WorkflowRun): WorkflowRun[] {
  const index = workflows.findIndex(workflow => workflow.id === incoming.id);
  if (index === -1) {
    return [normalizeWorkflowRun(incoming), ...workflows];
  }
  return workflows.map((workflow, itemIndex) => (itemIndex === index ? normalizeWorkflowRun(mergeWorkflowRun(workflow, incoming)) : workflow));
}
...
export function sortPhasesByExecution(phases: WorkflowPhase[]): WorkflowPhase[] {
  ...
  for (const child of children) {
    const parentIdx = result.findIndex(p => p.name === child.parent_phase || p.id === child.parent_phase);
    if (parentIdx >= 0) {
 ...
 */
export function computeSessionStatus<T extends { status: WorkflowStatus }>(members: T[]): WorkflowStatus {
 ...
 */
export function computeLoopStatus<T extends { status: WorkflowStatus }>(members: T[]): WorkflowStatus {
  if (members.length === 0) return 'planned';
 ...
 */
export function findActiveIterationIndex<T extends { status: WorkflowStatus }>(members: T[]): number {
