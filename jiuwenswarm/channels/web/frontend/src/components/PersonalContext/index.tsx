...
export function PersonalContextPanel({ isConnected, isActive }: PersonalContextPanelProps) {
  ...
  // infoTab === 'services' / 'settings' 优先：用户点击"去完成"/"去授权"后
  // 直接进入内容采集页/设置页，不被 showIntro 拦截
  if (infoTab === 'services') {
    return (
  ...
  // dev-stable 无 develop 的 settings registry 体系：设置页（开关/模型/授权）
  // 作为 infoTab 的第三个视图在本面板内呈现，授权入口经 'jiuwen:nav' 事件
  // 跳回主区并落到此视图。
  if (infoTab === 'settings') {
    return (
      <div className="pc-panel" data-testid="personal-context-panel">
