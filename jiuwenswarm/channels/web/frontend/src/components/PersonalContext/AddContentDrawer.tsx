...
export function AddContentDrawer({ initialProvider, editService, onClose, onCreated }: AddContentDrawerProps) {
  ...
  const { config, createService, updateService, pendingWrites, isProviderAuthorized, setInfoTab } = usePersonalContextStore();
                ...
                {PROVIDER_ORDER.map((p) => {
                  ...
                  return (
                    <button
                      ...
                      onClick={() => {
                        if (disabled) {
                          // 未授权（GitHub/GitCode）→ 跳设置视图授权（dev-stable：切 infoTab，无 settings registry）
                          setInfoTab('settings');
                          onClose();
                          return;
                        }
            ...
            {!isEdit && provider === 'feishu' && !authorized && (
              <div className="pc-drawer__field-hint">
                ...
                <button
                  ...
                  onClick={() => { setInfoTab('settings'); onClose(); }}
                >
