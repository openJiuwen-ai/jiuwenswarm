...
import { useChatStore, useGoalStore, usePlanStore, useSessionStore, useWorkspaceStore } from '../../stores';
...
export const InputArea = forwardRef<InputAreaHandle, InputAreaProps>(function InputArea(
  ...
  const hasPendingQuestion = useChatStore((s) => Boolean(s.runtimes[activeSessionId ?? '']?.pendingQuestion));
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      .then(([commandData, skillData]) => {
        const installedNames = new Set((skillData.plugins ?? []).flatMap((plugin) => plugin.skills ?? []));
        const availableSkills = (skillData.skills ?? []).filter(
          (skill) =>
            Boolean(skill.name) &&
            skill.enabled !== false &&
            Boolean(
  ...
  const imageInputDisabled = isListening || isTranscribing || composerDisabled || (isInterruptible && !isTeamMode);
  ...
  const executeSlashCommand = useCallback(async (command: SlashCommand, context: SlashCommandContext, args: string) => {
  ...
  const handleSubmit = useCallback(() => {
    if (composerDisabled || isListening || isTranscribing) return;
        ...
        if (e.key === 'ArrowUp') {
          ...
          if (composerSuggestionItems.length > 0) {
            setComposerSuggestionIndex(
              (index) => (index - 1 + composerSuggestionItems.length) % composerSuggestionItems.length,
            );
          }
          return;
        }
                                ...
                                <PickerPanel
                                  ...
                                  tabs={
                                    <div
                                      ...
                                      role="tablist"
                                      aria-label={t('agentManagement.detail.tabsLabel')}
                                    >
            ...
            return (
              <Fragment key={`${suggestion.kind}:${item.itemKind}:${item.id}`}>
                ...
                <button
                  ...
                  onClick={() => onPick(suggestion.kind, item.id, item.label, item.itemKind, item.takesArgs)}
                >
                  {isSlash ? (
                    <>
                      <span
                      ...
                      {item.source ? <span className="chat-composer-suggestion__source">{item.source}</span> : null}
                    </>
                  ) : (
                    <>
