...
export function isNativeSubagentRosterPayload(value: unknown): boolean {
  const raw = asRecord(value);
  if (!raw) return false;
  if (!asString(raw.subagent_id ?? raw.subagentId)) return false;
  if (raw.legacy_status != null) return true;
  const status = (asString(raw.status) ?? '').toLowerCase();
  return status === 'running' || status === 'idle' || status === 'closed' || status === 'starting' || status === 'pending';
}
