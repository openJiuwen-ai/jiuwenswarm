      ...
      return {
        runtimes: {
          ...
          [sessionId]: {
            ...
            taskQueue: [],
            // pendingQuestions 有意不在这里清空：重新挂载会话时，后端在
            // session.switch 上重推仍未回答的审批提示（parked ToolInterruptionState
            // 重建），而该推送可能赶在 history restore 的事件挂起窗口之前到达——若在
            // 这里清空，restore 收尾会把刚重推的提示抹掉，工具调用继续无声等待。
            // 队列的真正清空点是 consumePendingQuestion / clearPendingQuestions /
            // 新消息发送 / clearCurrentTurnData。
