...
async def call_agent_long_horizon(
    *,
    ...
    body = dict(params or {})
    body["action"] = action
    sid = str(service_id or body.get("service_id") or "").strip()
    aid = str(agent_id or body.get("agent_id") or "").strip()
    if sid:
        body["service_id"] = sid
    if aid:
        body["agent_id"] = aid
...
async def broadcast_stage_due(
    ...
    *,
    ...
    if hasattr(web, "broadcast_event"):
        ...
        await web.broadcast_event(STAGE_DUE_EVENT, frame, user_id=uid)
        return
    if hasattr(web, "send"):
