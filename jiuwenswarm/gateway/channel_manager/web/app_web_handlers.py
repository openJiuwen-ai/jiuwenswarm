...
def _register_web_handlers(bind: WebHandlersBindParams) -> None:
    ...
    async def _vendors_fetch_models(ws, req_id, params, session_id):
        ...
        try:
            ...
            # Some shared /models catalogues include models that the selected
            # plan cannot invoke. Restrict those responses to the curated,
            # plan-specific allowlist and keep its recommendation order.
            needs_plan_allowlist = vendor_key == "alibaba" or (
                vendor_key == "baidu" and plan == PlanKind.TOKEN_PLAN
            )
            if needs_plan_allowlist and remote_ids:
                allowlist_name = "Alibaba" if vendor_key == "alibaba" else "Baidu"
                ...
                if filtered_count:
                    logger.info(
                        "[vendors.fetch_models] filtered %d non-allowlisted %s models",
                        filtered_count,
                        allowlist_name,
                    )
                if not remote_ids:
                    remote_reason = (
                        f"no remote models matched the {allowlist_name} plan allowlist"
                    )
