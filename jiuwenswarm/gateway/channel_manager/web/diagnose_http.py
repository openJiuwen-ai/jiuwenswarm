...
from jiuwenswarm.observability.diagnosis.evidence import decode_span_records
...
def attach_diagnose_routes(
    ...
    *,
    ...
    async def diagnose_session(
        ...
        if offline:
            # 离线模式：不查 trace，纯日志诊断。trace_id 留空，records 空。
            # 证据链路对空 records 容错（locate_failure_window 返回 (0,0)，
            # _log_anchors 用 session_id + ERROR/Traceback 锚点纯日志定位）。
            if not uploaded_paths and not _truthy(fields.get("include_logs", False)):
                return _error(
                    "离线诊断必须提供日志（上传文件或目录导入）",
                    "OFFLINE_NO_LOGS",
                    400,
                )
            trace_id = ""
