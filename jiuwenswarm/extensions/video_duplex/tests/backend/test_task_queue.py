...
async def test_jobs_wait_for_execution_capacity(monkeypatch, same_session, fail_first, tmp_path):
    ...
    manager = video_search.VideoSearchManager(
        ...
        path=tmp_path / "tasks.sqlite", authorize=lambda ws, scope: ("test", scope),
    )
    first = manager.start(None, question="0", query="first", search_session_id="a", command_id="first")
    assert first["status"] == "queued"
    await asyncio.wait_for(entered[0].wait(), 2)
    second = manager.start(None, question="1", query="second", search_session_id="a" if same_session else "b", command_id="second")
    try:
        ...
        release.set()
        await asyncio.wait_for(asyncio.gather(*list(manager.service.workers.values())), 3)
