...
def _extract_placement_ips(instance_info: Mapping[str, Any] | None) -> tuple[str, str]:
    ...
    Register writes ``address=pending`` and ``node=pending`` because the
    registry rejects an empty address, and the first POST must not leave
    node empty either. Placement must be PATCHed once the actual IPs exist.
    YuanRong may use ``node_ip`` / ``sandbox_ip`` or pass through jiuwenbox
    ``ip_address``.
    """
...
class AgentOSRouterClient(AgentServerClient):
    ...
    async def _register_agent(self, agent_info: AgentInfo) -> None:
        ...
        try:
            ...
            # create 返回时沙箱通常还在探针中：address=pending，node=pending。
