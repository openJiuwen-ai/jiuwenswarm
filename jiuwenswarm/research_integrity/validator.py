...
def validate_integrity(
    *,
    ...
    Args:
        ...
        artifact_base_dir: Directory used to resolve relative expected
            artifact paths. Experiment runners should pass the actual process
            working directory so expected files are matched exactly.
    ...
    if pol.require_artifact:
        ...
        elif Path(spec.cwd).is_absolute():
            base_dir = Path(spec.cwd)

        for expected in spec.expected_artifacts:
            expected_path = Path(expected)
            if not expected_path.is_absolute() and base_dir is not None:
                expected_path = base_dir / expected_path

            if expected_path.is_absolute():
                expected_path = expected_path.resolve()
                found = any(
                    Path(record.path).resolve() == expected_path
                    for record in artifacts
                )
            else:
                # Legacy callers may provide a relative spec cwd without the
                # actual execution root. Compare whole path components in
                # that case; never use string suffixes (``other_results.json``
                # must not satisfy an expectation for ``results.json``).
                expected_parts = expected_path.parts
                found = any(
                    len(Path(record.path).parts) >= len(expected_parts)
                    and Path(record.path).parts[-len(expected_parts):]
                    == expected_parts
                    for record in artifacts
                )
            if not found:
