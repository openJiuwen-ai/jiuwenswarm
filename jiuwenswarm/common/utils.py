...
def _migrate_legacy_heartbeat_jobs(workspace_dir: Path) -> bool:
    ...
    ``agent/home`` predates the stable AgentServer data layout.  Heartbeat jobs
    were persisted there, so relocate that store while leaving unrelated home
    data in place.  The legacy Heartbeat gate routes the old store through this
    migration before AgentServer starts.
...
def _migrate_legacy_workspace(
    ...
    _migrate_legacy_heartbeat_jobs(workspace_dir)
