...
def e2a_response_from_agent_chunk(
    ...
    *,
    ...
    if event_type == "chat.delta":
        ...
        # Preserve the round identity and team-member attribution fields for
        # frontend display. ``rid`` is generated by the Team stream layer and
        # must survive the E2A conversion so delta and final share one key.
        for _key in ("rid", "role", "member_name", "team_name", "team_id", "source"):
            _val = pl.get(_key)
            if _val is not None:
...
def e2a_response_to_agent_chunk(e2a: E2AResponse) -> "AgentResponseChunk":
    ...
    if kind == E2A_RESPONSE_KIND_E2A_CHUNK and not e2a.is_final:
        ...
        if et == "chat.delta" or dk in ("text", "reasoning"):
            ...
            # Preserve the round identity and team-member attribution fields
            # for frontend display. ``rid`` is generated by the Team stream
            # layer and must survive the E2A conversion.
            for _key in ("rid", "role", "member_name", "team_name", "team_id", "source"):
                _val = body.get(_key)
                if _val is not None:
                    pl[_key] = _val
