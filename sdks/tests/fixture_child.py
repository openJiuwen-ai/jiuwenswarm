...
def emit(kind, **payload):
    global sequence
    record = {"schema_version": "0.1", "type": kind, "sequence": sequence, "request_id": request_id, "session_id": session_id, **payload}
    if mode == "bad_id":
    ...
    if mode == "duplicate":
        data = data.replace(b'"schema_version": "0.1"', b'"schema_version":"0.1","schema_version":"0.1"')
...
def finish(code=0):
    status = {0: "completed", 1: "failed", 2: "failed", 124: "timed_out", 130: "cancelled"}[code]
    common = {"status": status, "exit_code": code, "error": None if code == 0 else {"code": "TEST_ERROR", "message": "safe"}}
    if query:
        emit("query_result", operation=request["operation"], data={"fixture": True, "pid": os.getpid()}, **common)
    else:
...
if mode == "stubborn":
    ...
    Path(os.environ["SDK_MARKER"]).write_text(json.dumps([os.getpid(), child.pid]), encoding="utf-8")
    time.sleep(120)
    sys.exit(0)
if mode == "oversize":
...
if mode in ("ask", "two_questions", "wait", "callback_wait"):
    ...
    for index in range(count):
        emit("event", event_type="interaction.requested", payload={"interaction_id": f"opaque-{index}", "interaction": {"question": "answer"}})
        control = json.loads(sys.stdin.buffer.readline())
        assert control["request_id"] == request_id
        if control["type"] == "cancel":
            mark()
            finish(130)
            sys.exit(130)
        assert control["interaction_id"] == f"opaque-{index}"
        assert control["session_id"] == session_id
        assert control["answers"][0]["selected_options"] == ["yes"]
if mode == "error":
    finish(1)
    sys.exit(1)
