...
async def test_recommend_cache_real_entry_and_session_neutral_prewarm(tmp_path, monkeypatch):
    ...
    first = await manager.handle_skills_swarm_skills_hub_recommend({'top_k': 50, 'cache_mode': 'prefer_cache', '_catalog_anonymous': True, 'enrich': False})
    assert first['cache']['state'] == 'miss'
    await asyncio.sleep(.01)
    result = await manager.handle_skills_swarm_skills_hub_recommend({'top_k': 50, 'cache_mode': 'prefer_cache', 'session_id': 'second-session', 'enrich': False})
    ...
    private = await manager.handle_skills_swarm_skills_hub_recommend({'top_k': 50, 'cache_mode': 'prefer_cache', 'enrich': False})
...
async def test_server_authenticated_recommendation_is_nonblocking_memory_only(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(manager, '_team_skills_hub_http_post_data', post)
    first = await asyncio.wait_for(manager.handle_skills_swarm_skills_hub_recommend({'cache_mode': 'prefer_cache', 'top_k': 50, 'enrich': False}), .05)
    assert first['cache']['state'] == 'miss'
    gate.set()
    await asyncio.sleep(.01)
    result = await manager.handle_skills_swarm_skills_hub_recommend({'cache_mode': 'prefer_cache', 'top_k': 50, 'session_id': 'new-session', 'enrich': False})
    ...
    result = await manager.handle_skills_swarm_skills_hub_recommend({'cache_mode': 'prefer_cache', 'top_k': 50, 'enrich': False})
...
async def test_explicit_recommendation_credentials_and_sessions_are_isolated(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr(manager, '_team_skills_hub_http_post_data', post)
    params = {'cache_mode': 'prefer_cache', 'token': 'first', 'session_id': 'session-a', 'enrich': False}
