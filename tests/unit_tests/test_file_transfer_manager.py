...
def test_send_file_send_callback_failure_reports_send_error(manager, tmp_path):
    src = tmp_path / "ok.bin"
    src.write_bytes(b"x" * 150)

    async def send_callback(event_type, params):
        if event_type == FILE_DOWNLOAD_CHUNK:
    ...
    result = asyncio.run(_test())
    assert result["success"] is False
    assert "send file failed" in result["error"]
