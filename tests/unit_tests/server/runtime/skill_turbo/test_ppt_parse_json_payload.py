...
"""parse_json_payload 对流式杂散 fence 的容错回归测试。

背景（2026-09-11 PPT 生成任务中断事故，P4.2a）：
LLM 流式输出前置了一个内容为 ``*+0`` 的杂散空 fence，真实 JSON fence 的
开始反引号被杂散 fence 的闭合标记消耗，导致：
1. 首个 fence 匹配劫持提取（拿到 ``*+0`` 而非 JSON）；
2. 兜底正则 ``\\{[\\s\\S]*\\}`` 作用在「被替换后的 text」上，原始 raw 中的
   完整 JSON 再也无法被定位。
两者叠加使 parse_json_payload 对完整可解析的 JSON 返回 None，
触发 ContentPlanError → fallback → 权限中断链式失败。
"""
...
def test_stray_empty_fence_hijack_still_parses() -> None:
    """事故场景：杂散空 fence（"*+0"）劫持首个 fence 匹配后仍能解析。

    真实 JSON fence 的开始反引号被杂散 fence 的闭合标记消耗，
    fence 提取永远拿不到 JSON；兜底必须回退到「原始 raw」上做大括号定位。
    """
    raw = "```\n*+0\n```json\n" + _QUERIES_JSON + "\n```"
