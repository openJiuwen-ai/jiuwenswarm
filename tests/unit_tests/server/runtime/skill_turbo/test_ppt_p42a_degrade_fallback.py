...
契约：P4.2a 的 LLM 输出解析失败时——
- 无源素材且 topic 非空：降级为 topic 单查询继续快速调研；
- 有源素材或 topic 为空：保持响亮失败（抛 ContentPlanError）；
- JSON 合法但契约校验失败（数量/空 query 等）：响亮失败，不降级。
同时原始输出（截断）落 WARNING 日志便于排查。
"""
