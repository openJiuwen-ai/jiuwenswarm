...
@pytest.mark.asyncio
@pytest.mark.parametrize("published_shape", [False, True])
async def test_hub_agent_group_installs_and_uninstalls_by_asset_id(extension_workspace, tmp_path, published_shape):
    ...
    if published_shape:
        from jiuwenswarm.server.runtime.marketplace.asset_publish_adapters import normalize_and_validate
        from jiuwenswarm.server.runtime.marketplace.asset_publish_models import PublishIdentity

        normalize_and_validate(source, PublishIdentity("agent_group", "remote-group", "1.0.0"), {})
...
class TestAgentGroupLifecycle:
    ...
    def test_import_rootless_hub_group_archive_writes_local_uninstalled(
        self, extension_workspace: Path, tmp_path: Path
    ) -> None:
        source_workspace = tmp_path / "source-home" / "agent" / "workspace"
        source = _seed_valid_agent_group(
        ...
        assert imported.is_dir()
        assert catalog.is_agent_group_installed("hub-review") is False
