...
``_build_image_tool_fallback_notice`` tells the user that the attachment was
not sent to the answering model as native image input, why it was not, and that
an image-understanding tool was used instead. Only the streaming paths yielded
it, so a non-streaming request -- what chat connectors issue -- got the degraded
...
def _adapter(
    model_name: str, *, native_image_input: bool | None
) -> JiuWenSwarmDeepAdapter:
    adapter = object.__new__(JiuWenSwarmDeepAdapter)
    adapter._is_session_scoped_adapter = True
    adapter._parent_session_id = None
    # ``process_message_impl`` admits the request through
    # ``_permission_request_admission`` before dispatching. A default state is
    # neither isolated nor mid-update, and the Smart permission lifecycle is
    # declared unavailable, so admission falls straight through. Which
    # permission profile a session runs under is orthogonal to what the reply
    # payload carries.
    adapter._permission_state = SessionPermissionState()
    adapter._auto_permission_capability_enabled = MagicMock(return_value=False)
    adapter._enable_auto_permission = False
    adapter._permission_workspace_root = None
    adapter._root_permission_queue = None
    adapter._sys_operation = None
    adapter._sys_operation_card = None
    adapter._permission_dispatch = MagicMock()
    adapter._prepare_root_input_dispatch = AsyncMock(side_effect=lambda r, i: i)
    adapter._turn_tracker = MagicMock(sync=AsyncMock())
    adapter._active_loop_session = MagicMock(return_value=None)
    # ``None`` leaves ``enable_read_image_multimodal`` unset, which is what
    # sends ``_native_image_input_status`` to the probe cache for a verdict.
    adapter._config_cache = (
        {}
        if native_image_input is None
        else {"enable_read_image_multimodal": native_image_input}
    )
    # The notice under test is the one that says an image-understanding tool
    # stood in for the model, so the adapter must carry a vision tool config.
    adapter._vision_model_config = SimpleNamespace(model_name="vision-stub")
...
def _streamed_notice(
    adapter: JiuWenSwarmDeepAdapter, request: AgentRequest, status: str
) -> str:
    """The notice text the two streaming paths yield for this same turn.

    Built by calling the shared builder with the arguments the streaming call
    sites pass. The assertions then pin the non-streaming reply to the notice
    itself. The wording stays with the builder that owns it.
    """
    notice = JiuWenSwarmDeepAdapter._build_image_tool_fallback_notice(
        request,
        enable_read_image_multimodal=False,
        model=adapter._resolve_model_for_request(request),
        vision_tool_available=adapter._vision_model_config is not None,
        image_input_status=status,
    )
    return notice["content"]


def _status(adapter: JiuWenSwarmDeepAdapter, request: AgentRequest) -> str:
    return JiuWenSwarmDeepAdapter._native_image_input_status(
        adapter._config_cache,
...
def quiet_runtime(monkeypatch):
    ...
    # The run-span helpers are imported from the openjiuwen harness inside the
    # method under test, so the stubs are placed on their defining module.
    monkeypatch.setattr(
...
@pytest.mark.parametrize(
    "config_flag, cached_verdict, expected_status",
    [
        (False, None, "disabled"),
...
@pytest.mark.asyncio
async def test_notice_reaches_a_non_streaming_caller(
    quiet_runtime, monkeypatch, config_flag, cached_verdict, expected_status
):
    """The bug: a model without native image input answered from a tool
    summary and the payload said nothing about it.

    Every status that keeps native image input out of the turn is covered,
    because each one gives the reader a different reason and the non-streaming
    reply must give the reason the streaming paths give.
    """
    monkeypatch.setattr(
        "jiuwenswarm.server.runtime.agent_adapter.interface_deep."
        "get_cached_image_support",
        lambda _model: cached_verdict,
    )
    adapter = _adapter("glm-4", native_image_input=config_flag)
    request = _request()

    assert _status(adapter, request) == expected_status
    response = await adapter.process_message_impl(request, dict(request.params))

    assert response.ok
    payload = response.payload
    notice = _streamed_notice(adapter, request, expected_status)
    assert payload["content"] == f"{notice}\n\n{ANSWER}"
    # The answering model is named in the notice, so the reader can tell which
    # model did not read the attachment.
...
async def test_notice_stands_alone_when_the_run_produces_no_text(quiet_runtime):
    ...
    payload = response.payload
    assert payload["content"] == _streamed_notice(adapter, request, "disabled")
    assert set(payload) == {"content"}
...
async def test_notice_reaches_the_error_reply_that_still_carries_an_answer(
    ...
    notice = _streamed_notice(adapter, request, "disabled")
    assert payload["content"] == f"{notice}\n\n{ANSWER}"
