...
from jiuwenswarm.agents.harness.common.tools.deepresearch import rewrite_tools as rt
...
async def test_prepare_and_commit_tools_return_short_outcomes_and_deliver_file(tmp_path):
    ...
    with patch.object(rt, "_get_route", return_value=route), patch.object(
        rt, "_get_effective_request_output_dir", return_value=str(tmp_path)
    ):
...
@pytest.mark.asyncio
async def test_commit_publish_observer_runs_before_delivery_and_callback_failure_isolated():
    delivery_entered = asyncio.Event()
    allow_delivery = asyncio.Event()
    result = {
        "report_path": "/workspace/report-v2.md",
        "provenance_path": "/workspace/report-v2.provenance.json",
        "revision_id": "rev_child",
    }

    async def blocking_delivery(*_args):
        delivery_entered.set()
        await allow_delivery.wait()
        return True

    observed = []
    with patch.object(rt, "_validate_commit_contract"), patch.object(
        rt, "_get_route", return_value={"session_id": "S1", "channel_id": "C1"}
    ), patch.object(rt, "commit_rewrite", return_value=result), patch.object(
        rt, "_deliver_report", side_effect=blocking_delivery
    ), rt._observe_rewrite_publish(lambda published: observed.append(published)):
        task = asyncio.create_task(
            rt.deepresearch_commit_rewrite._func("token", {"units": []})
        )
        await delivery_entered.wait()
        assert observed == [result]
        allow_delivery.set()
        assert json.loads(await task)["report_delivered"] is True

    with patch.object(rt, "_validate_commit_contract"), patch.object(
        rt, "_get_route", return_value={"session_id": "S1", "channel_id": "C1"}
    ), patch.object(rt, "commit_rewrite", return_value=result), patch.object(
        rt, "_deliver_report", new=AsyncMock(return_value=True)
    ), rt._observe_rewrite_publish(
        lambda _published: (_ for _ in ()).throw(RuntimeError("observer-secret"))
    ):
        raw = await rt.deepresearch_commit_rewrite._func("token", {"units": []})

    assert json.loads(raw)["status"] == "completed"


@pytest.mark.asyncio
async def test_commit_publish_observer_is_context_local_between_concurrent_tasks():
    def commit(*, context_token, **_kwargs):
        return {
            "report_path": f"/workspace/{context_token}.md",
            "provenance_path": f"/workspace/{context_token}.json",
            "revision_id": f"rev_{context_token}",
        }

    async def invoke(label):
        observed = []
        with rt._observe_rewrite_publish(
            lambda published: observed.append(published["revision_id"])
        ):
            await rt.deepresearch_commit_rewrite._func(label, {"units": []})
        return observed

    with patch.object(rt, "_validate_commit_contract"), patch.object(
        rt, "_get_route", return_value={"session_id": "S1", "channel_id": "C1"}
    ), patch.object(rt, "commit_rewrite", side_effect=commit), patch.object(
        rt, "_deliver_report", new=AsyncMock(return_value=True)
    ):
...
async def test_commit_transport_failure_does_not_mask_completed_revision(tmp_path):
    ...
    with patch.object(rt, "_get_route", return_value=route), patch.object(
        ...
        with patch(
            "jiuwenswarm.server.gateway_push.transport.WebSocketGatewayPushTransport",
            return_value=push,
        ), patch.object(rt.logger, "exception") as exception_log, patch.object(
            rt.logger, "error"
        ) as error_log:
            committed_raw = await rt.deepresearch_commit_rewrite._func(
                context_token=prepared["context_token"],
    ...
    assert "secret transport detail" not in str(error_log.call_args_list)
    assert Path(committed["report_path"]).is_file()
...
async def test_generate_rewrite_html_requires_workspace_and_full_route_before_core(
    ...
    with patch.object(rt, "_get_route", return_value=route), patch.object(
        rt, "_get_effective_request_output_dir", return_value=output_dir
    ), patch.object(
