...
def test_real_workspace_migration_preserves_heartbeat_jobs(
    ...
    unrelated = legacy.parent / "other" / "history.json"
...
def test_runtime_preparation_ignores_unrelated_legacy_home(
    workspace: Path,
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    ...
    monkeypatch.setattr(
        common_utils,
        "prepare_workspace",
        lambda **_kwargs: pytest.fail("unrelated home must not trigger migration"),
    )
    monkeypatch.setattr(
        common_utils,
        "ensure_config_migrated_from_template",
