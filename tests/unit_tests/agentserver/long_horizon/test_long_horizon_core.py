...
def test_mutate_long_horizon_task_serializes_stage_writes(tmp_path):
    from jiuwenswarm.agents.harness.common.long_horizon.core import (
        mutate_long_horizon_task,
    )

    task = make_task()
    task.status = "active"
    upsert_long_horizon_task(tmp_path, task)
    first_id = task.stages[0].id
    second_id = task.stages[1].id

    def mark_first(current):
        current.stages[0].status = "due"
        return True

    def mark_second(current):
        assert current.stages[0].status == "due"
        current.stages[1].status = "in_progress"
        return True

    mutate_long_horizon_task(tmp_path, task.id, mark_first)
    mutate_long_horizon_task(tmp_path, task.id, mark_second)
    stored = get_long_horizon_task(tmp_path, task.id)
    assert stored is not None
    by_id = {stage.id: stage.status for stage in stored.stages}
    assert by_id[first_id] == "due"
    assert by_id[second_id] == "in_progress"


def test_rewrite_keeps_due_stage_due_at_in_the_past():
    from jiuwenswarm.agents.harness.common.long_horizon.core import rewrite_task_stages

    task = make_task()
    due_at = "2026-09-10T09:00:00+08:00"
    task.stages[0].status = "due"
    task.stages[0].due_at = due_at
    specs = [
        {
            "id": task.stages[0].id,
            "title": task.stages[0].title,
            "month": 11,
            "day": 20,
            "plan": task.stages[0].plan,
        },
        {
            "id": task.stages[1].id,
            "title": task.stages[1].title,
            "month": 12,
            "day": 20,
            "plan": task.stages[1].plan,
        },
    ]
    rewritten = rewrite_task_stages(task, specs, now=NOW)
    assert rewritten.stages[0].status == "due"
    assert rewritten.stages[0].due_at == due_at


def test_build_due_payload_prefers_task_owner_user_id():
    from jiuwenswarm.agents.harness.common.long_horizon.runtime import build_due_payload

    task = make_task()
    task.owner_user_id = "u_web_1"
    payload = build_due_payload(task, task.stages[0])
    assert payload["user_id"] == "u_web_1"
