...
def test_agent_eager_tools_include_long_horizon_task():
    config = get_config()
    ...
    assert "long_horizon_task" in list(eager or [])
    assert "long_horizon_task" in interface_deep._DEFAULT_PROGRESSIVE_EAGER_TOOLS
...
def test_config_yaml_eager_tools_list_long_horizon_task():
    ...
    assert "long_horizon_task" in eager
