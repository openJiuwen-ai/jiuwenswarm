...
@pytest.mark.asyncio
async def test_confirm_delivers_every_overdue_stage_and_schedules_future(
    workspace, fake_cron
):
    params = valid_draft_params()
    params["now"] = "2026-09-13T12:00:00+08:00"
    params["stages"] = [
        {
            "title": "昨天",
        ...
        {
            "title": "前天",
            "due_at": "2026-09-14T09:00:00+08:00",
            "plan": "补前天的工作。",
        },
        {
            "title": "未来",
            "due_at": "2026-12-20T09:00:00+08:00",
            "plan": "按计划推进。",
        },
    ]
    actions = LongHorizonActions(workspace, cron=fake_cron)
    drafted = await actions.handle(params)
    confirmed = await actions.handle(confirm_params(drafted["task"]["id"]))
    assert confirmed["success"] is True
    stored = load_long_horizon_tasks(workspace)[0]
    overdue = [s for s in stored.stages if s.title in ("昨天", "前天")]
    assert {s.status for s in overdue} == {"due"}
    future = next(s for s in stored.stages if s.title == "未来")
    assert future.status == "pending"
    assert len(fake_cron.jobs) == 1
    assert stage_job_id(stored.id, future.id) in fake_cron.jobs
    inbox = await actions.handle({"action": "inbox"})
    assert {item["stage"]["title"] for item in inbox["inbox"]} == {"昨天", "前天"}
