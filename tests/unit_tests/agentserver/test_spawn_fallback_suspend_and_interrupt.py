...
背景（2026-09-11 PPT 生成任务中断事故，fallback 通道）：
1. ``_suspend_subagent_permission_rails`` 传 ``{"enabled": False}`` 给
   ``rail.update_config``，而 agent-core 的 update_config 是整体替换语义，
   tools/file_guard/rules 被一并清空；fallback 期间
   WorkspaceUntrustedPolicyEngine 读到空 tools（web_search 无规则 → default ASK）
   与空 file_guard（信任判定必然 False），工具调用被中断。
2. interrupt result（``{"result_type": "interrupt", ...}``）无 output 字段，
   ``spawn_fallback`` 把它吞成空字符串返回，契约校验只能笼统报
   「未达成节点契约」，掩盖了真实原因（HITL 中断未被解决）。
"""
