...
async def test_canonical_personal_context_bypasses_chat_hooks_and_agent_creation(
    ...
    monkeypatch.setattr(
        ...
        AsyncMock(side_effect=AssertionError("PersonalContext must bypass chat hooks")),
    )
    monkeypatch.setattr(
        pipeline_module,
        "_handle_unary",
        AsyncMock(side_effect=AssertionError("PersonalContext must bypass Agent creation")),
    )
    ws = _FakeWebSocket()
    ...
    assert host.calls == [("get_status", None)]
    pipeline_module._trigger_before_chat_request_hook.assert_not_awaited()
    pipeline_module._handle_unary.assert_not_awaited()
...
async def test_personal_context_failure_keeps_connection_and_ordinary_request_available(
    ...
    async def ordinary_response(
        ...
        await server_module.send_wire_payload(ws, wire)

    monkeypatch.setattr(
        pipeline_module, "_trigger_before_chat_request_hook", AsyncMock()
    )
    monkeypatch.setattr(
        pipeline_module, "_ensure_auto_team_binding_for_chat", AsyncMock()
    )
    monkeypatch.setattr(
        pipeline_module, "_handle_unary", AsyncMock(side_effect=ordinary_response)
    )
    await server._handle_message(
        ws,
        json.dumps(
    ...
    assert ws.sent[1]["status"] == "succeeded"
    pipeline_module._handle_unary.assert_awaited_once()
