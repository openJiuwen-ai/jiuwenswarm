...
def _make_loop_adapter(chunks: list[Any]) -> JiuWenSwarmDeepAdapter:
    ...
    # Read by the login-model check upstream runs before every stream.
    adapter._model_cache = {}
    adapter._model_name_to_keys = {}
    # The trajectory turn tracker the loop resolves a root span against.
    adapter._turn_tracker = adapter_module.SessionTurnTracker()
    # Read by upstream's stream loop since the 2026-09 rebase; unused here.
    adapter._voice_agent_task_rail = None
    adapter._session_input_guard = None
    ...
    adapter._flush_pending_goal_objective_history = _async_none
