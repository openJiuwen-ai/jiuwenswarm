...
from jiuwenswarm.agents.harness.common.tools.deepresearch_plugin import document_rewrite as rewrite_module
from jiuwenswarm.agents.harness.common.tools.deepresearch_plugin import conversion_utils
from jiuwenswarm.agents.harness.common.tools.deepresearch_plugin import artifact_naming
...
def test_metadata_loaders_request_only_limit_plus_one_bytes(
    ...
    if loader == "provenance":
        report.with_suffix(".provenance.json").write_bytes(b"x")
        monkeypatch.setattr(rewrite_module, "PROVENANCE_MAX_BYTES", limit)

        def invoke():
            return rewrite_module._load_provenance(report)
    else:
        report.with_suffix(".final-result.json").write_bytes(b"x")
        monkeypatch.setattr(rewrite_module, "FINAL_RESULT_MAX_BYTES", limit)

        def invoke():
            return rewrite_module._load_final_result_citations(
                report,
                {
    ...
    def read_probe(_descriptor, size):
        requested_sizes.append(size)
        return b"x" * (limit + 1)

    monkeypatch.setattr(rewrite_module.os, "read", read_probe)
...
def test_commit_publishes_provenance_before_markdown(tmp_path, monkeypatch):
    ...
    def record_publish(path, payload, **kwargs):
...
def test_commit_retries_markdown_publication_collision_without_overwrite(
    ...
    def collide_once(path, payload, **kwargs):
        nonlocal raced
...
def test_commit_publication_failure_preserves_concurrent_content_and_parent(
    ...
    def fail_markdown_after_provenance(path, payload, **kwargs):
...
def test_commit_keyboard_interrupt_cleans_owned_provenance_and_reraises(
    tmp_path, monkeypatch
):
    report, _ = _write_document(tmp_path, "original\n")
    parent_markdown = report.read_bytes()
    ...
    def interrupt_markdown(path, payload, **kwargs):
        if path.name == "report-v2.md":
            raise KeyboardInterrupt
        return real_publish(path, payload, **kwargs)

    monkeypatch.setattr(
        rewrite_module, "_publish_create", interrupt_markdown
    )
...
def test_commit_keyboard_interrupt_preserves_replaced_provenance(tmp_path, monkeypatch):
    ...
    def replace_and_interrupt(path, payload, **kwargs):
        if path.name == "report-v2.md":
            provenance_path = path.with_suffix(".provenance.json")
            provenance_path.unlink()
            provenance_path.write_bytes(b"concurrent provenance")
    ...
    monkeypatch.setattr(
        rewrite_module, "_publish_create", replace_and_interrupt
    )
...
def test_sidecar_only_orphan_is_not_committed_and_next_commit_uses_v3(tmp_path):
    report, parent_provenance = _write_document(tmp_path, "original\n")
    orphan = dict(parent_provenance)
    orphan.update(
        {
            "revision_id": "rev_orphan",
            "version_number": 2,
            "version_base_stem": "report",
            "markdown_path": str(tmp_path / "report-v2.md"),
        }
    )
...
def test_document_lock_registry_reclaims_successful_commit(tmp_path):
    report, _ = _write_document(tmp_path, "original\n")
    prepared = _prepare(tmp_path, report, "original")
    with rewrite_module._CONTEXT_LOCK:
        rewrite_module._DOCUMENT_LOCKS.clear()
...
def test_concurrent_same_document_commits_are_serial_and_use_unique_versions(
    tmp_path,
):
    report, _ = _write_document(tmp_path, "original\n")
    prepared = [
        _prepare(tmp_path, report, "original"),
        _prepare(tmp_path, report, "original"),
    ]
    ...
    def run_commit(item):
        start.wait()
        return commit_rewrite(
            context_token=item["context_token"],
            session_id="S1",
            structured_result=_structured_payload(item),
        )

    with ThreadPoolExecutor(max_workers=2) as executor:
        results = list(executor.map(run_commit, prepared))

    assert sorted(Path(result["report_path"]).name for result in results) == [
        "report-v2.md",
        "report-v3.md",
    ]
    with rewrite_module._CONTEXT_LOCK:
...
def test_ssrf_filter_log_does_not_disclose_url_path_or_secret_query(monkeypatch):
    secret = "api-token-super-secret"
    url = f"https://images.example/private/image.png?token={secret}"
    messages = []

    def capture_log(message, *args):
        messages.append(message % args)

    monkeypatch.setattr(conversion_utils.logger, "warning", capture_log)

    filtered = conversion_utils._filter_remote_urls_for_ssrf(
        f"![preview]({url})"
    )

    assert filtered == "[远程图片已过滤: preview]"
    assert len(messages) == 1
    assert "https://images.example" in messages[0]
    assert "/private/image.png" not in messages[0]
    assert secret not in messages[0]


def _document_leaf(report: Path, provenance: dict, leaf: str) -> Path:
    if leaf == "report":
        return report
    if leaf == "provenance":
        return report.with_suffix(".provenance.json")
    return report.parent / provenance["final_result_path"]


def _replace_with_unsafe_leaf(
    path: Path, kind: str, outside: Path, monkeypatch
) -> bytes:
    ...
    if kind == "symlink":
        ...
        def guarded_open(self, *args, **kwargs):
            if self == path:
                raise AssertionError("unsafe FIFO open would block")
            return real_open(self, *args, **kwargs)

        def guarded_read_bytes(self):
            if self == path:
                raise AssertionError("unsafe FIFO read would block")
...
@pytest.mark.parametrize("leaf", ["report", "provenance", "final_result"])
@pytest.mark.parametrize("kind", ["symlink", "hardlink", "fifo"])
def test_prepare_rewrite_rejects_unsafe_document_leaves(
    tmp_path, monkeypatch, leaf, kind
):
    body = "original text\n"
    report, provenance = _write_document(tmp_path, body)
    outside = tmp_path / f"outside-{leaf}-{kind}"
    target = _document_leaf(report, provenance, leaf)
    original = _replace_with_unsafe_leaf(target, kind, outside, monkeypatch)
    ...
    with pytest.raises(RewriteError) as caught:
        prepare_rewrite(
            ...
            report_path=report,
            action="polish",
            selection=_selection(body, "original text"),
            session_id="S1",
        )
...
def test_prepare_rejects_initial_symlink_workspace_root(tmp_path):
    ...
    with pytest.raises(RewriteError) as caught:
        _prepare(workspace, report, "original")
...
@pytest.mark.parametrize("leaf", ["report", "provenance", "final_result"])
@pytest.mark.parametrize("kind", ["symlink", "hardlink", "fifo"])
def test_prepare_html_export_rejects_unsafe_document_leaves(
    tmp_path, monkeypatch, leaf, kind
):
    parent, result = _committed_child(tmp_path)
    child = Path(result["report_path"])
    child_provenance = json.loads(
        Path(result["provenance_path"]).read_text(encoding="utf-8")
    )
    outside = tmp_path / f"outside-export-{leaf}-{kind}"
    target = _document_leaf(child, child_provenance, leaf)
    original = _replace_with_unsafe_leaf(target, kind, outside, monkeypatch)
    ...
    if kind != "fifo":
        assert outside.read_bytes() == original
    assert parent.read_text(encoding="utf-8") == "original text\n"


def test_prepare_html_export_rejects_initial_symlink_workspace_root(tmp_path):
    outside = tmp_path / "outside"
    outside.mkdir()
    ...
    with pytest.raises(RewriteError) as caught:
        prepare_html_export(
            ...
            report_path=workspace / outside_child.name,
            revision_id=result["revision_id"],
        )
...
def test_fallback_reader_rejects_workspace_reparse_point(tmp_path, monkeypatch):
    report = tmp_path / "report.md"
    report.write_text("outside", encoding="utf-8")
    real_lstat = rewrite_module.os.lstat
    ...
    monkeypatch.setattr(rewrite_module.os, "lstat", reparse_root)

    with pytest.raises(OSError, match="root must be a direct directory"):
        rewrite_module._read_regular_bounded_fallback(
            report, tmp_path, Path("report.md"), 64, "report"
        )


def test_artifact_publication_rejects_reparse_directory_metadata():
    ...
    with pytest.raises(OSError, match="direct directory"):
        rewrite_module._opened_directory_identity(metadata)


@pytest.mark.parametrize("leaf", ["report", "provenance", "final_result"])
@pytest.mark.parametrize("kind", ["symlink", "hardlink", "fifo"])
def test_commit_rewrite_rejects_unsafe_document_leaves(
    tmp_path, monkeypatch, leaf, kind
):
    body = "original text\n"
    report, provenance = _write_document(tmp_path, body)
    prepared = _prepare(tmp_path, report, "original text")
    outside = tmp_path / f"outside-commit-{leaf}-{kind}"
    target = _document_leaf(report, provenance, leaf)
    original = _replace_with_unsafe_leaf(target, kind, outside, monkeypatch)
    ...
    expected_code = "DOCUMENT_NOT_FOUND" if leaf == "final_result" else "REVISION_CONFLICT"
...
def test_prepare_rewrite_rejects_oversized_markdown(tmp_path, monkeypatch):
    report, _ = _write_document(tmp_path, "original text\n")
    monkeypatch.setattr(rewrite_module, "MARKDOWN_MAX_BYTES", 8)
    ...
    with pytest.raises(RewriteError) as caught:
        prepare_rewrite(
            ...
            report_path=report,
            action="polish",
            selection=_selection("original text\n", "original text"),
            session_id="S1",
        )
...
def test_prepare_html_export_rejects_oversized_markdown(tmp_path, monkeypatch):
    _, result = _committed_child(tmp_path)
    monkeypatch.setattr(rewrite_module, "MARKDOWN_MAX_BYTES", 8)
...
def test_commit_rewrite_rejects_oversized_current_markdown(tmp_path, monkeypatch):
    report, _ = _write_document(tmp_path, "original text\n")
    prepared = _prepare(tmp_path, report, "original text")
    monkeypatch.setattr(rewrite_module, "MARKDOWN_MAX_BYTES", 8)
...
def test_safe_reader_rejects_opened_inode_mismatch(tmp_path, monkeypatch):
    path = tmp_path / "report.md"
    path.write_bytes(b"safe")
    real_fstat = rewrite_module.os.fstat
    ...
    monkeypatch.setattr(rewrite_module.os, "fstat", mismatched_fstat)

    with pytest.raises(OSError):
        rewrite_module._read_regular_bounded_beneath(
            path, tmp_path.resolve(), 16, "report"
        )


def _swap_artifact_ancestor(
    artifacts: Path, owned_old: Path, outside: Path
) -> None:
    artifacts.rename(owned_old)
    artifacts.symlink_to(outside, target_is_directory=True)


@pytest.mark.skipif(os.name == "nt", reason="POSIX directory descriptor contract")
def test_commit_rejects_ancestor_swap_after_allocation_without_outside_write(
    tmp_path, monkeypatch
):
    workspace = tmp_path / "workspace"
    artifacts = workspace / "artifacts"
    outside = tmp_path / "outside"
    owned_old = workspace / "artifacts-owned-old"
    artifacts.mkdir(parents=True)
    outside.mkdir()
    concurrent = outside / "concurrent.txt"
    concurrent.write_bytes(b"outside-concurrent")
    report, _ = _write_document(artifacts, "original\n")
    prepared = _prepare(workspace, report, "original")
    parent_markdown = report.read_bytes()
    parent_provenance = report.with_suffix(".provenance.json").read_bytes()
    real_allocate = rewrite_module._allocate_paths
    swapped = False

    def allocate_then_swap(*args, **kwargs):
        nonlocal swapped
        paths = real_allocate(*args, **kwargs)
        if not swapped:
            swapped = True
            _swap_artifact_ancestor(artifacts, owned_old, outside)
        return paths

    monkeypatch.setattr(
        rewrite_module, "_allocate_paths", allocate_then_swap
    )
    ...
    assert caught.value.code == "INVALID_PROVENANCE"
    assert concurrent.read_bytes() == b"outside-concurrent"
    assert sorted(path.name for path in outside.iterdir()) == ["concurrent.txt"]
    assert artifacts.is_symlink()
    assert (owned_old / report.name).read_bytes() == parent_markdown
    assert (owned_old / report.with_suffix(".provenance.json").name).read_bytes() == (
        parent_provenance
    )
    assert not (owned_old / "report-v2.md").exists()
    assert not (owned_old / "report-v2.provenance.json").exists()


@pytest.mark.skipif(os.name == "nt", reason="POSIX directory descriptor contract")
def test_commit_allocation_scan_does_not_read_swapped_outside_sidecars(
    tmp_path, monkeypatch
):
    workspace = tmp_path / "workspace"
    artifacts = workspace / "artifacts"
    ...
    report, _ = _write_document(artifacts, "original\n")
    prepared = _prepare(workspace, report, "original")
    malicious_name = "malicious-v99.provenance.json"
    (outside / malicious_name).write_text(
        json.dumps(
            {
                "document_id": "doc_test",
                "version_number": 99,
    ...
    (outside / "malicious-v99.md").write_text("outside", encoding="utf-8")
    real_snapshot = artifact_naming._snapshot_directory_names
    real_read = artifact_naming._read_file_bytes
    outside_reads = []
    swapped = False

    def swap_during_snapshot(directory, *args, **kwargs):
        nonlocal swapped
        if not swapped:
            swapped = True
            _swap_artifact_ancestor(artifacts, owned_old, outside)
        return real_snapshot(directory, *args, **kwargs)

    def record_reads(path, *args, **kwargs):
        if path.name == malicious_name:
            outside_reads.append(path)
        return real_read(path, *args, **kwargs)

    monkeypatch.setattr(
        artifact_naming, "_snapshot_directory_names", swap_during_snapshot
    )
    monkeypatch.setattr(artifact_naming, "_read_file_bytes", record_reads)
...
@pytest.mark.skipif(os.name == "nt", reason="POSIX directory descriptor contract")
def test_commit_rejects_trusted_workspace_root_swap_after_allocation(
    tmp_path, monkeypatch
):
    workspace = tmp_path / "workspace"
    artifacts = workspace / "artifacts"
    outside = tmp_path / "outside"
    owned_old = tmp_path / "workspace-owned-old"
    artifacts.mkdir(parents=True)
    outside.mkdir()
    concurrent = outside / "concurrent.txt"
    concurrent.write_bytes(b"outside-concurrent")
    ...
    assert (owned_old / "artifacts" / report.name).read_bytes() == parent_markdown
    assert (
        owned_old / "artifacts" / report.with_suffix(".provenance.json").name
    ).read_bytes() == parent_provenance
    assert not (owned_old / "artifacts" / "report-v2.md").exists()
    assert not (
        owned_old / "artifacts" / "report-v2.provenance.json"
    ).exists()
