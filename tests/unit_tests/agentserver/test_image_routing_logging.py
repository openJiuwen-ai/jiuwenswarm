...
Extraction runs before ``_native_image_input_status`` is consulted, so a log
...
the log to the branch that is actually taken, and to the status that sent it
there.
"""
...
@pytest.mark.parametrize("status", ["disabled", "unsupported", "unknown"])
def test_native_input_not_in_use_names_the_tool_route(status):
    """The production case: the attachment reaches the model through the
    image-understanding tool only, and the log must say so.

    The line also names the status, because a false
    ``enable_read_image_multimodal`` stands for three different findings and
    the log may not pick one of them on the code's behalf."""
    request = _request()
    inputs = JiuWenSwarmDeepAdapter._prepare_multimodal_image_inputs(
        request, {"query": request.params["query"]}
    )
    ...
    with _adapter_logs() as records:
        result = JiuWenSwarmDeepAdapter._prepare_react_image_tool_prompt(
            ...
            vision_tool_available=True, image_input_status=status,
        )
    ...
    messages = _messages(records)
    assert any(
        f"image_input_status={status}" in m and "image-understanding tool" in m
        for m in messages
    ), messages
    assert not any("Native image input enabled" in m for m in messages), messages
    for other in {"disabled", "unsupported", "unknown"} - {status}:
        assert not any(
            f"image_input_status={other}" in m for m in messages
        ), messages
...
def test_native_input_disabled_without_a_vision_tool_does_not_claim_the_route():
    ...
    with _adapter_logs(logging.DEBUG) as records:
        result = JiuWenSwarmDeepAdapter._prepare_react_image_tool_prompt(
            request, inputs, enable_read_image_multimodal=False,
            vision_tool_available=False, image_input_status="unknown",
        )
    ...
    assert any(
        ...
        and "image_input_status=unknown" in m
        for m in messages
    ), messages
    assert not any(
...
def test_unbuildable_tool_prompt_warns_instead_of_claiming_a_route():
    ...
    with _adapter_logs(logging.WARNING) as records:
        JiuWenSwarmDeepAdapter._prepare_react_image_tool_prompt(
            request, inputs, enable_read_image_multimodal=False,
            vision_tool_available=True, image_input_status="unsupported",
        )

    messages = _messages(records)
    assert any(
        "could not be built" in m
        and "not routed to the model" in m
        and "image_input_status=unsupported" in m
        for m in messages
    ), messages
