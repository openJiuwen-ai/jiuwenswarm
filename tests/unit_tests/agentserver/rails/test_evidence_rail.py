...
class _UnexpectedStoreError(Exception):
    """A store failure outside the rail's declared ``_STORE_ERRORS`` tuple."""


class _RaisingStore:
    """Store whose every entry point raises one caller-supplied error."""
...
class _ReceiptAppendFailingStore(FileEvidenceRailStore):
    """Real store that only fails when a tool receipt is appended."""

    def append_tool_receipt(self, receipt) -> None:
        del receipt
        raise OSError("receipt sink unavailable: secret-should-not-be-returned")


class _RejectingPromptBuilder:
    def add_section(self, section) -> None:
        del section
        raise TypeError("section rejected: secret-should-not-be-returned")

    def remove_section(self, name: str) -> None:
        del name


class _UndigestibleResult:
    """Result object whose ``repr`` fails, breaking the canonicalizer."""

    def __repr__(self) -> str:
        raise TypeError("repr unavailable: secret-should-not-be-returned")
...
def _rail(
    tmp_path: Path, *ids: str, evidence: list[EvidenceItem] | None = None
) -> EvidenceRail:
    return EvidenceRail(
        EvidenceRailConfig(
...
async def test_full_lifecycle_emits_hash_only_replay_artifacts(tmp_path: Path) -> None:
    ...
    context_lines = (
        (run_dir / "context_digests.jsonl").read_text(encoding="utf-8").splitlines()
    )
    tool_lines = (
        (run_dir / "tool_receipts.jsonl").read_text(encoding="utf-8").splitlines()
    )
...
@pytest.mark.asyncio
async def test_missing_verified_evidence_force_finishes_and_blocks(
    tmp_path: Path,
) -> None:
    rail = _rail(
        tmp_path,
...
async def test_tool_failure_retries_once_then_force_finishes(tmp_path: Path) -> None:
    ...
    receipts = [
        json.loads(line)
        for line in (run_dir / "tool_receipts.jsonl")
...
@pytest.mark.asyncio
async def test_nonretryable_permission_failure_blocks_immediately(
    tmp_path: Path,
) -> None:
    evidence = EvidenceItem(evidence_id="ev-1", source="local", content="verified")
    ...
    finish = ctx.consume_force_finish()
    assert finish is not None
    assert finish.result["evidencerail"]["reason_code"] == "TOOL_PERMISSION_DENIED"


@pytest.mark.asyncio
async def test_store_failure_uses_force_finish_not_callback_exception(
    tmp_path: Path,
) -> None:
    rail = EvidenceRail(
...
@pytest.mark.asyncio
async def test_store_error_outside_declared_types_propagates(tmp_path: Path) -> None:
    """An unforeseen store failure reaches the callback framework's boundary.

    The rail catches only the types a store can plausibly raise.  Anything else
    is a defect, and ``CallbackFramework.trigger`` -- the designated boundary
    for the callback chain -- logs it with its traceback.  Flattening it into a
    reason code here would hide the defect instead.
    """
    rail = EvidenceRail(
        EvidenceRailConfig(artifact_root=str(tmp_path)),
        store=_RaisingStore(_UnexpectedStoreError("unforeseen")),
        clock=lambda: FIXED_NOW,
        id_factory=_id_factory("unexpected"),
    )
    ctx = AgentCallbackContext(
        agent=SimpleNamespace(),
        inputs=InvokeInputs(query="q"),
    ...
    with pytest.raises(_UnexpectedStoreError):
        await rail.before_invoke(ctx)
...
@pytest.mark.asyncio
async def test_cancellation_is_never_swallowed(tmp_path: Path) -> None:
    """Cancellation derives from ``BaseException`` and must not be converted."""
    rail = EvidenceRail(
        EvidenceRailConfig(artifact_root=str(tmp_path)),
        store=_RaisingStore(asyncio.CancelledError()),
        clock=lambda: FIXED_NOW,
        id_factory=_id_factory("cancelled"),
    )
    ctx = AgentCallbackContext(
        agent=SimpleNamespace(),
        inputs=InvokeInputs(query="q"),
...
@pytest.mark.asyncio
async def test_missing_prompt_builder_blocks_before_model_call(tmp_path: Path) -> None:
    """Without a prompt builder the projection cannot reach the model."""
    evidence = EvidenceItem(evidence_id="ev-1", source="local", content="verified")
    rail = _rail(tmp_path, "no-builder", evidence=[evidence])
    # `init` is deliberately not called, so `_prompt_builder` stays None.
    ctx = AgentCallbackContext(
        agent=SimpleNamespace(),
        inputs=InvokeInputs(query="q"),
        extra={},
    ...
    assert (
        ...
        == "EVIDENCE_CONTEXT_INJECTION_FAILED"
    )
    assert "error_type" not in finish.result["evidencerail"]
    manifest = RunManifest.model_validate_json(
        (tmp_path / "run-no-builder" / "run_manifest.json").read_text(encoding="utf-8")
    )
    assert manifest.status == "blocked"
    assert manifest.terminal_reason_code == "EVIDENCE_CONTEXT_INJECTION_FAILED"


@pytest.mark.asyncio
async def test_prompt_section_rejection_blocks_without_leaking_detail(
    tmp_path: Path,
) -> None:
    """A prompt builder raising ``TypeError`` maps to the injection reason code."""
    evidence = EvidenceItem(evidence_id="ev-1", source="local", content="verified")
    rail = _rail(tmp_path, "reject", evidence=[evidence])
    rail.init(SimpleNamespace(system_prompt_builder=_RejectingPromptBuilder()))
    ctx = AgentCallbackContext(
        agent=SimpleNamespace(),
        inputs=InvokeInputs(query="q"),
        extra={},
    ...
    assert finish.result["evidencerail"]["error_type"] == "TypeError"
    assert "secret-should-not-be-returned" not in json.dumps(finish.result)
    assert not (tmp_path / "run-reject" / "context_digests.jsonl").exists()


@pytest.mark.asyncio
async def test_unsafe_receipt_id_blocks_with_receipt_reason(tmp_path: Path) -> None:
    """A custom id factory returning an unsafe path component fails closed."""
    rail = _rail(tmp_path, "unsafe-id", "../escape")
    ctx = AgentCallbackContext(
        agent=SimpleNamespace(),
        inputs=InvokeInputs(query="q"),
        extra={},
    ...
    await rail.before_invoke(ctx)
    ctx.inputs = ToolCallInputs(tool_name="fetch", tool_args={})
...
@pytest.mark.asyncio
async def test_receipt_persist_failure_blocks_with_store_reason(
    tmp_path: Path,
) -> None:
    """An ``OSError`` while appending a receipt maps to the store reason code."""
    rail = EvidenceRail(
        EvidenceRailConfig(artifact_root=str(tmp_path)),
        store=_ReceiptAppendFailingStore(tmp_path),
        clock=lambda: FIXED_NOW,
        monotonic=_sequence(10.0, 10.25),
        id_factory=_id_factory("persist", "attempt"),
    )
    ctx = AgentCallbackContext(
        agent=SimpleNamespace(),
        inputs=InvokeInputs(query="q"),
    ...
    assert "secret-should-not-be-returned" not in json.dumps(finish.result)
    manifest = RunManifest.model_validate_json(
        (tmp_path / "run-persist" / "run_manifest.json").read_text(encoding="utf-8")
    )
    assert manifest.status == "blocked"
    assert manifest.tool_receipt_ids == []


@pytest.mark.asyncio
async def test_undigestible_result_force_finishes_at_after_invoke(
    tmp_path: Path,
) -> None:
    """A result the canonicalizer cannot hash still ends in a terminal result."""
    rail = _rail(tmp_path, "bad-output")
    ctx = AgentCallbackContext(
        agent=SimpleNamespace(),
        inputs=InvokeInputs(query="q"),
        extra={},
    ...
    await rail.before_invoke(ctx)
    ctx.inputs = InvokeInputs(query="q", result=_UndigestibleResult())
