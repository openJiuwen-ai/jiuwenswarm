...
The rejection already logs a warning naming the id and the reason, so the loss
is not silent. What that warning does not carry is where the record lives, and
it renders a missing id as an empty string -- and a missing id is itself one of
the rejection reasons, so it is exactly the case where the id cannot identify
the entry. Neither store is at a fixed location either: the file follows the
workspace and the etcd prefix is configurable.

These pin the identifying detail in that warning, and the existing property
that keeps the rejected entry on disk so it stays repairable by hand, which
...
# The rejection is reported by the shared record parser, which both the file
# store and the etcd store call, not by either store module.
_STORE_LOGGER = "jiuwenswarm.runtime.cron.cron_job_mutations"


@contextmanager
def _store_logs(level: int = logging.DEBUG):
    """Collect records from the emitting logger itself, not through ``caplog``.
...
async def test_rejected_entry_warns_and_names_the_job(tmp_path):
    """Two entries on disk, one returned. The warning must name the rejected
    id, the reason, and the store the line is in."""
    path = _write(
    ...
    assert any(
        "job-broken" in message
        and "cron_expr" in message
