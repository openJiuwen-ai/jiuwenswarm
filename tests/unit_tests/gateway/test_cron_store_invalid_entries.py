...
e705f0cf6 moved the store to ``jiuwenswarm.runtime.cron.store`` and made the
rejection log a warning naming the id and the reason, so the loss is no longer
silent. What that warning does not carry is the store path, and it renders a
missing id as an empty string -- and a missing id is itself one of the
rejection reasons, so it is exactly the case where the id cannot identify the
entry. The store is not at a fixed location either; it follows the workspace.
...
_STORE_LOGGER = "jiuwenswarm.runtime.cron.store"
