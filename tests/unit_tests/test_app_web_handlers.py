...
@pytest.mark.asyncio
async def test_config_set_persists_trajectory_ui_without_runtime_reload(monkeypatch):
    channel = FakeWebChannel()
    persisted: list[bool] = []
    reload_options_seen: list[dict] = []
    ...
    assert channel.responses[-1]["payload"] == {
        "updated": ["trajectory_ui_enabled"],
        "applied_without_restart": True,
    }


@pytest.mark.asyncio
async def test_config_save_all_persists_trajectory_ui(monkeypatch):
    channel = FakeWebChannel()
    persisted: list[bool] = []
    monkeypatch.setattr(app_web_handlers, "get_config_raw", lambda: {})
    monkeypatch.setattr(app_web_handlers, "get_config", lambda: {})
    ...
    await channel.methods["config.save_all"](
        object(),
        "req-trajectory-ui-all",
        {"config": {"trajectory_ui_enabled": "true"}},
        "sess-trajectory-ui-all",
    )

    assert persisted == [True]
    assert channel.responses[-1]["ok"] is True
    assert "trajectory_ui_enabled" in channel.responses[-1]["payload"]["updated"]
