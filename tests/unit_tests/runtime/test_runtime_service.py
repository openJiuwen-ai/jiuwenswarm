...
async def test_quiesce_failure_does_not_prevent_runtime_cleanup(error_type) -> None:
    manager = FakeAgentManager()
    runtime = AgentRuntime(agent_manager=manager)
    coordinator_close = AsyncMock(side_effect=error_type("drain failed"))
    runtime._session_coordinator.close = coordinator_close

    await runtime.quiesce_session_executions()
    coordinator_close.side_effect = None
    coordinator_close.return_value = None
    await runtime.close()
...
@pytest.mark.asyncio
async def test_agent_server_quiesce_cancellation_still_closes_runtime(monkeypatch) -> None:
    from jiuwenswarm.server.agent_ws_server import AgentWebSocketServer
    ...
    server = AgentWebSocketServer()
    previous_runtime = server.get_runtime()
    previous_runtime.quiesce_session_executions = AsyncMock(
...
async def test_agent_server_stop_quiesces_sessions_before_kvc_shutdown(
    ...
    async def record_session_quiesce() -> None:
        await record("session-quiesce")

    async def record_runtime_close() -> None:
        await record("runtime-close")

    server = AgentWebSocketServer()
    closing_runtime = server.get_runtime()
    closing_runtime.quiesce_session_executions = AsyncMock(
        side_effect=record_session_quiesce
    )
    closing_runtime.close = AsyncMock(side_effect=record_runtime_close)
    monkeypatch.setattr(
        kv_cache_product_hooks,
    ...
    assert events == (["session-quiesce"] if affinity_enabled else []) + [
        "kvc-task-drain",
        "kvc-runtime-close",
        "runtime-close",
    ]
...
async def test_agent_server_stop_retains_runtime_when_close_is_rejected(
    ...
    previous_runtime.quiesce_session_executions = AsyncMock(
        side_effect=[close_rejected, None]
    )
    ...
    assert previous_runtime.quiesce_session_executions.await_count == 2
