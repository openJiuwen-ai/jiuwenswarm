...
@pytest.mark.parametrize(
    ...
    [
        ...
        (".jsonl", "jsonl[line=0].accuracy"),
    ],
)
def test_non_utf8_artifact_is_a_metric_parse_error(
    tmp_path: Path,
    suffix: str,
    locator: str,
) -> None:
    """Invalid UTF-8 is a traceable parse failure, not an uncaught exception."""
    artifact = tmp_path / f"results{suffix}"
    artifact.write_bytes(b"\xff\xfe")

    with pytest.raises(MetricParseError, match="not valid UTF-8"):
        parse_metric(artifact, locator)
