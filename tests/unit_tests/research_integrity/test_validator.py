...
def test_expected_artifact_uses_exact_path_not_string_suffix(tmp_path: Path) -> None:
    """A similarly suffixed metric source cannot mask a missing artifact."""
    spec = _spec(tmp_path)
    run = _run(tmp_path)
    other_path = tmp_path / "other_results.json"
    other_path.write_text('{"accuracy": 0.75}', encoding="utf-8")
    other_artifact = record_artifact(other_path, run_id=run.run_id)
