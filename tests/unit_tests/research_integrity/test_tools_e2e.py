...
def test_e2e_malformed_command_is_recorded_as_failed_run(tmp_path: Path) -> None:
    """Unbalanced command quoting produces an outcome instead of escaping."""
    spec = ExperimentSpec(
        experiment_id="exp_bad_command",
        name="bad_command",
    ...
    assert outcome.run.exit_code == -1
    assert outcome.run.success is False
    assert outcome.report is not None
    assert outcome.report.passed is False
    assert "FAILED_RUN" in {issue.code for issue in outcome.report.issues}
    assert outcome.run.stderr_path is not None
    assert "COMMAND_PARSE_ERROR" in Path(outcome.run.stderr_path).read_text(
        encoding="utf-8"
    )


def test_e2e_non_utf8_metric_is_recorded_as_parse_failure(tmp_path: Path) -> None:
    """A non-UTF-8 metric artifact fails validation without escaping."""
    writer = tmp_path / "write_invalid_utf8.py"
    writer.write_text(
        "from pathlib import Path\n"
        "Path('results.json').write_bytes(b'\\xff\\xfe')\n",
        encoding="utf-8",
    )
    spec = ExperimentSpec(
        experiment_id="exp_non_utf8",
        name="non_utf8",
    ...
    assert outcome.report is not None
    assert outcome.report.passed is False
    assert "accuracy" in outcome.parse_errors
    assert "not valid UTF-8" in outcome.parse_errors["accuracy"]
    assert "METRIC_PARSE_ERROR" in {issue.code for issue in outcome.report.issues}
