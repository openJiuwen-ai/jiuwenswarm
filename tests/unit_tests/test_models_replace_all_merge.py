...
def test_new_model_without_temperature_omits_sampling_parameter() -> None:
    item = {
        "model_name": "kimi-k3",
...
def test_clearing_existing_temperature_removes_persisted_value() -> None:
    raw = _raw_entry_with_placeholder()
    ...
    merged = _merge_models_for_replace_all(
        [item],
        [raw],
        [resolved],
        _StubCrypto(),
