...
def test_enrich_mounts_runtime_rails_on_named_llm_teammates() -> None:
    ...
    # The reviewer's pre-declared STREAM_EVENT is appended again by the common
    # capability rails, so exactly two mounts (pre-declared + capability set).
    assert reviewer_rails.count(registry.STREAM_EVENT) == 2
