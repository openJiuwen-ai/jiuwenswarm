...
def test_code_subagent_specs_use_factory_names() -> None:
    """Code modes declare only config-gated sub-agents via factory_name."""
    register_swarm_providers()
    config = {
        "react": {
            "subagents": {
    ...
    assert registry.PLAN_AGENT not in factory_names
...
def test_code_member_deep_spec_removes_base_disabled_subagents() -> None:
    """A base team spec cannot reintroduce disabled sub-agents in code mode."""
    ...
    base = DeepAgentSpec(
        subagents=[
            ...
            SubAgentSpec(
                agent_card=AgentCard(name="plan_agent"),
                system_prompt="",
                factory_name=registry.PLAN_AGENT,
            ),
        ]
    )
...
def test_rebuilt_member_spec_keeps_provider_declarations() -> None:
    ...
    assert registry.PLAN_AGENT not in leader_factory_names
