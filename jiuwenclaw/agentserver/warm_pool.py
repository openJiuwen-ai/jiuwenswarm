...
池 key = (channel, mode)。当前部署一个 AgentManager(租户)恒对应一个
路由三元组(group/bot/user), 同池实例的企业配置必然一致; 半成品上的
``_warm_routing_ctx`` 标记 + AgentManager take 后的一行断言仅防御
"多用户共享 workspace"配置形态回归(该特性已废弃)。
...
def build_warm_bootstrap_request(routing: Tuple[str, str, str] | None) -> Any:
    """合成预热用的 bootstrap_request(路由三元组, 供企业配置按租户键加载)."""
        ...
        try:
            while len(self._pool.get(key) or []) < self._min_idle:
                try:
                    ...
                    # CancelledError 为 BaseException 子类, 不会被此处捕获,
                    # 取消信号自然穿透到外层 finally
