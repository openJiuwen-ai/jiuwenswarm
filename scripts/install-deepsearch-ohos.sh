...
#      接受，自动用新机器的签名工具重签（scripts/ohos/ohos_musl_wheel_convert.py）。
...
# 离线 wheelhouse（2026-09 评审后 wheels/ 不再入团队仓库）：
#   - 默认 $REPO_ROOT/wheels（自建 wheelhouse 的构建机放这里即可）
#   - WHEEL_DIR 环境变量可指向任意 wheelhouse 目录
#   - 不存在时：纯 Python 依赖与 native 包走镜像在线安装/现场转换，
#     仅 enterprise_dev 血统的 openjiuwen_deepsearch wheel 需自备
WHEELS=${WHEEL_DIR:-$REPO_ROOT/wheels}
DEPS_DIR="$WHEELS/deepsearch-deps"
MUSL_DIR="$WHEELS/musl-sources"
CONVERTER="$REPO_ROOT/scripts/ohos/ohos_musl_wheel_convert.py"
...
[ -f "$CONVERTER" ] || warn "未找到 scripts/ohos/ohos_musl_wheel_convert.py —— wheel 重签/转换兜底不可用"
...
install_native "numpy"      "import numpy; numpy.array([1,2]).sum()" \
    "numpy-*harmonyos_aarch64.whl" "numpy==2.4.5 cp312" \
...
install_native "Pillow"     "from PIL import Image; Image.new('RGB',(8,8))" \
    "Pillow-*harmonyos_aarch64.whl" "Pillow==12.2.0 cp312" \
