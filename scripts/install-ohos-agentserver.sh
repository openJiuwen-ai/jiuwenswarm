...
#   3. pip install -r requirements-harmony.txt   （phase-1 逐包 + import 验证，含传递依赖）
...
#   7. pip install --no-deps -e .                 （jiuwenswarm 本体）
#
# 用法（鸿蒙 HiShell）:
...
#
#   注意（2026-09 评审）：离线 wheel 不再随仓库分发（wheels/ 已移除）。
#   WHEEL_DIR 缺失时 phase 0 自动改走「镜像在线下载 musllinux 源 wheel +
#   ohos_musl_wheel_convert.py 现场转换（本机签名）」路径；自建 wheelhouse
#   的构建机导出 WHEEL_DIR 指向本地目录即可恢复纯离线行为。
...
#   WHEEL_DIR          离线 wheelhouse（默认 $WHEEL_BUILD_ROOT/wheels；目录缺失时
#                      phase 0 自动改走在线 musl 下载 + 现场转换）
...
#                      =enterprise-dev 时启用离线 wheel openjiuwen_harmonyos-0.1.10
#                      （自备于 WHEEL_DIR，仓库不再分发），该 wheel 为 enterprise-dev
#                      血统，与 dev-stable jiuwenswarm 有 90 项模块缺口（2026-09 真机
#                      实测），仅限回退用途）
...
# ---------- 3. agentcore-minimal（harmonyos/pyproject.toml − requirements-harmony，含传递依赖）----------
