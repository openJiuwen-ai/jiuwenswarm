...
- 纯 Python 依赖从镜像在线装（2026-09 评审后 `wheels/` 离线包不再随仓库分发；
  自建 wheelhouse 的构建机可 `export WHEEL_DIR=...` 恢复离线路径，`wheels/deepsearch-deps/`
  存在时自动优先）；
- `openjiuwen_deepsearch` 本体：dev-stable 血统（默认）走本地源码 / git 克隆；
  enterprise_dev 血统（显式回退）需要自备 wheel 于 `WHEEL_DIR`；
- 原生包（numpy/pypdfium2/Pillow/pandas）三级兜底：
  ① `WHEEL_DIR` 内 `*harmonyos_aarch64.whl` 直接装（自备 wheelhouse 时）；
  ② 装不上（跨机器代码签名不被接受）→ 用**本机** binary-sign-tool 重签后再装；
  ③ 还不行 → 镜像在线下载 musllinux wheel 现场转换
...
**跨机器迁移（git 克隆）**：在新机器上克隆本仓库与 relay-claw
（skill 脚本在里面），先执行 `scripts/install-ohos-agentserver.sh` 建好基础 venv
（native 包自动走在线 musl 下载 + 现场转换，或 `export WHEEL_DIR=...` 用自备
wheelhouse），再跑上面的脚本即可。签名由新机器本地完成，不依赖旧机器的密钥。

> **离线 wheelhouse 说明（2026-09 评审变更）**：原 `wheels/` 目录（67 个 whl，
> 81.6 MB，含走 LFS 的 pandas 双平台 wheel）已从团队仓库移除——离线二进制包
> 不入库。需要纯离线安装的构建机自行维护 wheelhouse 目录并 `export WHEEL_DIR`
> 指向它（供给脚本的钉版表见 `scripts/ohos/ohos-wheel-preload.sh`）；harmonyos
> 预编 wheel 可由 `scripts/ohos/ohos_musl_wheel_convert.py` 从镜像 musllinux
> wheel 批量转换生成。
...
# 0. 纯 Python 依赖（镜像在线装，全部 --no-deps；自建 wheelhouse 时可
#    --no-index --find-links=$WHEEL_DIR/deepsearch-deps 离线装）
pip install --no-deps \
...
# 1. openjiuwen_deepsearch 本体（dev-stable 血统默认走源码；此处示例源码安装）
pip install --no-deps .cache/deepsearch-src

# 2. 原生包：pypdfium2 + pandas（镜像下载 musllinux 源 + 现场转换；
#    自建 wheelhouse 时直接装 $WHEEL_DIR/*harmonyos_aarch64.whl）
pip download --no-deps --only-binary :all: \
    --platform musllinux_1_1_aarch64 --python-version 3.12 -d /tmp/musl \
    pypdfium2==4.30.0
pip download --no-deps --only-binary :all: \
    --platform musllinux_1_2_aarch64 --python-version 3.12 -d /tmp/musl \
    pandas==2.3.1 Pillow==12.2.0
.venv/bin/python scripts/ohos/ohos_musl_wheel_convert.py \
    /tmp/musl/pypdfium2-4.30.0-py3-none-musllinux_1_1_aarch64.whl
.venv/bin/python scripts/ohos/ohos_musl_wheel_convert.py \
    /tmp/musl/pandas-2.3.1-cp312-cp312-musllinux_1_2_aarch64.whl
.venv/bin/python scripts/ohos/ohos_musl_wheel_convert.py \
    /tmp/musl/Pillow-12.2.0-cp312-cp312-musllinux_1_2_aarch64.whl
pip install --no-deps /tmp/musl/*harmonyos_aarch64.whl
