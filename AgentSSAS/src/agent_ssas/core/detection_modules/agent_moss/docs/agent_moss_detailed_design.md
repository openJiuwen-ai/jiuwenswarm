...
- [`../module.yaml`](../module.yaml)：当前模块配置。
- [`../event_adapter.py`](../event_adapter.py)：事件适配实现。
- [`../agent_moss_analyzer.py`](../agent_moss_analyzer.py)：分析器与报告映射。
- [`../engine/policy.py`](../engine/policy.py)：单事件和行为链策略。
- [`../engine/agent_behavior_graph.py`](../engine/agent_behavior_graph.py)：ABG 建模与检查。
- [`../engine/SOURCE.md`](../engine/SOURCE.md)：引擎快照来源。
