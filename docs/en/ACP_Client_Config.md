...
For users who installed JiuwenSwarm via `pip install workswarm`.
...
> Note: `jiuwenswarm-acp` is a command-line entry point auto-generated by pip install, just like `jiuwenswarm-init` and `jiuwenswarm-start`. Make sure VS Code is running in the same virtual environment where workswarm is installed. If not, provide the full path instead, e.g. `C:\path\to\venv\Scripts\jiuwenswarm-acp.exe` on Windows, or `/path/to/venv/bin/jiuwenswarm-acp` on Linux/macOS.