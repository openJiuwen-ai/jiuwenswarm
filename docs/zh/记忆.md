...
| `both` | 内置 + 外接并存                              |
| `none` | 全部关闭，不挂载内置或外接记忆 rail，Agent 不会自动读写或检索记忆 |
...
Query ──► Embed ──► Vector Search ──────────────┐
                                                ├─► Merge ──► Results
Query ──► FTS5 Search （SQLite 的全文检索扩展）────┘
```
...
默认数据根目录为 `~/.jiuwenswarm`；设置 `JIUWENSWARM_DATA_DIR` 后，团队数据存储在该变量指定目录下的 `.agent_teams/`。
...
主配置文件默认位于 `~/.jiuwenswarm/config/config.yaml`。内置记忆使用本地文件系统存储；以下是启用 agent 模式内置记忆的最小配置：
...
如需启用语义检索，可在配置文件的 `embed` 段配置 Embedding 服务，或用环境变量覆盖：
...
未配置 API Key 时，内置记忆会回退到 Mock Provider。它为相同文本生成稳定的伪向量，仅用于保证基础流程可运行，**不能提供可信的语义相似度检索**；此时应以 BM25 全文检索为主。
Embed 参数说明和 Web UI 配置方式请参阅[配置信息](配置信息.md#3-embed-配置)。

### 7.2 外接记忆配置

配置位置：`~/.jiuwenswarm/config/config.yaml` 的 `memory.external` 段。
...
| Provider | 说明 | 必要配置 |
|----------|------|----------|
| `openjiuwen` | 本地长期记忆（KV + 向量 + DB） | 无（默认路径 `~/.jiuwenswarm/memory/ltm`）；配置 Embed 后才可执行向量检索 |
| `mem0` | 云端事实抽取与语义检索 | `api_key`；在 [mem0.ai](https://mem0.ai/) 注册并创建项目后获取 |
| `openviking` | 上下文数据库服务 | 可访问的 `endpoint`；按部署方提供的方式获取 `api_key` |
...
| FTS5  | SQLite 的全文检索扩展 |
| vec0 | SQLite 的向量索引扩展 |
| watchdog| 监听 Markdown 文件变更并触发索引更新的文件监听器 |
