...
Leader 在会话内发起的 **SwarmFlow 后台工作流**同样跟随团队生命周期：暂停 → 工作流一并暂停（保留可续）；切换/新建会话 → 清理可续跑的工作流（进度保留，切回可续）；关窗/断连 → 兜底暂停、保留进度。启停与恢复的完整说明见上文「2.6 SwarmFlow 工作流的启停与恢复」。
