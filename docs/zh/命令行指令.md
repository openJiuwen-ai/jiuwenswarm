...
## 离线技能训练：`jiuwenswarm skill-train`

把 JiuwenSwarm coding agent 当作 **被评测的 target**，在 SearchQA / DocVQA / OfficeQA 基准上跑
ReflACT 离线技能优化
训练逻辑由 `openjiuwen.agent_evolving.skill_train` 提供（`run_offline_training`），本命令只负责
参数解析、Gateway 预检和启动。

### 工作方式

1. 每条样本在 `outputs/.../predictions/<id>/jiuwenswarm_exec/` 生成隔离工作区：
   `.agents/skills/reflact-target/SKILL.md`（当前待优化 skill）、`task.md`（题目 + 上下文）、
   可选 `attachments/` + `ATTACHMENTS.md`（DocVQA 图片）或 `docs/`（OfficeQA 语料软链）。
2. 以子进程运行 `jiuwenswarm chat --cwd <ws> --project-dir <ws> --mode code.normal --jsonl -- <prompt>`，
   从 JSONL 事件流提取 `chat.final` 作为回答并评分。
3. `chat.reasoning` / `chat.tool_call` / `chat.tool_result` 被压缩成
   `predictions/<id>/jiuwenswarm_trace_steps.txt`，reflect 阶段作为 `JiuwenSwarm Trace Steps` 附件
   喂给 analyst（chat 模型），用于生成 skill patch。
4. rollout → reflect → patch → gate 循环若干 epoch，输出 `best_skill.md`。

### 快速开始

```bash
# 终端 A：启动 Gateway 和 AgentServer
jiuwenswarm-start app

# 终端 B：训练（optimizer 的 API_KEY / API_BASE / OPTIMIZER_MODEL 从 .env 读取）
jiuwenswarm skill-train --env searchqa --data-root ./data

# 小规模调试
jiuwenswarm skill-train --env docvqa --limit 10 --num-epochs 1 --workers 2

# 只评估一个 skill，不训练
jiuwenswarm skill-train --env searchqa --eval-only --split test --limit 5 \
  --skill-init path/to/SKILL.md
```

等价的 agent-core 仓库入口：`TARGET_BACKEND=jiuwenswarm_cli_exec uv run python examples/agent_evolving/skill_reflact_train_example.py --env searchqa`。

### 选项

| 选项 | 默认值 | 说明 |
|---|---|---|
| `--env <name>` | `searchqa` | `searchqa` / `docvqa` / `officeqa` |
| `--backend <name>` | `jiuwenswarm_cli_exec` | target 后端；`openai_chat` 退回聊天模型（需 `TARGET_MODEL`） |
| `--data-root <path>` | `SKILL_TRAIN_DATA_ROOT` | 存放 `*_id_split` 与 DocVQA 图片 / OfficeQA 文档的数据根目录 |
| `--split-dir <path>` | 预设 | 显式指定 id-split 目录 |
| `--skill-init <path>` | 各 env 的 `skills/initial.md` | 初始 skill |
| `--output-dir <path>` | `./outputs/skill_train_<env>_<backend>_<时间>` | 输出目录（可断点续跑） |
| `--optimizer-model` / `--api-key` / `--api-base` | 环境变量 | analyst / optimizer 聊天模型凭据 |
| `--num-epochs` / `--train-size` / `--batch-size` / `--seed` | 预设 | 训练规模 |
| `--limit <n>` | — | 限制每次 **train rollout** 样本数（调试） |
| `--selection-eval-size <n>` | `40` | selection / gate 在 `valid_seen` 上的评测条数（`0`=全量） |
| `--workers <n>` | `4`（exec 后端上限） | 并行 `jiuwenswarm chat` 进程数 |
| `--exec-timeout <s>` | `120`（officeqa 预设 `600`） | 单次 `jiuwenswarm chat` 超时 |
| `--max-turns <n>` | 预设 | 无 `<answer>` 标签时最多重跑次数 |
| `--no-gate` | — | 关闭接受门禁 |
| `--gateway-url <url>` | `ws://127.0.0.1:19001/tui` | Gateway 地址 |
| `--name <instance>` | — | 多实例隔离，透传给 `chat --name` |
| `--mode <mode>` | `code.normal` | target 的 `chat --mode` |
| `--cli-path <path>` | 当前 venv 的 `jiuwenswarm` | 被调用的 CLI 可执行文件 |
| `--no-trace-to-optimizer` | — | 不把事件轨迹喂给 analyst |
| `--skip-gateway-check` | — | 跳过启动前的 Gateway 探测 |
| `--eval-only` / `--split` | — / `test` | 只 rollout 一次并写 `eval_summary.json` |
...
| 2 | 参数 / 数据路径 / 凭据错误 |
| 3 | Gateway 不可达 |
| 4 | 缺少 `openjiuwen.agent_evolving.skill_train`（需安装 agent-core） |
| 5 | 训练过程异常 |
| 130 | 用户中断（输出目录可续跑） |

> 提示：训练期间每个 worker 都是独立的 `jiuwenswarm chat` 会话（独立 session_id），
> 不要同时在同一 Gateway 上开 TUI 做交互，以免混淆产物。`jiuwenswarm chat` 退出码 3 时整批快速失败。
