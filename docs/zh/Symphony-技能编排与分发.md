...
技能图谱展示的是静态构建的“可衔接候选关系”，不保证所有场景都一定能直接串联。实际使用时仍需结合技能详情、输入参数和任务目标确认。

能力扫描、指纹抽取、归一化和质量评估由 agent-core 提供，JiuwenSwarm
只保留应用 Adapter 与图谱发布流程。图谱构建会发布统一的
...
`symphony_compose_graph` 的核心参数是 `query`，即原始用户任务；当前编排模式为 `fast`。如果已经通过技能检索缩小了候选范围，可以把 `skill_branch_explore` 返回的 `worker_id` 列表传入 `candidate_skill_ids`，让 Symphony 在这些候选技能及其可衔接邻居中生成技能链。若编排结果提示缺少合适技能，需要先通过 **技能** 页面搜索并安装新技能，再调用 `symphony_refresh_graph` 刷新图谱，最后用原始任务重新编排。
...
| **读取图谱** | 重新读取当前已经构建好的技能图谱 |
| **增量构建** | 在新增、删除或修改技能后，优先使用增量构建更新图谱 |
| **取消构建** | 图谱构建耗时较长时可取消，已完成的缓存和 checkpoint 会保留 |
| **全量重新构建** | 当图谱异常、关系明显过旧或需要彻底重算时使用 |
...
1. 打开左侧导航栏 → **技能** → **技能图谱**。
2. 在页面顶部开启 **启用技能编排**；开关点击后会自动保存并热更新。
3. 确认任务需要的技能已经安装；如果刚新增、删除或修改过技能，建议先到 **技能** → **技能图谱** 中执行一次 **增量构建**。如果只是确认当前图谱状态，可以先 **读取图谱**。
...
当前 Web 界面中，**启用技能编排** 开关位于技能图谱页，用于控制技能图谱与编排工具；配置页中的 **启用技能检索** 开关单独控制技能树检索工具。技能索引页提供索引构建、重新构建、取消构建、查看构建状态和查看索引树等操作；技能图谱页提供读取图谱、增量构建、取消构建和全量重新构建等操作。
...
开启后，新会话会注册 `symphony_read_graph`、`symphony_refresh_graph`、`symphony_compose_graph` 等编排工具。智能体可以读取或刷新技能图谱，并在已有候选技能基础上生成技能链。关闭后不注册这些工具，智能体不会使用技能图谱进行编排。
...
技能来源目录和技能图谱产物目录。默认模板值都是空字符串，表示使用运行时默认目录。

`symphony_refresh_graph` 会从 `skills_root` 读取技能并刷新技能图谱；`symphony_read_graph` 和 `symphony_compose_graph` 会读取 `graph_dir` 下的图谱产物。需要把图谱缓存到固定位置、或者在不同运行环境之间复用图谱时，可以显式配置这些目录。
...
| `min_edge_confidence` | `0.5` | 技能图谱边的最低置信度阈值，低于该值的连接关系不会优先用于编排 |
