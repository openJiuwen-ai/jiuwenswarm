...
> 如果你是研发同学，建议先阅读[新同学架构导览：Agent、Skill 与 Symphony](新同学架构导览-Agent-Skill与Symphony.md)，了解 Symphony 与 agent-core、JiuwenSwarm、SkillHub 和实际 Skill 执行链路的边界。
...
- **技能编排**解决“怎么用”：基于候选技能、输入输出结构和技能依赖关系，生成一条可解释、可执行的技能链。
...
技能编排会围绕任务目标、已有输入和候选技能，参考技能图谱里的可衔接关系生成一条可执行路线。遇到缺失输入时，系统可以沿关系图反向查找能够补上该输入的上游技能；规划状态为 `ready` 时，Agent 会逐个读取并执行当前可运行的 Skill，不需要用户先确认整张路线。

技能编排的重点不是把技能简单排成列表，而是判断上游结果是否真的能喂给下游技能。可衔接关系、输入输出结构、任务语义都会影响最终编排。这样生成的不是“看起来相关”的技能组合，而是依赖关系可解释、可核验、可执行的技能链。

技能图谱是编排使用的关系图。图中的每个节点代表一个技能，边代表一个技能的输出可以作为另一个技能的输入，也就是可衔接（`can_feed`）关系。它既可以帮助用户理解技能之间如何组合，也会为技能编排提供可参考的候选关系。

![技能图谱](../assets/images/symphony_graph.png)

技能图谱展示的是“可衔接候选关系”，不是已经执行过的任务记录，也不保证所有场景都一定能直接串联。实际使用时仍需结合技能详情、输入参数和任务目标核对。

#### 根据实际使用效果持续优化编排

开启 `symphony.evolution.enabled`（设置页中的 **发现技能包**）后，Symphony 会参考带版本快照的真实执行证据，调整后续相关任务的技能组合建议。关闭该配置后，编排会恢复为只使用静态图谱，已积累的数据不会删除；已经创建动态图谱 Service 的长运行进程可能需要有序重启，才能完整应用配置变化。静态相关性、动态执行证据和最终任务成功率是三类不同信息，不能互相替代。

#### 它解决什么问题

- **技能太多时怎么选**：避免把所有技能说明一次性塞进上下文，让智能体按任务需要逐步找到候选技能。
- **技能选对后怎么用**：不只判断技能是否相关，还判断上游输出能否真正接到下游输入。
- **多技能流程怎么稳定串起来**：基于技能图谱里的可衔接关系生成可解释、可执行的技能链，减少临场拼接导致的中断。
...
用户发起任务
  ↓
智能体通过 skill_index 浏览目录或搜索
  ↓
发现候选技能
  ↓
技能编排基于任务目标、候选技能和技能图谱生成技能链
  ↓
必要时补充输入或完成 Tool 权限审批
  ↓
继续执行具体技能
  ↓
...
技能检索主要负责“找技能”；技能编排负责把候选技能组织成一条面向任务的执行路线；技能图谱负责展示和沉淀“技能之间能否衔接”，为编排提供关系图依据。技能检索和技能编排构成 Symphony（技能交响乐）的两类核心能力。Symphony 负责给出候选和计划，Skill 的实际读取、Tool 调用和副作用仍由 Agent/Harness 执行。
...
| 工具 | 作用 | 使用时机 |
|------|------|----------|
| `skill_index` | 对只读 Skill 目录执行 `list` 或 `search`，返回分类、候选 Skill 和精确 Skill ID | 需要从较多已安装 Skill 中缩小候选范围时 |

`skill_index` 的分类是虚拟目录，不是磁盘路径。`list` 用于浏览某一分类的直接子分类和 Skill，`search` 用于按名称、别名、描述和 `SKILL.md` 内容检索。搜索结果只是相关候选，不代表已经验证其可执行性；如果后续需要编排，应把收敛后的精确 Skill ID 传给 `symphony_compose_graph` 的 `candidate_skill_ids`。索引构建和取消由技能页面或对应 RPC 管理，不是模型侧检索 Tool 的操作。
...
| `symphony_refresh_graph` | 抽取已安装 Skill 特征并刷新技能图谱 | 图谱缺失、过期，或安装新 Skill 后需要更新关系图时使用 |
| `symphony_compose_graph` | 基于任务目标、候选 Skill 和技能图谱生成执行图 | 任务需要多个 Skill 或有顺序依赖时使用 |

`symphony_compose_graph` 的核心参数是原始用户任务 `query`，可以选择 `fast` 或 `beam` 模式。已经通过检索缩小范围时，应一次性传入收敛后的 `candidate_skill_ids`，不要传入整个 Skill 目录。如果计划提示缺少输入，应先补充输入；如果没有合适 Skill，则先安装 Skill、刷新图谱，再用原始任务重新编排。
...
启用技能检索后，候选较少时模型可以直接从当前会话冻结的完整精简摘要中选择；候选较多时通常通过 `skill_index` 浏览或搜索。索引暂不可用时运行时会保留平铺候选回退，不会让模型自行构建索引。找到 PDF、Excel、文档审阅、报告生成等候选后，模型再决定按需读取哪些 `SKILL.md`。

### 6. 查看检索过程

在聊天消息中查看 `skill_index` Tool 调用，可以看到：

- 模型列出了哪些分类；
- 使用了哪些检索词和分类范围；
- 是否通过分页 cursor 继续读取；
- 最终出现了哪些候选 Skill 及其精确 ID。
...
- 描述完整目标，而不是只写单个步骤。
- 如果只想查看方案、不希望执行，应在原始请求中明确写“只生成编排方案，不执行”。

#### 看懂编排与执行过程

开启后，界面会展示技能编排图和简短说明。图中的每个方块是一个 Skill，箭头表示执行顺序和结果传递关系。规划状态为 `ready` 时，Agent 不会等待用户确认整张图，而会按图逐个读取和执行当前可运行的 Skill；只有缺少输入、触发 Tool 权限审批或遇到其他运行时约束时才会中断。下面示例中的路线是 `image-translate → yescan-ocr-universal → general-writing → imap-smtp-email`。



根据运行状态处理即可：

- **`ready`**：通常不需要额外回复，Agent 会继续执行。
- **`needs_input`**：补充系统提示中缺少的文件、链接、邮箱、账号或参数。
- **Tool 权限审批**：根据具体操作选择允许或拒绝；这是副作用审批，不是对整张编排图的确认。
- **只想看方案**：在原始请求中明确限制为只规划、不执行。
...
开启后，当前 Agent runtime 会同步 `symphony_read_graph`、`symphony_refresh_graph`、`symphony_compose_graph` 等编排工具。智能体可以读取或刷新技能图谱，并在已有候选 Skill 基础上生成技能链。关闭后会移除这些工具，智能体不再使用技能图谱进行编排。
...
| 配置项 | 默认值 | 说明 |
|--------|--------|------|
| `mode` | `fast` | 默认编排模式；`fast` 适合候选清晰的短链，`beam` 适合多路径和需要反向补前置能力的复杂任务 |
| `top_k` | `3` | Beam 搜索每轮保留的候选路线数量上限 |
| `max_depth` | `4` | 技能链最大搜索深度，用于限制一次任务中串联技能的长度 |
| `min_edge_confidence` | `0.5` | 技能图谱边的最低关联强度阈值，低于该值的连接关系不会进入静态编排候选边集合 |
...
开启后，系统会注册 `skill_index`；新会话再根据创建时冻结的 Skill 候选规模选择完整精简摘要或渐进检索策略，较大的候选集合主要通过 `skill_index` 的 `list` / `search` 操作按需披露。关闭后不注册检索工具，不注入技能目录提示，回到原有 Skill 流程。

适用于需要在大量已安装技能中，根据任务查找相关技能的场景。如果技能数量很少，或者希望完全使用原来的 `list_skill` 流程，可以关闭。
...
构建由技能页面或对应的产品 RPC 发起，不再由模型调用 `skill_index_build`。构建完成后，新会话会固定使用当时可用的索引版本；已有会话不会无条件切换到新的目录快照。
...
- [技能系统](技能.md)
- [新同学架构导览：Agent、Skill 与 Symphony](新同学架构导览-Agent-Skill与Symphony.md)
- [配置说明](配置信息.md)
