...
> 原「企业技能」接口已并入 **§13 技能**（与单机 `skills.*` 同章）：`skills.enterprise.install` / `source.*` 见 **13.6–13.8**；已装列表见 **13.1 `skills.list`**；卸载见 **13.5 `skills.uninstall`**（原 `skills.enterprise.uninstall` 已并入）。
...
**含义**：发现型只读列表（单机与企业共用，**无 `list_source`**）。Agent 扫描本地 skills 目录、内置 skills 目录与 marketplace 本地缓存，返回当前**可浏览的可用技能**（含尚未安装的内置/marketplace 项）；**不是**「仅返回已安装技能」。可选 `with_installed: true` 时同响应附带 `plugins`（已装 marketplace 插件，与 `skills.installed` 一致）。本接口只列目录元数据，不把技能加载进当前会话执行面；是否可被对话使用还取决于是否已装入本地目录及 `enabled`（见 `skills.toggle`）。
...
| key                  | 必填  | 类型     | 默认值   | 版次   | 说明              |
| -------------------- | --- | ------ | ----- | ---- | --------------- |
| refresh_marketplaces | 否   | bool   | false | 单机   | 先刷新 marketplace |
| with_installed       | 否   | bool   | false | 单机   | 同时返回 `plugins`  |
| session_id           | 否   | string | —     | 共用   | 会话路由（前端常传） |
| metadata             | 否   | object | —     | 企业新增 | 可选身份/路由扩展；约定 key 见下表 |


**`metadata` 约定字段（均可选；均为企业新增）**
...
| key     | 必填  | 类型    | 说明                                                          |
| ------- | --- | ----- | ----------------------------------------------------------- |
| skills  | 是   | array | 可用技能列表；元素见下表                                                |
| plugins | 否   | array | 仅 `with_installed=true` 时返回；已装 marketplace 插件，元素见 **13.24** |
...
| tags                | 否   | string[] | 标签；缺省 `[]`                                                       |
| allowed_tools       | 否   | string[] | 允许工具；缺省 `[]`                                                     |
| path                | 否   | string   | SKILL.md 路径                                                      |
| source              | 是   | string   | 来源：`local` / `project` / `builtin` / marketplace 名 / `clawhub` 等 |
| installed           | 否   | bool     | 是否已装入用户本地 skills 目录；本地扫描为 `true`，未装内置/marketplace 预览为 `false`    |
| enabled             | 否   | bool     | 是否启用；见 `skills.toggle`                                           |
| is_builtin          | 否   | bool     | 是否按内置技能处理（如不可删）                                                  |
| is_builtin_source   | 否   | bool     | 内置源目录中是否存在同名技能                                                   |
| has_evolutions      | 否   | bool     | 本地目录是否存在 `evolutions.json`                                       |
| marketplace         | 否   | string   | marketplace 预览项上的源名称                                             |
| display_name        | 否   | string   | 展示名（安装来源落盘，如 ClawHub 原始大小写）                                      |
| origin              | 否   | string   | 安装身份标识，如 `clawhub:owner/slug`、SkillNet URL；重名时用于精确定位             |
| market_short_desc   | 否   | string   | TeamSkillsHub 等市场卡片短描述（有则优先于 `description` 展示）                   |
| market_display_name | 否   | string   | TeamSkillsHub 等市场展示名                                             |
| config              | 否   | object   | 部分扫描路径会带 `{ "enabled": bool }`                                   |


##### 13.2 `skills.get`

**含义**：按名称取单个技能详情。优先查用户本地 skills 目录；未命中再查 marketplace 本地缓存。重名时可用 `origin` 精确定位。相对 `skills.list`，详情会返回正文 `content`（由 `body` 转换）与 `file_path`（由 `path` 转换）。
...
| ------ | --- | ------ | --- | --------------------------------- |
| name   | 是   | string | —   | 技能名                               |
| origin | 否   | string | —   | 安装身份标识；重名时按 origin 精确定位，避免返回同名另一条 |


**出参**（`payload`，`ok=true`）：技能详情对象（字段与 **13.1** `skills` 元素大体相同，差异如下）


| key                 | 必填  | 类型       | 说明                                                               |
| ------------------- | --- | -------- | ---------------------------------------------------------------- |
| name                | 是   | string   | 技能名                                                              |
| content             | 是   | string   | SKILL.md 正文（list 不返回；由内部 `body` 转换）                              |
| file_path           | 否   | string   | SKILL.md 路径（list 中为 `path`；get 转为 `file_path`）                   |
...
| source              | 是   | string   | 来源：`local` / `project` / `builtin` / marketplace 名 / `clawhub` 等 |
| enabled             | 否   | bool     | 是否启用                                                             |
| is_builtin          | 否   | bool     | 是否按内置技能处理                                                        |
| is_builtin_source   | 否   | bool     | 内置源目录中是否存在同名技能                                                   |
| has_evolutions      | 否   | bool     | 是否存在 `evolutions.json`                                           |
| marketplace         | 否   | string   | marketplace 预览命中时的源名称                                            |
| display_name        | 否   | string   | 展示名                                                              |
| origin              | 否   | string   | 安装身份标识                                                           |
| market_short_desc   | 否   | string   | 市场卡片短描述                                                          |
| market_display_name | 否   | string   | 市场展示名                                                            |
| config              | 否   | object   | `{ "enabled": bool }`                                            |


未找到技能时请求失败（如 `ValueError: 未找到 skill: ...`）。

##### 13.3 `skills.install`

**含义**：从 marketplace 或内置源安装技能到用户本地 skills 目录，并写入 `installed_plugins` 状态。
...
| key   | 必填  | 类型     | 默认值   | 说明                                               |
| ----- | --- | ------ | ----- | ------------------------------------------------ |
| spec  | 是   | string | —     | `plugin@marketplace`；裸技能名或 `name@builtin` 可装内置技能 |
| force | 否   | bool   | false | 目标目录已存在时是否覆盖重装                                   |
...
| key        | 必填  | 类型     | 说明                                                                                                           |
| ---------- | --- | ------ | ------------------------------------------------------------------------------------------------------------ |
| success    | 是   | bool   | 是否安装成功                                                                                                       |
| detail     | 否   | string | 失败原因（如已存在、marketplace 不存在、git clone 失败等）                                                                     |
| detail_key | 否   | string | 失败时国际化 key，如 `skills.marketplace.errors.skillAlreadyInstalled`、`skills.builtin.errors.skillAlreadyInstalled` |


成功时通常仅返回 `{ "success": true }`。
...
| key        | 必填  | 类型     | 默认值 | 版次   | 说明                                                          |
| ---------- | --- | ------ | --- | ---- | ----------------------------------------------------------- |
| name       | 是   | string | —   | 共用   | 技能名                                                         |
| origin     | 否   | string | —   | 单机   | 安装身份标识（如 `clawhub:owner/slug`、URL）；重名时按 origin 精确定位，避免误删另一条 |
| session_id | 否   | string | —   | 共用   | 会话路由（前端常传）                                                  |
| metadata   | 否   | object | —   | 企业新增 | 可选身份/路由扩展；约定 key 见下表（企业路径常用；亦可由连接上下文补全）                    |
...
| key     | 必填  | 类型     | 说明                         |
| ------- | --- | ------ | -------------------------- |
| success | 是   | bool   | 是否卸载成功                     |
| detail  | 否   | string | 失败原因（如未找到、内置技能不允许删除、参数非法等） |


成功时通常仅返回 `{ "success": true }`（单机与企业路径出参一致）。

##### 13.6 `skills.enterprise.install`

企业版：按 URL 安装技能包（zip）到当前租户；写盘并写入 `installed_skill`。多第三方源共用本接口，用 `skill_source` 区分来源（不为每源新增 method）。

**含义**：下载 `url` 指向的技能包 → 可选验签/校验（见 `metadata`）→ 安装到租户 skills 目录 → 账本记录 `skill_source`。客户端典型路径：先 `skills.enterprise.source.search` 取得 `download_url`，再调用本接口；安装后用 **`skills.list`** 查看已装列表。
...
| key          | 必填  | 类型     | 说明 |
| ------------ | --- | ------ | --- |
| url          | 是   | string | 技能包下载 URL（http/s zip） |
| skill_source | 是   | string | 账本来源标识，如 `web:` / `vendor_a:` / `vendor_a:{asset_id}` |
| service_id   | 是   | string | 租户 ID |
| agent_id     | 是   | string | Agent ID |
| metadata     | 否   | object | 可选扩展字段；平台识别的约定 key 见下表，其余可透传/落库 |
...
| key             | 类型     | 说明 |
| --------------- | ------ | --- |
| signature       | string | HMAC 验签；企业可配置强制 |
| checksum_sha256 | string | zip 内容校验 |
| display_name    | string | 展示名 |
| short_desc      | string | 短描述 |
| force           | bool   | 已存在时是否按企业规则覆盖；默认 `false` |
| group_id        | string | 群组 |
| bot_id          | string | 机器人 |
| user_id         | string | 用户 |
| session_id      | string | 会话路由 |
...
```json
{
  "url": "https://vendor-a.example/skills/weather.zip",
  "skill_source": "vendor_a:weather",
  "service_id": "svc_1",
  "agent_id": "agent_1",
  "metadata": {
    "checksum_sha256": "...",
...
##### 13.7 `skills.enterprise.source.providers`

列出已注册的可插拔技能源（客户 Provider）。用于前端动态展示多源入口。

**含义**：只返回平台注册表中的源元数据，不访问第三方目录。客户**不实现**本 RPC，而是实现并注册技能源 Provider（SPI）；设计见 [技能源SPI接口设计.md](./技能源SPI接口设计.md)。
...
| key        | 必填  | 类型     | 说明 |
| ---------- | --- | ------ | --- |
| service_id | 否   | string | 租户；可与连接上下文合并 |
| agent_id   | 否   | string | |
| group_id   | 否   | string | |
| bot_id     | 否   | string | |
| user_id    | 否   | string | |
...
| key          | 必填  | 类型     | 说明 |
| ------------ | --- | ------ | --- |
| source       | 是   | string | Provider `source_id`，供 search 的 `source` |
| display_name | 否   | string | 展示名 |



##### 13.8 `skills.enterprise.source.search`

按 `source` 调度客户实现的技能源 Provider，返回可安装技能列表（含 zip `download_url`）。

**含义**：平台不解析第三方私有协议；`filter` 原样透传给 Provider。安装请再调 **13.6**，传入条目中的 `download_url` → `url`、`skill_source` 等；已装列表用 **13.1 `skills.list`**。
...
| key        | 必填  | 类型     | 默认值 | 说明 |
| ---------- | --- | ------ | --- | --- |
| source     | 是   | string | —   | 与 providers 中的 `source` 对应 |
| q          | 否   | string | —   | 关键词；空则由 Provider 决定默认列表 |
| page       | 否   | int    | 1   | 页码 |
| page_size  | 否   | int    | 20  | 每页条数，建议上限 100 |
| filter     | 否   | object | —   | Provider 扩展过滤；平台不解释 |
| session_id | 否   | string | —   | 会话路由（前端常传） |
| metadata   | 否   | object | —   | 可选身份/租户扩展；约定 key 见下表 |
...
> 平台将 `metadata`（及顶层 `session_id`）合并注入 Provider 的 `context`，供第三方鉴权/路由。
...
| success       | 是   | bool   | |
| source        | 是   | string | 回显 |
| skills        | 是   | array  | 可安装条目；元素见下表 |
| count         | 否   | int    | 本页条数 |
| error_code    | 否   | string | 如 `unsupported_source` |
| error_message | 否   | string | |


**`skills` 数组元素（InstallableSkill）**


| key             | 必填  | 类型     | 说明 |
| --------------- | --- | ------ | --- |
| download_url    | 是   | string | zip 下载地址 → install 的 `url` |
| skill_source    | 是   | string | 建议 `{source}:` 或 `{source}:{asset_id}` |
| name            | 否   | string | 技能名 |
| display_name    | 否   | string | 展示名 |
| summary         | 否   | string | 短描述（可与 short_desc 同义） |
| short_desc      | 否   | string | 短描述 |
| version         | 否   | string | |
| checksum_sha256 | 否   | string | 有则放入 install `metadata` |
| signature       | 否   | string | 有则放入 install `metadata` |
| asset_id        | 否   | string | 源侧主键 |


**调用示例**

```json
// 1) 搜索（skills.enterprise.source.search）
{
  "source": "vendor_a",
  "q": "天气",
  "page": 1,
...
// 2) 安装（skills.enterprise.install，取搜索结果字段）
{
  "url": "https://vendor-a.example/skills/weather.zip",
  "skill_source": "vendor_a:weather",
  "service_id": "svc_1",
...
##### 13.10 `skills.marketplace.list`

**入参**：无

**出参**：`marketplaces`（array）

##### 13.11 `skills.marketplace.add`
...
##### 13.12 `skills.marketplace.remove`
...
##### 13.13 `skills.marketplace.toggle`
...
**出参**：`success`、`pending`、`install_id`；完成后轮询 **13.16**

##### 13.16 `skills.skillnet.install_status`
...
##### 13.17 `skills.skillnet.evaluate`
...
##### 13.18 `skills.clawhub.get_token`

**入参**：无

**出参**：`success`、`token`（掩码）、`has_token`

##### 13.19 `skills.clawhub.set_token`
