...
| 用户 | `config.user.yaml` | 桌面 xiaoyi/Celia/sandbox；`patch_user_config`；dump 默认 | **不覆盖** |
...
```mermaid
flowchart TD
  Desk[桌面 prepare / ensureConfigFile] --> Ov["xiaoyi / Celia 写 overlay"]
...
| API | 落到哪 | 用途 |
|---|---|---|
| `dump_yaml_round_trip(CONFIG_YAML_PATH)` / `update_config` / `load_yaml_round_trip` 默认 | 有 overlay 或还没有系统 yaml → **overlay** | xiaoyi、Celia、`sandbox.enabled`、白名单其它项 |
...
小艺桌面：`configWriteFile()` 同 overlay 规则（xiaoyi / Celia / sandbox）；`applyPermissionProfile` 写 `configFile()`（系统 yaml）。
...
| 桌面 `src/core/runtime/jiuwen-config.ts` | xiaoyi / Celia / 权限档 YAML 补丁 |
