...
| macOS | macOS 11 (Big Sur) 或更高 | 见下方「架构说明」 | 首次启动需在 Finder 右键「打开」以放行 GateKeeper | CPU 2 核、内存 4 GB、可用磁盘 2 GB |
...
当前 macOS 桌面包为**单架构产物**，不是 Universal 二进制：

- 在 **Apple Silicon（M 系列）** 构建机产出的 dmg 仅含 `arm64` 原生二进制与 Node 运行时。
- 在 **Intel** 构建机产出的 dmg 仅含 `x64` 原生二进制与 Node 运行时。

因此请在 Release 页面根据你的 Mac 芯片类型选择对应产物（如 `JiuwenSwarm-<version>-arm64.dmg` 或 `JiuwenSwarm-<version>-x64.dmg`）。**下载错误架构的 dmg 将无法正常启动内置 Node 运行时**，导致 Web 前端无法构建/加载。如不确定芯片类型，可在终端执行 `uname -m`：输出 `arm64` 为 Apple Silicon，输出 `x86_64` 为 Intel。
...
| 依赖项 | 版本要求 | 适用方式 | 说明 |
|--------|----------|----------|------|
| Python | ≥3.11, <3.14 | pip 安装、源码安装 | 推荐使用 Python 3.11 |
...
python --version
# 预期输出：Python 3.11.x、3.12.x 或 3.13.x
...
见上方 [macOS：用 curl 下载 dmg（推荐）](#1-macos用-curl-下载-dmg推荐)。浏览器下载的 dmg 带隔离标签，推荐用终端 `curl` 下载。另外请确认下载的是与你 Mac 芯片匹配的架构版本，见 [架构说明（macOS）](#架构说明macos)。

### Q: 源码安装时提示 "Node.js not found" 或找不到 `npm`

请安装 Node.js 18.x 或更高版本，然后重新构建前端。桌面安装包和 pip 包已包含前端静态资源，正常安装和启动不需要 Node.js。版本检查详见：[环境检查](#环境检查)
