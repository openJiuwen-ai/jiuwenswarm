...
| 接口 | 说明 |
| --- | --- |
| `POST /api/v1/auth/authorize` | `{authorizeUrl, state, claimToken, expiresIn}`；既没有交换服务也没有 secret 时 503。发起 / 认领 / 登出都要带 `X-Jiuwen-Auth: 1`，否则 403 |
