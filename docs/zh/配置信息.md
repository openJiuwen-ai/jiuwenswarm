...
| `model_provider` | `client_provider`     | 模型协议/供应商类型，决定请求格式与鉴权方式    | 支持 `OpenAI`、`DeepSeek`、`DashScope`、`SiliconFlow`、`AscendAffinity`、`OpenRouter`、`OpenAIAccount`，用于适配不同服务商的 API 格式；视频/音频/视觉模型当前仅支持 `OpenAI`；图片生成模型使用 `OpenAI` 并需要 `endpoint_profile: dashscope`（仅支持 DashScope 托管的生图模型） |
