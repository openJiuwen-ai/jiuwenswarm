...
| `skills.external_dirs` | 外部技能目录，该目录下的技能与通过 UI 安装的技能完全等价。支持 YAML 列表或分号分隔字符串（也可通过 `EXTERNAL_SKILL_DIRS` 环境变量设置） | `""`（空） |
| `skills.external_only` | 为 `true` 且 `skills.external_dirs` 非空时，排除个人技能目录，仅暴露外部目录中的技能。用于基准测试/CI 环境，防止无关的已安装技能干扰 | `false` |
