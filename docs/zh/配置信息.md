...
> 💡 **提示**：默认模型是系统运行的基础，必须正确配置。视频视觉、音频、视觉、图片生成、视频生成模型为可选配置，当您需要相应多模态能力时再配置即可。图片生成与视频生成均可在「设置 → Agent → 工具」中配置。
...
| `model_provider` | `client_provider`     | 模型协议/供应商类型，决定请求格式与鉴权方式    | 支持 `OpenAI`、`DeepSeek`、`DashScope`、`SiliconFlow`、`AscendAffinity`、`OpenRouter`、`OpenAIAccount`，用于适配不同服务商的 API 格式；视频/音频/视觉模型当前仅支持 `OpenAI`，视频生成模型当前仅支持 `DashScope` |
