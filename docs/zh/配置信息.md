...
| 开关 | 对应配置 | 默认值 | 作用 |
| --- | --- | --- | --- |
| **启用 Skills 自演进**（总开关） | `react.evolution.enabled` | `true` | 自演进功能总开关。开启并挂载演进 Rail 后，被动信号扫描始终可用；关闭后整条自演进链路（含 `/evolve` 手动指令）均不可用 |
| **软触发 Review** | `react.evolution.review_trigger` | `false` | 开启后周期性入队 review 自检 follow-up。环境变量 `EVOLUTION_REVIEW_TRIGGER` 优先 |
| **自动保存审批** | `react.evolution.auto_save` | `true` | 开启后演进审批无需用户点击即可自动通过；经验落盘后会自动版本合并 |
| **自动建议创建新技能** | `react.evolution.skill_create` | `false` | 开启后，系统可以在缺少合适 Skill 时提出自动创建建议。环境变量 `SKILL_CREATE` 优先 |

> 💡 **说明**：`react.evolution.enabled` 是 Skills 正文自演进的产品总开关（默认开启）。挂载演进 Rail 后被动扫描始终开启，不再提供独立的 `auto_scan` / `signal_trigger` 子开关。`review_trigger` / `auto_save` / `skill_create` 为可选子开关；当 `skill_create` 关闭且未开启 `review_trigger` 时，系统仍会做被动扫描，并响应手动 `/evolve` 指令。执行轨迹固定落盘到默认目录 `~/.jiuwenswarm/agent/evolution_trajectories`，不再通过配置项或环境变量覆盖。

> 📖 **详情**：关于 Skill 自演进机制的详细说明，请参考 [Skill 自演进](Skill自演进.md)。

---

## TTSE 配置

TTSE（`react.ttse.enabled`）与 Skills 正文演进相互独立，**默认开启**。挂载 TTSERail、后台归纳 FACT/TIP，以及把 `ttse_consult` 放入首轮 schema。设 `enabled: false` 可关闭。

> 📖 **详情**：请参考 [TTSE 双轨自演进](TTSE.md)。
...
| **启用技能交响乐** | `symphony.enabled` | `false` | 注册 `symphony_read_score`、`symphony_refresh_score`、`symphony_compose_score` 等技能总谱与编排工具 |
...
| `symphony.paths.score_dir` | 空字符串 | 技能总谱产物目录；为空时使用运行时默认目录 |
| `symphony.fingerprint.scan.max_depth` | 空 | 技能文件扫描最大深度；为空时使用运行时默认值 |
| `symphony.fingerprint.extraction.workers` | `4` | 技能指纹抽取并发数 |
| `symphony.fingerprint.extraction.batch_size` | `2` | 技能指纹抽取批大小 |
| `symphony.fingerprint.extraction.body_limit` | 空 | 单个技能正文参与抽取的长度限制；为空时使用运行时默认值 |
| `symphony.fingerprint.normalization.workers` | `4` | 技能指纹归一化并发数 |
| `symphony.fingerprint.normalization.batch_size` | `2` | 技能指纹归一化批大小 |
| `symphony.fingerprint.normalization.duplicate_name_similarity_threshold` | `0.8` | 判断技能名称近似重复的相似度阈值 |
| `symphony.fingerprint.normalization.max_vocab_size` | 空 | 动态词表最大规模；为空时使用运行时默认值 |
