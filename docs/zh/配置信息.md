...
## 14. 技能同步配置（Server / Client 分离部署）

Server 与 Client 分离部署时，两边的技能库（`~/.jiuwenswarm/agent/workspace/skills/`）各自独立。技能同步接口（`/skill-sync/*`，含差异对比 / 打包下载 / 上传应用）允许 client 将本地技能与 server 同步。该能力**默认关闭**，仅在显式配置后启用。

### 14.1 配置项

以下配置位于主配置文件（`config.yaml`），需手动修改：

| 配置项 | 说明 | 默认值 |
| --- | --- | --- |
| `skill_sync.enabled` | 是否启用技能同步接口 | `false` |
| `skill_sync.token` | 同步接口 Bearer token，**必须非空才能启用**；三接口（diff / package / apply）统一校验 | 空（未启用） |
...
| 环境变量 | 说明 |
| --- | --- |
| `JIUWENSKILL_SYNC_TOKEN` | 覆盖 `skill_sync.token`，用于容器 / 分离部署注入；设置后无需修改配置文件即视为已配置 token |

> ⚠️ **安全提示**：
> - 未配置 token（配置与环境变量均为空）时接口一律返回 503，**不存在"空 token 放行"**；
> - token 用于 diff / package / apply 三个接口的统一鉴权（错误 token 返回 401）；
> - 生产环境必须使用 HTTPS（或等价的部署层加密 / mTLS）传输；
> - 配额：apply 上传 60MB（超限 413）；package 批量打包 100 技能 / 5000 文件 / 解压内容 100MB / 响应 zip 50MB（上传上限高于打包上限，为 multipart 开销留余量）。

接口协议细节（SkillDigest 清单格式、diff 状态机、错误码、`checksum_algo_version` 算法版本对齐要求、builtin 技能只拉不推的客户端规则）见 [技能文档](技能.md) 的「Server / Client 分离部署：技能同步接口」一节。
