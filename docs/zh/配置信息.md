...
当 `react.context_engine_config.enabled=true` 时，即使运行配置或基准测试配置只覆盖了 `react.context_engine_config` 的一部分，JiuwenSwarm 也会保留默认的 `reasoning_tool_loop_compact_config`。如果基准测试配置整体替换该段，应保留或明确关闭 `reasoning_tool_loop_compact_config`；否则重复 reasoning / tool 循环会失去压缩保护。
...
| `react.task_loop_no_progress_guard` | 无人值守 task-loop 连续返回空或近空 no-tool answer 时停止循环 | `enabled: true`, `max_consecutive_empty_answers: 3`, `min_answer_chars: 20` |
