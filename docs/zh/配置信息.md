...
| 界面项 | 对应配置 | 默认值 | 作用 |
| --- | --- | --- | --- |
| **检索档位** | `code_graph.profile` | `off` | `off`：原来的 grep / read / edit；`graph`：挂 `find_*`，能完整建图时藏 grep / glob |
| **挂载点** | `code_graph.agent` | `root`（产品模板） | `root` 挂主对话；`code_agent` 挂子代理（需打开 `code_agent`） |
| **最大文件数** | `code_graph.max_files` | `5000` | 超过则不建图，恢复 grep |
| **最大源码字节** | `code_graph.max_source_bytes` | `41943040` | 源码体积上限 |
| **建图内存上限（MB）** | `code_graph.max_build_rss_mb` | `4096` | 建图过程内存硬停 |
| **索引磁盘上限（MB）** | `code_graph.max_cache_size_mb` | `2048` | 索引磁盘硬停 |

`profile: off` 时图闲置，不刷新、不作废。安全配置的工具表会列出 `resolve_symbol`、`find_callers` 等图工具，便于单独设权限。详情见 [Code Graph](CodeGraph.md)。
...
Code Graph 默认关闭。打开后获得 `find_*` 检索工具，用来定位符号和调用关系，然后再改代码。挂在 Root 还是 Code Agent 由 `code_graph.agent` 决定；Plan / Explore 不会挂这些工具。

**改 yaml 之前先自己装 parser**：`uv pip install tree-sitter-language-pack`。语法在 `jiuwenswarm-init` / `jiuwenswarm-start` 时下载。保持 `profile: off` 时不挂图、不藏 grep。缺失或下载失败时也不挂 `find_*`，行为与关闭相同。

在主配置中设置：

```yaml
code_graph:
  profile: "graph"   # off（默认）或 graph；graph 需要先自行安装 tree-sitter-language-pack
...
### Q: 把 `code_graph.profile` 改成 `graph` 后 `find_*` 不可用？

A: yaml 只打开工具挂载。还要自己装 `tree-sitter-language-pack`；语法在 init/start 时下载。缺失或下载失败时不挂 `find_*`，Coding Agent 仍是原来的 grep / read / edit。保存后会热更新，请等几秒再试。说明见 [Code Graph 代码检索](CodeGraph.md)。
