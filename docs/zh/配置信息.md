...
| **视频视觉模型** | 视频理解与分析，支持视频内容问答、场景识别等         | 需具备**视频理解**能力，能处理视频输入                   | ⭕ 可选   |
...
| **视频生成模型** | 根据文本描述生成视频，支持文生视频等                 | 需具备**视频生成**能力，能根据文本生成视频               | ⭕ 可选   |

> 💡 **提示**：默认模型是系统运行的基础，必须正确配置。视频视觉、音频、视觉、图片生成、视频生成模型为可选配置，当您需要相应多模态能力时再配置即可。其中图片生成模型暂不在前端配置面板中展示，需通过主配置（`models.image_gen`）或环境变量（`IMAGE_GEN_API_BASE` 等）配置。
...
| `model_provider` | `client_provider`     | 模型协议/供应商类型，决定请求格式与鉴权方式    | 支持 `OpenAI`、`DeepSeek`、`DashScope`、`SiliconFlow`、`InferenceAffinity`、`OpenRouter`、`OpenAIAccount`，用于适配不同服务商的 API 格式；视频/音频/视觉模型当前仅支持 `OpenAI`，视频生成模型当前仅支持 `DashScope` |
