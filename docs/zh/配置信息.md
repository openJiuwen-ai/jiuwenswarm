...
**改 yaml 之前先自己装 parser**：`uv pip install tree-sitter-language-pack`。语法在 `jiuwenswarm-init` / `jiuwenswarm-start` 时下载。缺失或下载失败会提示并走 grep。
...
完整说明见 [Code Graph 代码检索](CodeGraph.md)。
...
A: yaml 只打开工具挂载。还要自己装 `tree-sitter-language-pack`；语法在 init/start 时下载。缺失或下载失败会提示 `Download tree-sitter-language-pack to enable Code Graph. Falling back to grep.`，查询走 grep。说明见 [Code Graph 代码检索](CodeGraph.md)。
