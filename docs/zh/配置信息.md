...
- **其他配置**：第三方服务、自演进配置、上下文压缩、Code Graph、技能交响乐等
...
| `model_provider` | `client_provider`     | 模型协议/供应商类型，决定请求格式与鉴权方式    | 支持 `OpenAI`、`DeepSeek`、`DashScope`、`SiliconFlow`、`InferenceAffinity`、`OpenRouter`、`OpenAIAccount`，用于适配不同服务商的 API 格式；视频/音频/视觉模型当前仅支持 `OpenAI` |
...
列表首位为默认使用的模型；可点击「设为主对话默认」按钮调整首位。
...
配置面板 **其他配置 → Code Graph** 对应 yaml 的 `code_graph` 段。只改这里即可开关图检索，不必手改 yaml。

| 界面项 | 对应配置 | 默认值 | 作用 |
| --- | --- | --- | --- |
| **检索档位** | `code_graph.profile` | `off` | `off`：关闭。`graph`：按符号与调用关系 |
| **挂载点** | `code_graph.agent` | `root`（产品模板） | `root` 挂主对话；`code_agent` 挂子代理（会自动打开该子代理） |
| **最大文件数** | `code_graph.max_files` | `5000` | 超过则不建图 |
| **最大源码体积（MB）** | `code_graph.max_source_bytes` | 界面 `40`，yaml `40MB` | 源码体积上限，可填 `40` / `40MB` / `1GB` |
| **建图内存上限（MB）** | `code_graph.max_build_rss_mb` | `4096` | 建图过程内存硬停 |
| **索引磁盘上限（MB）** | `code_graph.max_cache_size_mb` | `2048` | 索引磁盘硬停 |

`profile: off` 时图闲置，不刷新、不作废。安全配置的工具表会列出 focused 五件套：`resolve_symbol`、`find_code_symbols`、`search_source_text`、`inspect_code_structure`、`focus_code`，便于单独设权限。产品 `graph` 没有 `find_callers` / `read_symbol` 等 classic 工具。详情见 [Code Graph](CodeGraph.md)。
...
- **配置项**：`swarmflow.enabled`
- **默认值**：`false`（关闭）
- **作用**：启用后，Team 模式下会显示多智能体协作的流程图和实时监控面板

> 💡 **说明**：SwarmFlow 提供可视化的工作流编排和监控能力，帮助用户理解多智能体协作的执行过程。关闭此开关不影响 Team 模式的基础功能，仅关闭流程视图的展示。
...
| 配置项 | 默认值 | 说明 |
| --- | --- | --- |
| `proactive_recommendation.interval_minutes` | `60` | 主动推荐的调度间隔（分钟） |
| `proactive_recommendation.max_recommendations` | `5` | 单次推荐的最大建议数量 |
...
Code Graph 默认关闭。打开后获得 focused 检索工具（`resolve_symbol` / `find_code_symbols` / `search_source_text` / `inspect_code_structure` / `focus_code`），用来定位符号和调用关系，然后再改代码。挂在 Root 还是 Code Agent 由 `code_graph.agent` 决定；Plan / Explore 不会挂这些工具。

**改 yaml 之前先自己装 parser**：`uv pip install tree-sitter-language-pack`。语法在 `jiuwenswarm-init` / `jiuwenswarm-start` 时下载。保持 `profile: off` 时不挂图、不藏 grep。缺失或下载失败时也不挂图工具，行为与关闭相同。

在主配置中设置：

```yaml
code_graph:
  profile: "graph"   # off 或 graph；graph 需要先自行安装 tree-sitter-language-pack
  agent: "root"      # 产品模板默认；也可 code_agent
  max_files: 5000
...
文件数和源码体积决定能不能进图。内存和磁盘是建图过程中的硬停：先丢掉本仓旧图，再按创建时间从旧到新清没有窗口在用的其它仓；仍不够才清本仓并提示抬上限。抬了热加载会再试，没抬一直用 grep。进了上限的仓会等到图建完。保存后热更新。手册类文本默认不进索引；覆盖率报告、打包产物、前端构建缓存等通用生成目录默认跳过。完整说明见 [Code Graph 代码检索](CodeGraph.md)。

---

## 常见问题

### Q: 把 `code_graph.profile` 改成 `graph` 后图工具不可用？

A: yaml 只打开工具挂载。还要自己装 `tree-sitter-language-pack`；语法在 init/start 时下载。缺失或下载失败时不挂图工具，Coding Agent 仍是原来的 grep / read / edit。保存后会热更新，请等几秒再试。说明见 [Code Graph 代码检索](CodeGraph.md)。
...
A: 保存后系统会自动检测并热重载（大部分配置项即时生效，少数需要进程重启）。`code_graph` 也走同一条热更新。如果改的是实例配置（`~/.jiuwenswarm-instances/<name>/config/config.yaml`），确认改的是正在运行的那一份。长时间未生效时先检查 YAML 格式。
