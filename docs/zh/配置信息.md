...
在主配置中启用 Team 和 Code Team 模式下原生队友的自动路径请求：
...
当 `read_file` 返回 `File not found:` 时，队友会向实际 Leader 发送消息，
包含尝试路径、解析后的路径和工作目录，请求可访问的绝对路径。当前调用
结束，但不会将任务标记为完成。后续 Leader 消息可以启动新调用并重试读取。
错误检测和请求生成不调用模型。

同一解析路径的重复失败在会话状态中只保留一个待处理请求，Rail 重建后
仍然有效。只有成功读取该确切路径才会清除请求；读取其他同名文件不会
清除原请求，但成功读取新路径后仍可正常继续工作。

该功能不会搜索、复制文件、更改权限、拦截其他工具或修复 PDF 解析错误。
Leader 仍需提供有效路径。消息入队不保证送达，已经并行执行的调用无法
由该 Rail 撤销。找不到 Leader 或消息入队失败时返回阻塞结果。请在开始
新对话前配置此选项。
