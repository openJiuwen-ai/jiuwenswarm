...
- **openyuanrong 已正常安装部署**：执行本脚本前，需确保 openyuanrong 已在本机上安装并启动，集群处于正常运行状态。
- 操作系统：Linux
- 系统架构：amd64 或 arm64
- 硬件资源：单机最低配置为 16 核 CPU 及 32GB 内存
- 多网卡环境需通过 `--ip` 显式指定本机使用的 IP 地址
...
| 变量                  | 说明                                                                                               | 默认值    |
| ------------------- | ------------------------------------------------------------------------------------------------ | ------ |
| `BIND_IP`            | 本机使用的 IP 地址。多网卡环境需显式指定，避免自动探测不准。也可通过命令行 `--ip` 参数指定，命令行优先级更高。不设置时默认自动探测本机 IP | `""`   |
| `YR_PYTHON_VERSION` | Python 版本，用于安装 jiuwenswarm 依赖（当前仅支持3.11）                                                       | `3.11` |

#### jiuwenswarm 配置

| 变量                        | 说明                                                                                                           | 默认值  |
| ------------------------- | ------------------------------------------------------------------------------------------------------------ | ---- |
| `YR_FUNC_CODE_DIR`        | jiuwenswarm 函数代码目录（本机上的路径）。留空则自动从本机 pip 安装路径推断：`$(pip show jiuwenswarm Location)/jiuwenswarm/extensions` | `""` |
| `JIUWENSWARM_PACKAGE_URL` | jiuwenswarm 安装包 URL（本机未安装 jiuwenswarm 时使用此 URL 进行 pip 安装）。如果本机已安装 jiuwenswarm，此配置不需要设置                   | `""` |
...
| `EXTENSION_DIRS`            | 扩展包搜索目录（本机上的路径，多个目录用分号 `;` 分隔）。留空则自动从 jiuwenswarm 安装路径推断：`<pip_location>/jiuwenswarm/extensions` | `""`                |
...
# 指定本机 IP 部署所有模块（多网卡环境）
./deploy.sh up --ip 192.168.1.1

# 仅部署 jiuwenswarm 模块（安装jiuwenswarm + 函数注册）
./deploy.sh up jiuwenswarm --ip 192.168.1.1

# 仅部署 gateway 模块（需先部署 jiuwenswarm 或在 .env.custom 中配置 FUNCTION_ID 等参数）
./deploy.sh up gateway --ip 192.168.1.1
```
...
```
# 卸载所有模块
./deploy.sh down --ip 192.168.1.1

# 仅卸载 gateway
./deploy.sh down gateway --ip 192.168.1.1

# 仅卸载 jiuwenswarm
./deploy.sh down jiuwenswarm --ip 192.168.1.1
```

### 一键重启

```
./deploy.sh restart --ip 192.168.1.1
```
...
```
./deploy.sh up --ip 192.168.1.1       # 部署所有模块（jiuwenswarm + gateway）
./deploy.sh down --ip 192.168.1.1     # 卸载所有模块
./deploy.sh restart --ip 192.168.1.1  # 重启所有模块
```
...
- `jiuwenswarm`：在本机安装 jiuwenswarm + 在 yr master 节点注册函数
- `gateway`：jiuwenswarm Gateway 服务（进程模式）

单模块操作示例：

```
./deploy.sh [操作命令] jiuwenswarm --ip 192.168.1.1   # 仅操作 jiuwenswarm 模块
./deploy.sh [操作命令] gateway --ip 192.168.1.1       # 仅操作 gateway 模块
```
...
- `--ip`：指定本机使用的 IP 地址，多网卡环境需显式指定以避免自动探测不准。也可在 `.env.custom` 中通过 `BIND_IP` 配置，命令行参数优先级更高。不指定时默认自动探测本机 IP
- `-h, --help`：显示帮助信息

参数使用示例：

```
./deploy.sh up --ip 192.168.1.1                          # 指定本机 IP 部署所有模块
./deploy.sh up jiuwenswarm --ip 192.168.1.1              # 仅部署 jiuwenswarm
./deploy.sh down gateway --ip 192.168.1.1                # 仅卸载 gateway
./deploy.sh up                                              # 默认本机部署所有模块
```

## 重要约束

- **前置条件**：执行 deploy.sh 前需确保 openyuanrong 已在本机上正常安装并启动
- **jiuwenswarm 模块**：在本机安装 jiuwenswarm
- **多机部署**：每个节点各自执行 deploy.sh，不再通过 SSH 远程执行。多网卡环境需通过 `--ip` 指定本机 IP
