...
本次沿用上述功能和现有界面，仅新增下面的权限确认修复和 Qwen 回复语言配置完善。此前 PR 的功能已作为基线存在，不重复提交其代码，也不合入 `develop` 或 `fix/Multi-language-support` 的其他历史及任务协议重构。
...
移植 `fix/Multi-language-support` 的语言逻辑，提供“跟随对话 / 简体中文 / 英语”配置，默认跟随对话，在下次启动全双工时生效。会话回复与任务结果播报使用同一配置，替代 Qwen 原先跟随全局 `preferred_language` 的行为。

此配置仅适用于 Qwen，设置界面也仅在 Qwen 模式下显示；JoyAI 保持现有语言逻辑。
...
- 后端 `test_video_live.py`：106 passed，覆盖权限确认、连续确认、取消及语言配置。
